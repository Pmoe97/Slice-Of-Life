...
| `src/ref/wip/CONTINUOUS-SIMULATION-ROADMAP.md` | **Umbrella for five linked plans, none built.** Retires the 30-minute tick as the unit NPC decisions happen in: `SCHEDULES`, `resolveScheduleActivity`, and every tick-counted field across the codebase convert to `clockToAbsolute`-space commitments. Holds the cross-cutting decisions `C1`–`C8` all five plans inherit (the action/drive boundary, anchors, movement, determinism, closed-form fast-forward, geometric perception carried forward as an unscheduled thesis). Documentation only, design session 2026-08-14. |
| `src/ref/wip/continuous-behavior-engine-plan.md` | The core of the five — `npc.commitment` (real absolute-minute completion times) replaces `npc.pursuit`'s tick-counted hold; event-driven decision scheduling (each NPC's own next-decision time) replaces the flat per-tick scan of every active NPC; routine survives as a time-of-day scoring weight, not a hard `blockFilter` gate; absorbs `floorplan-and-movement-plan.md`'s position/walk/render-split work as its own physical-output layer. 6 phases, none started. |
| `src/ref/wip/needs-and-heartbeat-plan.md` | Needs decay/restoration, phone battery, and memory decay move off the 30-minute tick onto their own fine periodic heartbeat (`HEARTBEAT_MINUTES`, proposed 1) — one more accumulator inside the existing continuous loop. Fast-forward always resolves in closed form, never by looping the heartbeat. `hunger` (already derived via `HUNGER_RHYTHM`) is explicitly excluded from the per-minute conversion. 4 phases, none started. |
| `src/ref/wip/external-world-retiming-plan.md` | The visit spine, restaurant hours, food-delivery ETAs, and gig work blocks convert from `[startTick,endTick)` to `clockToAbsolute`-space windows. Restaurant hours convert as a *recurring* daily rule (minutes-from-midnight, still wrap-aware) rather than a one-shot window like the other three — the plan's one genuine subtlety. Day rollover, rent, and bills confirmed out of scope. 4 independent phases, none started. |
| `src/ref/wip/npc-initiative-retiming-plan.md` | Every `*CooldownTicks` field and the shared `isOnCooldown`/`setCooldown` mechanism (`drives.js`) convert to absolute-minute comparison, closing the representational hole behind **D34** (`npc-initiative-plan.md`'s finding: a cooldown at or above `ticksPerDay` could never elapse) rather than re-tuning around it. This session's cross-check against `continuous-behavior-engine-plan.md` found a second, previously unclaimed wrapped-delta site sharing the same field — `cognition.js`'s `recencyMultiplier` — folded into the same phase. 3 phases, none started. |
| `src/ref/wip/decor-economy-plan.md` | A new "Home" computer app sells furniture through Nile's exact checkout/delivery mechanism (`checkoutCart`/`processDeliveriesForDay`), unmodified. Placing a delivered piece writes a real object instance into `gameState.objects` — not a `ROOM_DECOR` visual entry — so `continuous-behavior-engine-plan.md`'s anchor resolution finds player-furnished rooms the same way it already finds a stove. Structural changes (walls, doors, room type) stay `STRUCTURAL_UPGRADES`/RenoFix, a deliberately separate catalog. 3 phases, none started. |
