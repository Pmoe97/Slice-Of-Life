...
//
//   node src/src/dev/verify/verify-c5.js
//
...
const srcOf = (f) => require('fs').readFileSync(require('path').join(__dirname, '..', '..', 'srcfiles', f), 'utf8')
  .replace(/\/\*[\s\S]*?\*\//g, ' ')
  .replace(/^\s*\/\/.*$/gm, '')
  .replace(/([^:])\/\/.*$/gm, '$1');
const SRCFILES = require('fs').readdirSync(require('path').join(__dirname, '..', '..', 'srcfiles')).filter(f => f.endsWith('.js'));
