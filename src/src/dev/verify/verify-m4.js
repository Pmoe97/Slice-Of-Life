...
//
//   node src/src/dev/verify/verify-m4.js
//
...
//      w.speed × gameSeconds and lands the walk at the anchor.
...
const fs = require('fs');
const srcOf = (f) => fs.readFileSync(path.join(__dirname, '..', '..', 'srcfiles', f), 'utf8')
  .replace(/\/\*[\s\S]*?\*\//g, ' ')
  .replace(/^\s*\/\/.*$/gm, '')
  .replace(/([^:])\/\/.*$/gm, '$1');
...
check('a cross-room walk starts at the start point and ends exactly at the anchor point', api(`
  (() => {
    ...
    return Math.hypot(w.path[0].x - sx, w.path[0].y - sy) < 0.01 &&
           ...
           w.totalUnits > 0 && w.coveredUnits === 0 &&
           Math.abs(w.speed - w.totalUnits / ((w.completesAtAbs - w.startedAtAbs) * 60)) < 1e-6;
  })()
`));
...
console.log('\nD9 — the live integrator: game-time at w.speed');
check('coveredUnits advances by exactly w.speed × gameSeconds', api(`
  (() => {
    const g = __mk();
    const id = __ids(g)[0];
    g.npcs[id].location = 'bedroom_1';
    ...
    const halfMinutes = (w.totalUnits / w.speed / 60) * 0.5;
    advanceFrameWalks(g, halfMinutes);
    return g.npcs[id].walk &&
           Math.abs(g.npcs[id].walk.coveredUnits - w.speed * halfMinutes * 60) < 1e-9;
  })()
`));
check('a walk completes when its units are covered — at the anchor, arrived flips', api(`
  (() => {
    ...
    const needed = npc.walk.totalUnits / npc.walk.speed / 60;
