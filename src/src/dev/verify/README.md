...
technique described in `src/src/ref/structural/ARCHITECTURE.md`** for anything
...
6. **`loadgame.js`'s `ORDER` must list every file `index.html` loads.** A new
   `src/src/srcfiles/*.js` needs a line in BOTH. `rumination.js` shipped in
...
8. **Always pass `required:` to `loadEngine()`.** Without it the loader
   ...
   harnesses pointed at `srcfiles/` instead of `src/src/srcfiles/`, and six used a
...
9. **Paths are repo-root-relative, two levels up.** From `dev/verify/`:
   `path.join(__dirname, '..', '..', 'srcfiles')` for sources,
