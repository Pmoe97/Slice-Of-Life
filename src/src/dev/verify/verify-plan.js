...
console.log('\n   seconds, derived from the map (D9 → D15/D19)');
// Phase 0 of npc-avatar-liveliness: the per-room tier (10s NPC / 3s player
// per hop) absorbs the old per-doorway beat, and the in-room legs stay
// distance-derived. These assert the NPC tier — the user-facing ask was NPC
// walking speed — and the next-door and cross-flat numbers it produces.
const secs = (a, b) => api(`resolveWalk(__g, '${a}', '${b}', 'npc').seconds`);
...
check('even the longest NPC walk stays under 90 game-sec', far < 90, `${far.toFixed(1)}s`);
check('walk time is still DERIVED from geometry, not authored (D15)', api(`
  (() => {
    const before = resolveWalk(__g, 'living_room', 'dining', 'npc').seconds;
    const save = JSON.parse(JSON.stringify(ROOM_LAYOUT.dining));
    // Make the dining room enormous. The per-room beat is constant, but the
    // in-room leg (door midpoint → the room's centre) is distance-derived,
    // so a bigger room must still mean a longer walk — just by less than
    // before the per-room tier absorbed the geometry.
    ROOM_LAYOUT.dining = [[40, 355, 170, 300]];
    const after = resolveWalk(__g, 'living_room', 'dining', 'npc').seconds;
    ROOM_LAYOUT.dining = save;
    return after > before;
  })()
`), 'the per-room beat is constant, but the in-room leg must stay geometric');
...
`));
check('every walk tier has a positive per-room cost', api(`
  Object.entries(WALK.secondsPerRoom).every(([tier, n]) => typeof n === 'number' && n > 0)
`));
