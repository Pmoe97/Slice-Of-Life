...
function resolveActionAnchor(gameState, actionId, actorId) {
  ...
  if (bestObj) {
    // D9/D10 (npc-avatar-liveliness Phase 2): the stand-point computation now
    // lives in defs.placement.js. resolveObjectStandPoint applies the priority
    // placed obj.pos → authored ROOM_DECOR placement → resolveAutoPlacements
    // footprint → room centroid, branching authored-vs-auto exactly as
    // renderRoomFurniture does, and offsets to the object's drawn edge (or
    // centre for lie-on/sit-in surfaces). Pure and deterministic, same inputs
    // as before.
    return { roomId, objId: bestObj.id, point: resolveObjectStandPoint(gameState, roomId, bestObj) };
  }
  const [cx, cy] = roomCentre(roomId);
  return { roomId, objId: null, point: { x: cx, y: cy } };
}
