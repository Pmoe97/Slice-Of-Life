...
// Phase 3 (D14): at the start of every conversation turn, FILTER the scene's
// present/active/ambient sets against live co-location with the player —
// never recompute. getSceneParticipants (sim.js) returns a fresh
// engagement: {} every call, so reassigning from it would silently wipe the
// engagement counters advanceEngagement builds and assessSceneIfFull reads,
// plus every ambient→active promotion the scene has earned (D14). Returns a
// NEW scene state; the input is not mutated.
function reconcileScenePresence(sceneState, gameState) {
  const roomId = gameState?.player?.location;
  const keep = (id) => {
    const npc = gameState?.npcs?.[id];
    return !!npc && npc.location === roomId;
  };
  return {
    ...sceneState,
    present: (sceneState?.present || []).filter(keep),
    active: (sceneState?.active || []).filter(keep),
    ambient: (sceneState?.ambient || []).filter(keep),
  };
}

// Phase 3 (D12): the DEPARTURE AWARENESS prompt block, destination-typed.
// Called by buildScenePrompt ONLY when doConvSend set
// context.departureAwareness (the turn the awareness line is emitted) or
// context.departureAck (the one-player-beat ack turn) — it must never
// self-compute each turn, or the line would re-emit on later turns and break
// D13's one-beat ack. Returns '' (and keeps the prompt's blank-line rhythm)
// when neither flag is set.
function conversationDepartureLine(context, gameState) {
  const npc = gameState?.npcs?.[context.conversationNpcId];
  const name = npc?.bible?.name || 'they';
  if (context.departureAwareness) {
    const dep = context.departureAwareness;
    const room = ROOMS[dep.destRoomId]?.name || dep.destRoomId;
    const when = dep.minutesAway > 0 ? `in about ${Math.max(1, Math.round(dep.minutesAway))} minutes` : 'shortly';
    const going = dep.destType === 'offflat' ? `out ${when}` : `to ${room} ${when}`;
    let tier;
    if (dep.destType === 'shared') {
      tier = `They are heading to ${room} and are happy for the player to come along — invite them ("join me if you like") in their own voice, implying the conversation can continue there.`;
    } else if (dep.destType === 'privacy') {
      tier = `They need a bit of privacy in ${room}. Signal it firmly but warmly — no invitation to follow.`;
    } else {
      tier = 'They have to leave the flat (usually for work). This is a real farewell — say a proper goodbye.';
    }
    return `\n- DEPARTURE AWARENESS: ${name} is planning to go ${going}. ${tier} Say this line, then the conversation ends after the player's one final reply — find a naturally stopping point. Do not drag the exchange out.`;
  }
  if (context.departureAck) {
    const dep = context.departureAck;
    const room = ROOMS[dep.destRoomId]?.name || dep.destRoomId;
    const going = dep.destType === 'offflat' ? 'out (off to work)' : `to ${room}`;
    return `\n- ${name} just told the player they were about to leave (heading ${going}), and the player's message above is a reply to that. ${name} should acknowledge it naturally in ONE brief line — a goodbye, a "see you there", a "don't let me keep you" — and then be ready to leave. This is the FINAL exchange of the conversation. Do not continue the topic at length and do not ask follow-up questions.`;
  }
  return '';
}

// Bug report (2026-08-30) — canned room-entry greetings. The scene prompt gave
// the model no guidance for a room-entry beat, so it greeted every arrival with
// the same "back already / took you long enough, fair enough" frame, byte-for-
// byte at times (the entry turns at 19:39 and 20:49 in one save were identical).
// Fired only on the literal entry turn ('You walk into the room.' is the exact
// playerAction ui.js passes for a room change) — never on a real message. Its
// job: there is no question to answer, so the NPC must not treat the player's
// arrival as something to comment on, and must never repeat an earlier arrival
// line (the anti-copy rule below also covers this).
function buildTurnTypeDirective(context, playerAction) {
  if (playerAction === 'You walk into the room.') {
    return `\nTURN TYPE — the player just walked into the room and has NOT said anything yet.
- Do NOT greet the player, do not remark on their presence or return, and do NOT say "back already", "you again", or anything like it — the player did not keep anyone waiting; they simply walked in mid-moment.
- If someone in the room has a genuine reason to speak unprompted (they were waiting to tell the player something, they need to ask something, something just happened worth mentioning), that NPC speaks in their own words — and it must be a DIFFERENT line from any arrival line already in the transcript.
- Otherwise let the room settle: narration shows what the people in the room are doing, and nobody has to say anything. One short line, or none.
- Never reuse or reword an arrival/greeting line that appears in [Memories — recent].`;
  }
  return '';
}
...
- Read "Player's action" again before writing. Your dialogue MUST directly respond to the specific content of that line — answer the question that was asked, address what was actually said or done. Never answer a topic category or a vibe instead of the message itself; if the reply could be swapped into any other exchange unchanged, it is wrong.
- NEVER reuse a line from [Memories — recent] — not a dialogue line, not a thought, not a physical beat, not an arrival or greeting line. Every line must be new text. If you recognize phrasing you already wrote, rewrite it. Repeating yourself verbatim is the single most visible failure in this conversation.
- The narration must ADVANCE the scene: show new motion, new engagement, or a new sensory detail that reacts to the player's action. Do not re-describe a character's established pose, body, or the room in the same words as earlier narration. A character can keep lying where they are, but the narration must say what CHANGES or what is new — not re-state the pose.
- actions (the *...* beats) must be meaningful reactions to the player's action. No filler micro-actions used to pad a turn — blinking, tilting the head, shifting weight, adjusting posture — unless that exact movement is a real reaction to what just happened. If nothing new happens physically, emit fewer actions (even zero) rather than recycling one.
- internal thoughts must vary in structure and content, and should reference something SPECIFIC that was just said or done (the swim, the question, the towel, the doorway) rather than a generic rephrase of the player's mood. Never reuse a thought frame ("he's definitely…") turn after turn, and never repeat a thought verbatim.
- topic must describe WHAT the exchange was concretely about (e.g. "swimming plans", "moving to the bedroom", "a missing towel") — never an emotional tone label ("sexual anticipation", "tension"). A topic label that repeats in the transcript is a failure to track variety.
...
- Do not break the fourth wall. Do not describe the format. Just tell the story.`;
  ...
  // Bug report (2026-08-30) — canned room-entry greetings. Fires only on the
  // literal room-entry turn; the directive lands near the output contract so
  // the model reads it right before composing (see buildTurnTypeDirective).
  prompt += buildTurnTypeDirective(context, playerAction);
...
function buildNpcBlockV2(npc, query, channel, day, gameState) {
  ...
  // [History]. Correctness plan Phase 5 reserved this for "a future plan" —
  // no plan since ever claimed it (grepped knowledge-gossip-memory-plan.md,
  // the roadmap's Plan 4: zero mentions). Every generated or hand-authored
  // character carries a full backstory paragraph that has never once reached
  // a conversation — only the terse want/wound/blindSpot/boundary tags below
  // did. Wired in rather than left reserved for a claim that never came.
  if (b.history) block += `[History]: ${b.history}\n`;
  ...
  block += '\n';
  // Bug report (2026-08-30): the old wording ("tics: honestly, right?" +
  // Things they say: "fair enough") read to the model as a required format,
  // and every line came out as "honestly… X, right? fair enough." Reworded so
  // they are unmistakably rare seasoning — flavor added occasionally, never an
  // opener/closer/format, never per-line.
  if (b.speech.verbalTics?.length > 0) {
    block += `  Speech flavor (RARE — at most ONE of these per line, never in most of their lines; they are NOT a required opener, closer, or format): ${b.speech.verbalTics.join(', ')}\n`;
  }
  if (b.speech.catchphrases?.length > 0) {
    block += `  Catchphrases (only occasional — a handful per conversation at most, never as a filler ending or tag): ${b.speech.catchphrases.map(c => `"${c}"`).join(' ')}\n`;
  }
  // `sampleLines` was reserved for "Plan 4 — few-shot voice examples" (the
  // correctness plan's Phase 5 table) and generated by every AI character fill
  // ("3-5 lines of dialogue in their actual voice") for exactly this purpose,
  // but no session ever wired it to a live prompt — it only ever reached the
  // Character Studio's profile view. Worded the same "cadence, not script" way
  // as catchphrases/verbalTics above, per the 2026-08-30 bug report on taking
  // seasoning too literally.
  if (b.sampleLines?.length > 0) {
    block += `  Example lines in their voice (cadence and vocabulary only — do NOT repeat these verbatim or reuse their exact wording): ${b.sampleLines.map(l => `"${l}"`).join(' ')}\n`;
  }
...
// --- Verbatim copy-loop guard (bug report 2026-08-30) ---
// The writer sees its own transcript in [Memories — recent] and, under
// repetition pressure, re-emits prior lines word-for-word (the same dialogue
// line twice, the same internal thought twice). The prompt rules above cut
// the frequency; these helpers enforce it mechanically:
//   findTranscriptDuplicates — returns every produced dialogue/thought/action
//     line that already appears in the speaker's scene transcript.
//   retryWithDedupRule — one focused second attempt that bans the offending
//     lines. One retry max; a second failure is accepted rather than burning
//     more generations on a stubborn model.
function normalizeDupText(text, isAction) {
  let s = String(text || '').toLowerCase();
  if (isAction) s = s.replace(/^\*+|\*+$/g, '');
  return s.replace(/[^a-z0-9]+/g, ' ').trim();
}

function findTranscriptDuplicates(proposal, context) {
  const recentLines = new Set();
  for (const npc of context.activeNpcs || []) {
    for (const e of (npc.memory?.recent || [])) {
      if (!e.text || (e.channel || 'scene') !== 'scene') continue;
      const type = e.type || (e.speaker === 'player' ? 'player_input' : 'dialogue');
      if (type === 'dialogue' || type === 'internal') {
        recentLines.add(normalizeDupText(e.text, false));
      } else if (type === 'action') {
        recentLines.add(normalizeDupText(e.text, true));
      }
    }
  }
  const dups = [];
  const check = (line, isAction) => {
    if (!line || typeof line !== 'string') return;
    const norm = normalizeDupText(line, isAction);
    if (norm.length < 15) return; // too short to be a meaningful repeat
    if (recentLines.has(norm)) dups.push(String(line).trim());
  };
  for (const d of proposal.dialogue || []) check(d.text, false);
  check(proposal.internal, false);
  for (const a of proposal.actions || []) check(a, true);
  return dups;
}

async function retryWithDedupRule(basePrompt, dups) {
  const ban = dups.slice(0, 3).map(d => `"${d}"`).join(', ');
  const extra = `\n\nHARD RULE: Your previous attempt repeated lines that already appear in the conversation transcript. You must NOT repeat them — or any other transcript line — verbatim: ${ban}. Keep the same scene and characters, but write every line freshly.`;
  try {
    const response = await root.generateText({ instruction: basePrompt + extra, startWith: '{' });
    let jsonStr = response.trim();
    if (!jsonStr.startsWith('{')) jsonStr = '{' + jsonStr;
    if (!jsonStr.endsWith('}')) jsonStr = jsonStr + '}';
    try { return JSON.parse(jsonStr); } catch (e) { /* fall through to brace-match */ }
    ...
    if (lastValid > 0) {
      try { return JSON.parse(jsonStr.substring(0, lastValid + 1)); } catch (e2) {}
    }
  } catch (e) {
    console.warn('LLM dedup retry failed:', e.message);
  }
  return null;
}
...
async function callLLM(context, playerAction) {
  ...
  try {
    ...
    // Bug report (2026-08-30) — the verbatim copy-loop, enforced. If any
    // produced line duplicates a prior line in the speaker's scene transcript,
    // retry ONCE with an explicit ban on the offending lines (see the guard
    // helpers above). A second failure is accepted rather than burning more
    // generations on a stubborn model.
    const dups = findTranscriptDuplicates(proposal, context);
    if (dups.length > 0) {
      console.warn('LLM repeated its own transcript lines; retrying with a ban:', dups);
      const retryProposal = await retryWithDedupRule(prompt, dups);
      if (retryProposal && (retryProposal.narration || retryProposal.dialogue)) {
        const retried = stripWriterJudgement(retryProposal);
        const { valid: retryValid } = validateProposal(retried, context);
        if (retryValid) proposal = retried;
      }
    }
