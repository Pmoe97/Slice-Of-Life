...
const ICONS = {
  ...
  `),
  // actions-and-activities-overhaul-plan.md Phase 1: the Calendar app — a
  // page with two hangers and a header rule, the standard calendar glyph.
  calendar: () => svgWrap(`
  ...
  `),
  // actions-and-activities-overhaul-plan.md Phase 14 (D23): DailyGrid — a
  // 3x3 crossword grid with its center cell blacked out. The key must exist
  // or the tile renders blank (the documented `upgrades` landmine above).
  puzzles: () => svgWrap(`
    <rect x="3" y="3" width="18" height="18" rx="2"/>
  ...
  `),
  // actions-and-activities-overhaul-plan.md Phase 15 (D24): Chatter — a
  // heart over a caption line, reading as "social feed" distinct from `im`'s
  // speech-bubble. The key must exist or the tile renders blank (the
  // documented `upgrades` landmine above).
  social_feed: () => svgWrap(`
