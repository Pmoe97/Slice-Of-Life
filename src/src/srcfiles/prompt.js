...
- A character's listed speech tics and catchphrases are RARE seasoning, never a format. Never open line after line with the same tic, never close every line with one, and never build a rigid "tic … content … tic" template out of them — a line built on a repeated tic template reads as a loop, not a person. A tic or catchphrase should appear at most once every few lines; several consecutive tic-free lines is healthy. If the transcript already shows a tic or catchphrase used over and over, STOP using it entirely — more of it is never the fix.`;
}
