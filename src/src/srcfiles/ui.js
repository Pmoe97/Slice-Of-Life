...
async function advanceAndResolve(ticks, opts = {}) {
  ...
  // here: discrete callers save their own boundary and the continuous
  // checkpoint path never saves (matches narrateOvertureArrivals above).
  checkConversationWalkOut();
...
async function doKnock(roomId) {
  ...
  // (30 game-minutes) PLUS a manual decayPlayerNeeds(tickMinutes) — the
  // time was charged twice and a 3-second overture cost half an hour of
  // decay. Now it costs BOUNDARY.durationMinutes.knock through the exact-
  // minutes path, which decays player needs exactly once for the true span.
  await advanceAndResolveMinutes(BOUNDARY.durationMinutes.knock);
  render(currentGameState, currentSceneState);
...
async function doUnlockDoorFromOutside(roomId) {
  ...
  // 2026-08-30: same overcharge as doKnock — a full 30-minute tick + manual
  // decay for a 2-second turn of a key. Now the exact-minutes path.
  await advanceAndResolveMinutes(BOUNDARY.durationMinutes.unlock_door);
  render(currentGameState, currentSceneState);
...
const ENERGY_GATE_EXEMPT = new Set([
  ...
  // npc-avatar-liveliness Phase 2b (D20): moveToNpc is a travel action — it
  // IS a move (doMove under the hood), so it must stay reachable at 0 energy
  // for exactly the same reason 'move' is.
  'moveToNpc',
...
async function handleAction(action, npcId, extra) {
  ...
  switch (action) {
    ...
    case 'move':
      ...
      break;
    // npc-avatar-liveliness Phase 2b (D20): reverse overture — click an NPC's
    // floor-plan avatar to walk the player TO the person, not just the room.
    // npcId arrives directly from the dispatcher's [data-avatar-id] branch.
    case 'moveToNpc':
      if (npcId) await doMoveToNpc(npcId);
      break;
...
// Chat-image persistence (2026-08-31): an image bubble is a moment, not a
// frame — it must survive closing and reopening the conversation. Every chat
// image ALSO becomes a record in npc.flags._convImages (capped), which
// convRenderImages re-paints as data-past bubbles above the 'Now' separator
// on reopen, filling pixels from the shared LRU on demand. The reroll
// closure on the LIVE bubble and the re-painted bubble both reference the
// same persisted record object, so an edit survives both paths.
const CONV_IMAGE_CAP = 40;
function convPushImage(record) {
  const npc = currentGameState?.npcs?.[convState?.npcId];
  if (!npc || !record) return;
  npc.flags = npc.flags || {};
  npc.flags._convImages = npc.flags._convImages || [];
  npc.flags._convImages.push(record);
  if (npc.flags._convImages.length > CONV_IMAGE_CAP) {
    npc.flags._convImages.splice(0, npc.flags._convImages.length - CONV_IMAGE_CAP);
  }
  if (typeof saveAtBoundary === 'function') {
    // Fire-and-forget — saveAtBoundary is async, so swallow rejections
    // (the real game state saves fine; a synthetic/fake state must not
    // surface an unhandledrejection here).
    try { Promise.resolve(saveAtBoundary('conv-image', currentGameState)).catch(() => {}); } catch (e) { /* non-fatal */ }
  }
}
...
// Image-generating variant of the typing bubble (2026-08-31): same animated
// dots, prefixed with an image icon, shown while a chat image is being
// generated (ask photos, conversation-scene panels). Same remove-closure
// contract as convShowTyping.
function convShowGeneratingImage() {
  const log = document.getElementById('conv-log');
  if (!log) return null;
  const el = document.createElement('div');
  el.className = 'conv-typing conv-typing-image';
  el.setAttribute('aria-label', 'Generating image');
...
// --- Phase 3 (npc-avatar-liveliness, D11/D12/D13/D21): departure awareness ---
// The conversation layer watches its partner for an imminent departure. These
// helpers are the presentation of that watch: pure reads for the rechecks,
// and the two end paths (natural handoff vs. "they're already gone") that
// close the session, the overlay and the time context exactly like Leave
// does, plus the [Join] button a shared invite earns (D21).

// Is the conversation partner actually still here to reply? Co-located AND
// not mid-walk-out. Reads only — never writes sim state (invariant 1).
function conversationPartnerPresent(gs, npcId) {
  const npc = gs?.npcs?.[npcId];
  if (!npc || npc.location !== gs?.player?.location) return false;
  if (npc.walk && walkDestRoom(npc) !== npc.location) return false;
  return true;
}

// The destination phrase for the end-of-conversation narration line, typed by
// destType like the D12 prompt tiers.
function departureDestPhrase(dep) {
  if (dep.destType === 'offflat') return 'out for work';
  return `to ${roomPhrase(dep.destRoomId)}`;
}

// End a conversation because its partner is leaving or has left. Captures the
// departure record BEFORE closeConversationOverlay (which nulls convState).
// mode: 'handoff' = the ack turn completed, the NPC walks away naturally;
// 'gone' = the partner vanished mid-turn, no farewell was said; 'silent' =
// the partner left a room the player isn't in (nothing to narrate). opts.save
// === false skips the save — the continuous clock loop's checkpoint path and
// the advanceAndResolve hook don't persist (their callers do).
function endDepartureConversation(npcId, mode, opts = {}) {
  const dep = convState?.departure || null;
  const npc = currentGameState?.npcs?.[npcId];
  const name = npc?.bible?.name || 'they';
  const hadOverlay = !!convState;
  if (hadOverlay) {
    closeConversationOverlay();
    if (currentGameState) popTimeContext();
  }
  ...
  if (mode === 'silent') {
    ...
    addLogEntry('narration', `${name} heads ${departureDestPhrase(dep)}.`);
  } else {
    addLogEntry('narration', `${name} has already stepped out.`);
  }
  render(currentGameState, currentSceneState);
  if (opts.save !== false) saveAtBoundary('conv-departure', currentGameState);
}

// Hook for advanceAndResolve's post-processing: if the current conversation's
// partner walked out while the batch ran (discrete actions, or the continuous
// clock loop's checkpoints), end the session. Silent when the partner left a
// room the player isn't in — no narration for someone they can't see. The
// continuous checkpoint path never saves here (matches runSimCheckpoint's
// existing no-save discipline); discrete callers save their own boundary.
// Keyed off the OPEN OVERLAY (convState), not the session record: the
// partner's location change inside the batch (settleWalks/advanceFrameWalks
// snapping them into the next room) has ALREADY deleted player.conversation
// via activeConversationSession's lazy location-check, so a hook that only
// looked at the session would see nothing and leave the overlay hanging open
// over a dead talk. A paused talk (overlay closed, convState null) is the
// avatar bubble's lazy-check domain, not this hook's.
function checkConversationWalkOut() {
  const gs = currentGameState;
  if (!gs) return;
  const npcId = convState?.npcId;
  if (npcId) {
    if (!conversationPartnerPresent(gs, npcId)) {
      const sameRoom = gs.npcs?.[npcId]?.location === gs.player?.location;
      endDepartureConversation(npcId, sameRoom ? 'gone' : 'silent', { save: false });
    }
    return;
  }
  ...
  if (!sess) return;
  const id = sess.npcId;
  if (conversationPartnerPresent(gs, id)) return;
  const sameRoom = gs.npcs?.[id]?.location === gs.player?.location;
  endDepartureConversation(id, sameRoom ? 'gone' : 'silent', { save: false });
}

// The [Join] button (D21): a shared invite (destType 'shared') earns the
// player a way to actually go along — clicking walks the player to the
// partner's destination via the established doMove flow, so both end up
// co-located where the talk can continue. Rendered into the log as a chip
// after the awareness reply.
function convRenderJoinButton(dep) {
  const log = document.getElementById('conv-log');
  if (!log) return;
  const npc = currentGameState?.npcs?.[dep.npcId];
  const name = npc?.bible?.name || 'them';
  const room = ROOMS[dep.destRoomId]?.name || dep.destRoomId;
  const btn = document.createElement('button');
  btn.className = 'conv-join';
  btn.textContent = `Join ${name} to ${room}`;
  btn.addEventListener('click', async () => {
    const npcId = dep.npcId;
    const destRoomId = dep.destRoomId;
    closeConversationOverlay();
    if (currentGameState) popTimeContext();
    endConversationSession(currentGameState);
...
async function doConvSend(forcedText, giftDefId, borrowDefId, returnDefId, featurePhotoId) {
  ...
  try {
    ...
    // Phase 3 (D11): presence recheck BEFORE the turn. If the partner walked
    // out while the player was composing (or the talk was reopened onto
    // someone already gone), there is no one to answer — bail out cleanly:
    // session + overlay end, a narration line, no ghost reply, no ask
    // effects.
    ...
    // Phase 3 (D14): reconcile the scene's present/active/ambient sets
    // against live co-location BEFORE the prompt is assembled — a partner
    // who left mid-batch must not linger in CHARACTERS PRESENT. Keeps the
    // scene's engagement counters intact (never reassigned from
    ...
    // Phase 3 (D12/D13): departure awareness, staged exactly once per turn
    // by reading the stored probe — never self-computed by the prompt. An
    // existing probe (npcId matches) advances the one-beat ack: beat
    // counter drops to 0 and THIS turn is tagged as the ack (the last
    // exchange). A fresh probe stores the departure and tags this turn as
    // the awareness line.
    let departure = convState?.departure;
    if (departure?.npcId === myNpcId) {
      departure.beatsLeft -= 1;
      if (departure.beatsLeft <= 0) context.departureAck = departure;
    } else {
      const imminent = imminentDeparture(currentGameState, myNpcId, CONVERSATION.departureWatchWindowMinutes);
      if (imminent) {
        departure = { ...imminent, npcId: myNpcId, beatsLeft: 1 };
        convState.departure = departure;
        context.departureAwareness = departure;
      }
    }
    ...
    // Phase 3 (D11): presence recheck AFTER the reply — if the partner left
    // while the model was writing (game time is still flowing at 1 game-sec
    // per real-sec), the freshly generated reply must not be painted: there
    // is no one there to have said it. The session ends with the "already
    // stepped out" narration; the reply and any ask effects are dropped.
    if (!conversationPartnerPresent(currentGameState, myNpcId)) {
      endDepartureConversation(myNpcId, 'gone');
      return;
    }
    ...
    // painted. An ack turn (the stored probe's beat counter reached 0) ends
    // the talk with the handoff narration — the player's last reply was
    // answered, and the partner walks away. A shared invite (destType
    // 'shared') earns the [Join] button so the player can actually go along.
    const depState = convState?.departure;
    if (depState?.npcId === myNpcId && depState.beatsLeft <= 0) {
      endDepartureConversation(myNpcId, 'handoff');
      return;
    }
    if (context.departureAwareness?.destType === 'shared') {
      convRenderJoinButton(context.departureAwareness);
    }
...
async function runAskPhotoFlow(askTurn, context) {
  ...
  // 2026-08-30: the photo record's discriminator is the per-NPC serial
  // (nextAskPhotoSerial), not the ladder streak — see asks.js. Each accepted
  // photo request is its own image; the shared cache still dedupes reloads.
  const serial = nextAskPhotoSerial(npc);
  // 2026-08-31 (photo-request fix): LLM drafting pass — rewrite the player's
  // raw request into a grounded, identity-specific image prompt (asks.js
  // draftAskPhotoPrompt) before it reaches the image generator. Deterministic
  // per serial (persisted on npc.flags); null on failure falls back to the
  // old composition.
  let draft = null;
  try { draft = await draftAskPhotoPrompt(currentGameState, npc, askTurn.flavor, serial); }
  catch (e) { console.warn('Ask-photo draft skipped:', e); }
  const record = buildAskPhotoRecord(currentGameState, npc, convState?.npcId, askTurn.flavor, day, serial, draft);
  ...
  convSetStatus('In conversation');
  if (url) {
    // 2026-08-31 (chat-image persistence): the bubble's record is the
    // durable copy — convPushImage stores it on the NPC and the reroll
    // closure edits it in place, so the sent photo survives reopen and
    // reroll (image.js rerollChatImage / getChatImageUrl).
    const chatRec = {
      ...
      prompt: record.prompt,
      seed: record.seed,
      negativePrompt: IMAGE_NEGATIVE.photo,
    };
    convPushImage(chatRec);
    ...
    convAddImageBubble('npc', url, record.caption, '📷 Photo', chatRec);
  } else {
...
async function doConvSharePhoto(photoId) {
  ...
  // 2026-08-31 (chat-image persistence): share the SAME durable record the
  // camera roll and the re-painted chat bubble reproduce from — rerollChatImage
  // routes 'shared' records back through the camera-roll photo (rerollPhotoImage)
  // so the roll and the chat always show the same picture.
  const record = {
...
async function doMove(targetRoomId) {
  ...
  // Phase 0 (D19): the player walks on the faster tier — resolveWalk threads
  // it to walkSeconds so the clock, the blocker math and the move cost can
  // never disagree about the same doorway.
  const walk = resolveWalk(currentGameState, currentRoom, targetRoomId, 'player');
...
// Clicking an NPC's avatar on the floor plan walks the player TO the person.
// The destination and the "gone before you could reach them" line come from
// resolveMoveToNpc (pure, reads sim state only — invariant 9); doMove does
// the actual room-level walk, so its blocked-door handling, scene/stealth
// narration, scene open, assessor pass and save all apply unchanged. The
// feedback line is added after the move so it reads after the arrival beat;
// addLogEntry persists it via queueWrite. No-op (resolveMoveToNpc → null) is
// the D20 case for an NPC already off-map or already co-located. On arrival
// at a stationary NPC the standard talk affordance is simply present — the
// player arrived where they stand, so the "Talk to {name}" chip surfaces
// (the flagged D20 completion; deliberately no auto-open of the overlay).
async function doMoveToNpc(npcId) {
  if (!npcId || !currentGameState) return;
  const plan = resolveMoveToNpc(currentGameState, currentGameState.player.location, npcId);
  if (!plan) return;
  await doMove(plan.targetRoomId);
  if (plan.feedback) addLogEntry('narration', plan.feedback);
}
...
async function approveCastAndStartGame() {
  ...
  try {
    ...
    await Promise.all(Object.entries(pendingCast.npcs).map(async ([id, npc]) => {
      ...
      // src/src/ref/complete/contractor-tutorial-overhaul-plan.md Phase 1) — keep the
...
function showImageInfoFloatBtn(imgEl) {
  ...
  // 2026-08-31: never float the ⓘ over the conversation header. When the
  // hovered image lives inside the open conversation, the button's floor is
  // the header's bottom edge — and if the image has scrolled fully up under
  // the header (no visible part left), hide the button instead of letting it
  // sit on the header.
  let minTop = 6;
  const conv = document.getElementById('conversation-overlay');
  if (conv && conv.hasAttribute('data-open') && conv.contains(imgEl)) {
    const header = conv.querySelector('.conv-header');
    if (header) minTop = header.getBoundingClientRect().bottom + 6;
  }
  if (r.bottom < minTop) { hideImageInfoFloatBtn(); return; }
  btn.style.left = Math.max(6, Math.min(innerWidth - 34, r.right - 30)) + 'px';
  btn.style.top = Math.max(minTop, r.top + 6) + 'px';
...
function attachEventHandlers() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!target) {
      // npc-avatar-liveliness Phase 2b (D20): a click on an NPC's floor-plan
      // avatar walks the player TO the person, not just the room. Only NPC
      // markers are clickable — the player's own marker stays inert so the
      // room rect underneath still takes its click (see the
      // .fp-people [data-avatar-id] rules in index.html).
      const avatar = e.target.closest('[data-avatar-id]');
      if (avatar && avatar.getAttribute('data-avatar-id') !== 'player') {
        const npcId = avatar.getAttribute('data-avatar-id');
        if (npcId) handleAction('moveToNpc', npcId);
        return;
      }
