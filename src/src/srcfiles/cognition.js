...
// --- Phase 3: conversation departure awareness (D12/D13/D21) ---
// All pure reads — nothing here ever writes sim state (invariant 1); the
// harness asserts the state is byte-identical after imminentDeparture.

// The walk record's destination room: the commitment anchor when it carries a
// real room (a work walk's anchor IS the front-door room, frontDoorAnchor
// above), else the room its final waypoint lands in — the same derivation
// resolveMoveToNpc (movement.js) uses for click-to-NPC targeting.
function walkDestRoom(npc) {
  const w = npc && npc.walk;
  if (!w) return null;
  const anchorRoom = npc.commitment?.anchor?.roomId;
  if (anchorRoom && ROOMS[anchorRoom]) return anchorRoom;
  const wp = w.path && w.path[w.path.length - 1];
  if (wp) {
    const room = roomsContainingPoint(wp)[0];
    if (room) return room;
  }
  return npc?.location || null;
}

// A room the NPC treats as private — the "don't follow" privacy tier (D12):
// a bathroom (bathroom_a/bathroom_b carry no type flag; the id is the only
// test) or the NPC's own bedroom.
function isPrivacyRoom(roomId, npc) {
...
// current room, within windowMinutes? Returns
// { reason, destType, destRoomId, leavesAtAbs, minutesAway } or null.
// destType drives the D12 phrasing tier and whether a [Join] button appears
// (D21): 'shared' (another room in the flat — inviting, joinable), 'privacy'
// (bathroom / own bedroom — don't follow), 'offflat' (leaving the flat for
// work — absolute farewell). destRoomId is ADDITIVE to the plan's locked
// shape (which carried only reason/destType/leavesAtAbs/minutesAway) — D21's
// Join target needs the concrete room.
//
// Three signals, in priority:
//   1. walk-out — the NPC is mid-walk to a different room. (Reachable by the
//      pure function and its harness; in the LIVE conversation D11's presence
//      recheck ends the session before this could surface as an awareness
//      line — mid-walk is D11's bail, not D12's stage.)
//   2. work boundary — the schedule says they should be heading off-site
//      (resolveScheduleActivity block work/commute + npcIsOffsite). An
//      uncommitted NPC is due NOW (nextDecisionAbs = now), so this fires
//      whenever an off-site worker talks mid-shift.
//   3. next room — a schedule block boundary within the window implies an
//      in-flat move: meal-ish blocks (morning/midday/evening) → a shared room,
//      sleep/wind_down → their own bedroom. Deterministic — no drive scorer,
//      no rng, so no C6 cast shift.
function imminentDeparture(gameState, npcId, windowMinutes = 8) {
  const npc = gameState?.npcs?.[npcId];
  if (!npc) return null;
  const clock = gameState.meta.clock;
  const nowAbs = clockToAbsolute(clock);

  // Signal 1 — a walk in flight out of the room they are standing in.
  if (npc.walk) {
    const destRoomId = walkDestRoom(npc);
    if (destRoomId && destRoomId !== npc.location) {
      const completesAtAbs = npc.walk.completesAtAbs ?? nowAbs;
      return {
        reason: 'walk-out',
        destType: npc.commitment?.kind === 'work' ? 'offflat'
          : isPrivacyRoom(destRoomId, npc) ? 'privacy' : 'shared',
        destRoomId,
        leavesAtAbs: completesAtAbs,
        minutesAway: Math.max(0, completesAtAbs - nowAbs),
      };
    }
  }

  // Signal 2 — the schedule has them off-site, so the next decision sends
  // them to the front door. commute_home is deliberately absent: that block
  // is a RETURN, not a departure.
  const sched = resolveScheduleActivity(npc, clock, gameState, npcId);
  if (sched && (sched.block === 'work' || sched.block === 'commute')
      && npcIsOffsite(npc, sched.block, clock, npcId)) {
    const leavesAtAbs = nextDecisionAbs(npc, gameState);
    if (leavesAtAbs - nowAbs <= windowMinutes) {
      return {
        reason: 'work-boundary',
        destType: 'offflat',
        destRoomId: frontDoorAnchor(gameState).roomId || 'entry',
        leavesAtAbs,
        minutesAway: Math.max(0, leavesAtAbs - nowAbs),
      };
    }
  }

  // Signal 3 — a schedule boundary within the window implies an in-flat move.
  const next = nextScheduleBoundary(npc, clock);
  if (next && next.boundaryAbs - nowAbs <= windowMinutes) {
    const b = next.block;
    let destType = null;
    let destRoomId = null;
    if (b === 'sleep' || b === 'wind_down') { destType = 'privacy'; destRoomId = npc.residency?.room || null; }
    else if (b === 'midday' || b === 'evening') { destType = 'shared'; destRoomId = 'dining'; }
    else if (b === 'leisure') { destType = 'shared'; destRoomId = 'living_room'; }
    else if (b === 'morning') { destType = 'shared'; destRoomId = 'kitchen'; }
    else if (b === 'commute' || b === 'work') { destType = 'offflat'; destRoomId = frontDoorAnchor(gameState).roomId || 'entry'; }
    if (destType && destRoomId && destRoomId !== npc.location) {
      return {
        reason: 'schedule-boundary',
        destType,
        destRoomId,
        leavesAtAbs: next.boundaryAbs,
        minutesAway: next.boundaryAbs - nowAbs,
      };
    }
  }
  return null;
