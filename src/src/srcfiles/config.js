...
// what a penthouse of this footprint would be, call it 25 metres.
//
// Walk time is now PER-ROOM, PER-TIER (npc-avatar-liveliness plan Phase 0,
// D15/D19). The user's Q1 ask: each room-to-room transition ≈ 10 game-sec
// and a cross-flat walk ≈ 1 game-min at 1:1 (bedroom_1 → bedroom_3 measures
// exactly 60 game-sec). The player walks on a faster tier (~3 game-sec per
// hop, D19) so catching a moving NPC is the norm — and because the player's
// move only advances the clock ~3s/room, a cross-flat player move costs
// ~21 game-sec rather than ~70. The ROUTE, the door midpoints and the
// interpolation are still pure geometry (D9's map-means-something half of
// the old model survives); only the TIME per hop is authored.
const WALK = {
  // Game-seconds per room-to-room hop, per walker tier. The player tier is
  // ~3x the NPC tier (D19). Tunable; the exact multiplier is a flagged
  // design-session item, not a locked constant.
  secondsPerRoom: { npc: 10, player: 3 },
  // KEPT, retuned 28 -> 11.5 (D15): the in-room leg is still distance-
  // derived, so a same-room walk to a stand-point is a real walk and not a
  // 1-second blur. 11.5 is the effective speed the per-room tiers already
  // produce between rooms (measured across all 342 ordered room pairs,
  // 2026-08-28), so the in-room and cross-room regimes agree instead of
  // contradicting each other.
  unitsPerSecond: 11.5,
  // secondsPerThreshold RETIRED (D15): the per-room tier absorbs the
...
// Presentation layer budget (npc-avatar-liveliness plan Phase 1, D5/D6).
// The real-time catch-up tunables that turn a sim snap into a visible walk.
// floorSec/capSec/shortHopSec are D5's locked catch-up duration floor/cap —
// a catch-up animates over clamp(mechanicalRealSec, floor, cap) real seconds,
// where mechanicalRealSec = the movement's game-seconds / the dilation scale.
// teleportAfterGameMinutes is D6's fade-teleport gate: a catch-up whose gap
// spans more than this much GAME time (sleep 8 hours, hidden-tab catch-up,
// a long wait) presents as a brief fade, not a walk replay — nobody believes
// a single stroll covered eight hours.
//
// liveGapGameSeconds separates a LIVE track from a D22 batch partial-advance:
// when an avatar holds a `walk` record and the game-time gap since our last
// present frame is under this, the walk is being integrated per-rAF by
// advanceFrameWalks and the marker follows the sim directly; over it, the
// record was advanced by a discrete batch (a player move, a wait) and the
// marker catches up ALONG the walk's own path. This is a TIME gate (D6's
// quantity, invariant 7b) — never a per-frame distance, which would be
// refresh-rate and dilation dependent. 2 game-sec sits above every live
// per-frame advance (0.33 at idle 20x, 1.0 at peek 60x, 5 at the x100
// preset, which reads as a smooth pursuit instead) and below the smallest
// player move (>= minSeconds, typically 3+ game-sec per hop).
const PRESENT = {
  ...
  liveGapGameSeconds: 2,  // walk record + game-time gap under this → TRACK
  fadeMs: 350,            // off-map / on-map fade-in/out duration (real ms)
  teleportMs: 450,        // D6 fade-teleport blink duration (real ms)
  lineAboutOpacity: 0.55, // D18 dotted path, about-to-take opacity
  lineActiveOpacity: 0.9, // D18 dotted path, actively-taking opacity
  lineSmooth: 0.12,       // per-frame exponential smoothing toward the target
  // De-overlap (2026-08-29): the floor-plan markers are 36 units across
  // (diameter x2), so the per-frame separation keeps at least a 4-unit gap
  // between any two markers — avatars never overlap, whatever the sim says.
  avatarSeparationMin: 40,
  avatarSeparationIterations: 20,
};

// Conversation departure awareness (Phase 3, D12/D13/D21): how far ahead of
// its next move the conversation layer watches the partner for an imminent
// departure. Game-minutes (absolute-time space, like clockToAbsolute's
// day*1440+minutes). With 30-min ticks this only fires inside the last 8
// game-min of a schedule block or the start of a walk — exactly the window
// where "about to leave" is honest rather than predicted.
const CONVERSATION = {
  departureWatchWindowMinutes: 8,
};
...
const BOUNDARY = {
  ...
  durationMinutes: {
    sleep_with: 30, sleep_watch: 10, throuple: INTIMACY.durationMinutes.sex,
    // 2026-08-30: door socials used to cost a full tick (CLOCK.tickMinutes
    // = 30 game-minutes) each — a knock rounded a 3-second overture to half
    // an hour of decayed needs. They now cost what they are: a knock is a
    // brief exchange, an outside unlock is near-instant.
    knock: 0.5,
    unlock_door: 1,
  },
...
const PEEK = {
  ...
  // of spending quota.
  // 2026-08-30: raised 2/6 → 4/8 so the frame-refresh cadence (every
  // frameRefreshSec while holding) can actually evolve the scene — 2 fresh
  // frames died ~10s into a hold, exactly the "worthless to peek past a few
  // seconds" complaint. 4/8 still caps a long or repeated peeking habit.
  imageBudget: { freshPerSession: 4, freshPerDay: 8 },
  // 2026-08-30: a held peek re-rolls a fresh frame every frameRefreshSec
  // real seconds even when the act hasn't changed, so the scene visibly
  // evolves while you watch instead of freezing on the first frame. Each
  // fresh frame carries a unique cache key (peek.js's frameSeq) so repeated
  // frames are genuinely new art, not the same cached blob — within the
  // image budget above, of course. An act change still re-rolls immediately.
  frameRefreshSec: 5,
  listen: {
...
const DRIVE_DEFS = {
  ...
  shower: {
    ...
    //
    // Bug report (2026-08-30): setsClothing was 'towel' — the POST-shower
    // state from the state machine ('towel — post-shower (transient)') — and
    // it applied at the moment the drive OPENED. A shower holds for 30 min =
    // exactly one sim tick, so there was never a HELD tick for pass 2's
    // activity rule (npcClothingForContext: activity 'showering' + nudeShower
    // → 'nude') to override it: the NPC was towel-wrapped for the whole
    // shower, and every keyhole peek showed a towel (gate closed, 'towel'
    // not in NAKED_CLOTHING_STATES). The in-shower state is 'nude', exactly
    // like the player's own shower (def.transientClothing 'nude') and exactly
    // like the masturbate drive's setsClothing. The leftover 'nude' reverts
    // to 'dressed' on the tick the activity ends (npcClothingForContext's
