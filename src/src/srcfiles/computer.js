...
function workGigBlock(gameState, gigId, device) {
  ...
  // 2026-08-30: scale the gig drain by the F1 need-decay settings — this
  // was a direct write that bypassed both decayPlayerNeeds AND the
  // per-action ADJUST_NEED path, so "limitless energy" left a gig as the
  // one action that still drained the bar.
  gameState.player.energy = clamp(
    gameState.player.energy - GIG_ENERGY_PER_BLOCK * needDecayScaleFor(gameState),
    0, 100
  );
...
// was silently dropped. Their replacement is src/src/srcfiles/concept.js,
