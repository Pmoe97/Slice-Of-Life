...
// How long a walk takes, in game-SECONDS, per tier (D15,
// npc-avatar-liveliness Phase 0). One source of truth for the clock, the
// blocker math and the player's own move cost: planWalk (movement.js)
// returns the same seconds for the same route on the NPC tier, and the
// player's move (doMove) reads this on the player tier. The per-room tier
// (10s NPC / 3s player per hop) absorbs the old per-doorway beat, and the
// in-room legs stay distance-derived so a long room genuinely takes longer
// to cross than a short one (the D9 half of the old model that survives).
function walkSeconds(route, tier = 'player') {
  if (!Array.isArray(route) || route.length < 2) return 0;
  const transitions = route.length - 1;
  let inRoomUnits = 0;
  // First leg: this room's centre → the first doorway's midpoint.
  const seg0 = sharedWallSegment(route[0], route[1]);
  if (seg0) {
    const c0 = roomCentre(route[0]);
    const mid0 = [(seg0.x1 + seg0.x2) / 2, (seg0.y1 + seg0.y2) / 2];
    inRoomUnits += Math.hypot(mid0[0] - c0[0], mid0[1] - c0[1]);
  }
  // Last leg: the last doorway's midpoint → the destination room's centre.
  // (For a 2-room route the two legs are the two halves of the one door.)
  const segLast = sharedWallSegment(route[route.length - 2], route[route.length - 1]);
  if (segLast) {
    const cLast = roomCentre(route[route.length - 1]);
    const midLast = [(segLast.x1 + segLast.x2) / 2, (segLast.y1 + segLast.y2) / 2];
    inRoomUnits += Math.hypot(cLast[0] - midLast[0], cLast[1] - midLast[1]);
  }
  return Math.max(
    WALK.minSeconds,
    WALK.secondsPerRoom[tier] * transitions + inRoomUnits / WALK.unitsPerSecond
  );
}
...
function resolveWalk(gameState, fromRoom, toRoom, tier = 'player') {
  ...
  out.seconds = walkSeconds([fromRoom, ...out.crossed], tier);
  return out;
}
...
// src/src/ref/BrineOS-The-Phone-plan.md says presence is DERIVED from the object's
