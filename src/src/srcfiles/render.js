...
function renderFloorPlanStatic(gs, container) {
  ...
  for (const id of [...Object.keys(gs.npcs || {}), 'player']) {
    svg += `<clipPath id="fp-clip-${id}"><circle cx="0" cy="0" r="18"/></clipPath>`;
  }
  svg += '</defs>';
  ...
  // --- Layer 4.5: the D18 dotted-route overlay (npc-avatar-liveliness Phase
  // 1). Empty here — the presentation layer (movement.present.js) draws one
  // <path class="fp-route"> per NPC into this group per frame. Inserted
  // AHEAD of .fp-people so a route line never paints over a marker.
  svg += '<g class="fp-paths"></g>';

  // --- Layer 5: the live avatar layer (D12). One marker per NPC plus the
  // player, CREATED here, repositioned every frame by the presentation layer
  // (movement.present.js via renderFloorPlanLive) through transform/class
  // attribute mutation only — never innerHTML, never a rebuild. Off-map NPCs
  // (work, dormancy) start hidden; everyone else is placed by pos where one
  // exists, else at their room's centre.
...
const FP_FURNITURE = {
  bed: { ...FP_FOOTPRINTS.bed, draw: (x, y, w, h, s) => {
  ...
  desk: { ...FP_FOOTPRINTS.desk, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  study_desk: { ...FP_FOOTPRINTS.study_desk, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  wardrobe: { ...FP_FOOTPRINTS.wardrobe, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}"/>`
    + `<line class="fp-f-detail-l" x1="${x + w / 2}" y1="${y}" x2="${x + w / 2}" y2="${y + h}"/>` },
  nightstand: { ...FP_FOOTPRINTS.nightstand, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  bookshelf: { ...FP_FOOTPRINTS.bookshelf, draw: (x, y, w, h) => shelfSymbol(x, y, w, h) },
  study_bookshelf: { ...FP_FOOTPRINTS.study_bookshelf, draw: (x, y, w, h) => shelfSymbol(x, y, w, h) },
  desktop_computer: { ...FP_FOOTPRINTS.desktop_computer, draw: (x, y, w, h) =>
    `<rect class="fp-f-detail" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  // --- Bathroom / wet rooms ---
  shower: { ...FP_FOOTPRINTS.shower, draw: (x, y, w, h, s) =>
    `<rect class="fp-f${s?.grime === 'soap-scummed' ? ' dirty' : ''}" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>`
    + `<circle class="fp-f-detail" cx="${x + w / 2}" cy="${y + h / 2}" r="2.5"/>` },
  toilet: { ...FP_FOOTPRINTS.toilet, draw: (x, y, w, h, s) =>
    `<rect class="fp-f${s?.clean === 'dirty' ? ' dirty' : ''}" x="${x}" y="${y}" width="${w}" height="${h}" rx="4"/>` },
  sink_bathroom: { ...FP_FOOTPRINTS.sink_bathroom, draw: (x, y, w, h) => sinkSymbol(x, y, w, h) },
  sink_kitchen: { ...FP_FOOTPRINTS.sink_kitchen, draw: (x, y, w, h, s, o) => sinkSymbol(x, y, w, h, dishLevelOf(o) !== 'clean') },
  bathroom_mirror: { ...FP_FOOTPRINTS.bathroom_mirror, draw: (x, y, w, h) =>
    `<rect class="fp-f-detail" x="${x}" y="${y}" width="${w}" height="${h}"/>` },
  lockers: { ...FP_FOOTPRINTS.lockers, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}"/>`
    + [1, 2, 3].map(i => `<line class="fp-f-detail-l" x1="${x + (w / 4) * i}" y1="${y}" x2="${x + (w / 4) * i}" y2="${y + h}"/>`).join('') },
  changing_bench: { ...FP_FOOTPRINTS.changing_bench, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  // --- Kitchen ---
  stove: { ...FP_FOOTPRINTS.stove, draw: (x, y, w, h, s) => {
  ...
  fridge: { ...FP_FOOTPRINTS.fridge, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>`
    + `<line class="fp-f-detail-l" x1="${x + w - 3}" y1="${y + 2}" x2="${x + w - 3}" y2="${y + h - 2}"/>` },
  freezer: { ...FP_FOOTPRINTS.freezer, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>`
  ...
  dishwasher: { ...FP_FOOTPRINTS.dishwasher, draw: (x, y, w, h, s, o) =>
  ...
  microwave: { ...FP_FOOTPRINTS.microwave, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>`
  ...
  pantry: { ...FP_FOOTPRINTS.pantry, draw: (x, y, w, h) => shelfSymbol(x, y, w, h) },
  kitchen_table: { ...FP_FOOTPRINTS.kitchen_table, draw: (x, y, w, h, s, o) => tableSymbol(x, y, w, h, s, o) },
  dining_table: { ...FP_FOOTPRINTS.dining_table, draw: (x, y, w, h, s, o) => tableSymbol(x, y, w, h, s, o) },
  coffee_table_lr: { ...FP_FOOTPRINTS.coffee_table_lr, draw: (x, y, w, h, s) => tableSymbol(x, y, w, h, s) },
  balcony_table: { ...FP_FOOTPRINTS.balcony_table, draw: (x, y, w, h, s) => tableSymbol(x, y, w, h, s) },
  trash_kitchen: { ...FP_FOOTPRINTS.trash_kitchen, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="2"/>` },
  coffee_maker: { ...FP_FOOTPRINTS.coffee_maker, draw: (x, y, w, h) =>
    `<rect class="fp-f-detail" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  // --- Living / leisure ---
  sofa: { ...FP_FOOTPRINTS.sofa, draw: (x, y, w, h) =>
    `<rect class="fp-f-soft" x="${x}" y="${y + 3}" width="${w}" height="${h - 3}" rx="2"/>`
    + `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="4" rx="1"/>` },
  armchair: { ...FP_FOOTPRINTS.armchair, draw: (x, y, w, h) =>
    `<rect class="fp-f-soft" x="${x}" y="${y + 3}" width="${w}" height="${h - 3}" rx="2"/>`
    + `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="4" rx="1"/>` },
  tv: { ...FP_FOOTPRINTS.tv, draw: (x, y, w, h) =>
    `<rect class="fp-f-detail" x="${x}" y="${y}" width="${w}" height="${h}"/>` },
  pool_table: { ...FP_FOOTPRINTS.pool_table, draw: (x, y, w, h) =>
    `<rect class="fp-f-felt" x="${x}" y="${y}" width="${w}" height="${h}" rx="2"/>`
    + `<circle class="fp-f-detail" cx="${x + w / 2}" cy="${y + h / 2}" r="1.6"/>` },
  game_console: { ...FP_FOOTPRINTS.game_console, draw: (x, y, w, h) =>
    `<rect class="fp-f-detail" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  dartboard: { ...FP_FOOTPRINTS.dartboard, draw: (x, y, w, h) =>
    `<circle class="fp-f" cx="${x + w / 2}" cy="${y + h / 2}" r="${w / 2}"/>`
    + `<circle class="fp-f-detail" cx="${x + w / 2}" cy="${y + h / 2}" r="1.5"/>` },
  treadmill: { ...FP_FOOTPRINTS.treadmill, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>`
    + `<rect class="fp-f-detail" x="${x + 1}" y="${y + 5}" width="${w - 2}" height="${h - 7}" rx="1"/>` },
  weight_set: { ...FP_FOOTPRINTS.weight_set, draw: (x, y, w, h) =>
  ...
  yoga_mat: { ...FP_FOOTPRINTS.yoga_mat, draw: (x, y, w, h) =>
    `<rect class="fp-f-soft" x="${x}" y="${y}" width="${w}" height="${h}" rx="3"/>` },
  swimming_pool: { ...FP_FOOTPRINTS.swimming_pool, draw: (x, y, w, h, s) =>
  ...
  pool_loungers: { ...FP_FOOTPRINTS.pool_loungers, draw: (x, y, w, h) =>
  ...
  // --- Utility / entry ---
  washer: { ...FP_FOOTPRINTS.washer, draw: (x, y, w, h) => applianceSymbol(x, y, w, h) },
  dryer: { ...FP_FOOTPRINTS.dryer, draw: (x, y, w, h) => applianceSymbol(x, y, w, h) },
  laundry_hamper: { ...FP_FOOTPRINTS.laundry_hamper, draw: (x, y, w, h) =>
    `<rect class="fp-f" x="${x}" y="${y}" width="${w}" height="${h}" rx="2"/>` },
  doormat: { ...FP_FOOTPRINTS.doormat, draw: (x, y, w, h) =>
    `<rect class="fp-f-soft" x="${x}" y="${y}" width="${w}" height="${h}" rx="1"/>` },
  shoe_rack: { ...FP_FOOTPRINTS.shoe_rack, draw: (x, y, w, h) => shelfSymbol(x, y, w, h) },
  coat_rack: { ...FP_FOOTPRINTS.coat_rack, draw: (x, y, w, h) =>
  ...
  rug: { ...FP_FOOTPRINTS.rug, draw: (x, y, w, h) =>
    `<rect class="fp-f-soft" x="${x}" y="${y}" width="${w}" height="${h}" rx="2"/>`
...
function renderAutoFurniture(gs, roomId) {
  // The packing itself lives in defs.placement.js (D8): resolveAutoPlacements
  // returns the same footprints, so the anchor resolver (resolveObjectStandPoint)
  // reads exactly what this draws — invariant 2. render.js keeps only the draw()
  // half of FP_FURNITURE and merges the footprints in from defs.placement.js.
  const placements = resolveAutoPlacements(gs, roomId);
  if (!placements) return '';
  let out = '<g class="fp-furniture">';
  const bucket = gs.objects?.[`room_${roomId}`] || {};
  for (const p of placements) {
    const def = FP_FURNITURE[p.defId];
    if (!def) continue;
    const obj = bucket[p.objId];
    out += def.draw(p.x, p.y, p.w, p.h, obj?.state || {}, obj);
  }
  return out + '</g>';
}

// One avatar marker. Created by the static pass with NO meaningful position —
// the presentation layer (movement.present.js) owns placement and mutates
// transform/class attributes per frame (D12, Phase 1). `id === 'player'` is
// the player's marker; everyone else is an NPC. The portrait image fills in later and only from cache — see
...
function avatarMarkerHtml(id, isPlayer, npc, plausible) {
  ...
  // npc-avatar-liveliness Phase 2b (D20): every NPC marker is clickable — the
  // player clicks a person to walk TO them. The player's own marker is not
  // (is-clickable never set for isPlayer), so its click falls through to the
  // room rect underneath.
  if (!isPlayer) cls += ' is-clickable';
  if (plausible) cls += ` plausible-${plausible.tier}`;
  let caption = '';
  if (plausible) {
    caption = `<text class="fp-avatar-activity" x="0" y="27">${escapeHtml(plausibleCaption(plausible.label))}</text>`;
  } else if (isPlayer) {
    const pc = playerFloorCaption(npc);
    if (pc) caption = `<text class="fp-avatar-activity" x="0" y="27">${escapeHtml(plausibleCaption(pc))}</text>`;
  }
  // The native SVG title gives the D20 hover affordance its label — the
  // marker's <g> is what receives the hover (pointer-events are re-enabled on
  // NPC markers in index.html). No player marker, no title.
  const tip = isPlayer ? '' : `<title>Go to ${escapeHtml(npc?.bible?.name || 'them')}</title>`;
  return `<g class="${cls}" data-avatar-id="${id}" transform="translate(0,0)">`
    + tip
    + `<circle class="fp-avatar-bg" cx="0" cy="0" r="18"/>`
    ...
    + `<image class="fp-avatar-img" data-avatar-for="${id}" x="-18" y="-18" width="36" height="36" preserveAspectRatio="xMidYMid slice" clip-path="url(#fp-clip-${id})" href="" hidden="hidden"/>`
    + `<text class="fp-avatar-initials" data-initials-for="${id}" x="0" y="4">${escapeHtml(label)}</text>`
    + `<circle class="fp-avatar-ring" cx="0" cy="0" r="18"/>`
    + caption
    + '</g>';
}
...
// fixed small cost while the static layer sits untouched. Phase 1
// (npc-avatar-liveliness): this entry now hands each frame to the
// presentation layer — movement.present.js's presentFrame, which decides
// per-avatar TRACK-vs-CATCH-UP (D4), animates catch-ups on real time (D1/D5),
// writes the transform/class mutations, and draws the D18 dotted paths.
// floorPlanAvatarPlacement stays right below as the SIM-position source
// presentFrame reads and never mutates (invariant 1). presentFrame also runs
// on its own rAF (D23) so motion survives a paused clock; the two calls per
// frame are idempotent. Every live container (sidebar + large overlay) gets
// the same treatment because presentFrame drives floorPlanContainers itself.
function renderFloorPlanLive(gs) {
  if (typeof presentFrame === 'function') presentFrame(gs);
}
