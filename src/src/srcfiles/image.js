...
function composePeekKey(gs, roomId, npc, actKey) {
  ...
  const stylePart = imageStyleToken();
  // Bug report (2026-08-30): the key now folds the subject's CLOTHING state
  // (shower → 'nude' since the shower-drive fix, was 'towel'). The key never
  // carried it, so frames cached under the old buggy state — a towel-wrapped
  // 'showering' frame — were served forever under the same (room, npc, phase,
  // actKey) tuple. Every other image key in this file folds the clothing the
...
function composePeekPrompt(gs, roomId, npc, actKey, npcId) {
  ...
  // 2026-08-30: the old framing ("at ease in their own space") read as a
  // POSE — the model painted a composed portrait looking at the viewer, so
  // "on a video call" rendered as "standing there, undressed, posed for
  // you". A keyhole view is CANDID: the occupant is mid-action, unaware,
...
// image budget only on genuinely fresh generations. `frameKey` (2026-08-30)
// lets peek.js drive per-frame sequencing: a held peek refreshes under a
// unique `_fN` key each cadence so the scene evolves, while a plain call
// (revisit of a moment) still lands on the canonical deterministic key.
async function getPeekImage(gs, roomId, npc, npcId, frameKey) {
  const actKey = npc.activity || npc.clothing || 'hanging_out';
  const key = frameKey || composePeekKey(gs, roomId, npc, actKey);
  const cached = await getCachedImage(key);
  if (cached) return { url: createObjectUrl(key, cached), cached: true, key };
  try {
...
function actionWindowKeyBase(gs, plan, discriminator) {
  ...
  // 2026-08-31 (action-photo fix): a paired act's frame is about BOTH people,
  // so the key folds the partner's identity + the clothing state the prompt
  // names for them (mirroring the player's own clothing part). No partner
  // (every solo/repetitive verb) → no extra part, so existing keys are
  // unchanged. Different partners can no longer share a cached frame.
  const partner = plan.partnerId ? gs?.npcs?.[plan.partnerId] : null;
  const partnerPart = partner
    ? `_${cutoutIdentityToken(partner, false)}_${plan.partnerClothing || 'dressed'}`
    : '';
  return `${IMAGE_PROMPT_VERSION}_${plan.verbId}_${plan.roomId || 'anywhere'}`
    + `_${playerIdentityToken(gs?.player)}_${clothing}_${discriminator}_${gate}_${sceneOrientation()}${partnerPart}`;
}
...
// def — this file never invents what the verb looked like. 2026-08-31
// (action-photo fix): a paired act (plan.partnerId, threaded from
// result.shared.withIds[0] by actionwindow.js) adds the partner's OWN visual
// clause — before this the phrase named the partner in words but gave the
// diffusion model nobody's body, so "making love with Megan" rendered only
// the player.
function composeActionWindowPrompt(gs, plan) {
  ...
  const partner = plan.partnerId ? gs?.npcs?.[plan.partnerId] : null;
  const pw = (partner && plan.partnerClothing) ? { ...partner, clothing: plan.partnerClothing } : partner;
  const partnerClause = pw ? buildVisualCharacterClause(pw, { gameState: gs, npcId: pw.id, intimate: true }) : null;
  const people = partnerClause ? `${clause} and ${partnerClause}` : clause;
  const roomName = ROOMS[plan.roomId]?.name || plan.roomId || 'apartment';
  const light = phaseLighting(gs?.meta?.clock?.phase);
  return `${people}, ${plan.phrase}, in the ${String(roomName).toLowerCase()}, ${light}, `
...
function captureActionMoment(gameState, def, actionId, actor, ctx) {
  ...
  const roll = gameState.world.phone.camera.roll;
  const slot = roll.length;
  // 2026-08-31 (action-photo fix): paired acts freeze the partner in the
  // moment too — the Make-a-Move pick (ctx.actTargetNpcId) is available here
  // (executeAction sets it before capture), folded into the id/seed so each
  // pairing gets its own reproducible photo, and passed to the prompt builder
  // so the partner's body is actually in the frame.
  const partner = (ctx?.actTargetNpcId && gameState.npcs?.[ctx.actTargetNpcId]) || null;
  const partnerToken = partner ? cutoutIdentityToken(partner, false) : '';
  const id = `photo_${hashStr(`${gameState.meta.seed}|moment|${actionId}|${partnerToken ? partnerToken + '|' : ''}${day}|${tick}|${slot}`).toString(36)}`;
  const seed = hashStr(`${gameState.meta.seed}|photo_seed|${id}`);
  const prompt = buildActionMomentPrompt(actor, def, { roomId, gameState, partner });
...
// caller documents. 2026-08-31 (action-photo fix): when the act is paired,
// ctx.partner adds the partner's own clause (through their DURING-act
// clothing, def.paired.npcClothing) and the partner's name rides in the
// phrase, so a shared act is actually shared on screen.
function buildActionMomentPrompt(actor, def, ctx) {
  ...
  const pw = (partner && def.paired?.npcClothing) ? { ...partner, clothing: def.paired.npcClothing } : partner;
  const partnerClause = pw ? buildVisualCharacterClause(pw, { gameState: ctx.gameState, npcId: pw.id, intimate: true }) : null;
  const people = partnerClause ? `${clause} and ${partnerClause}` : clause;
  const partnerName = pw?.bible?.name || null;
  const phrase = def.momentPhrase || 'in a private, intimate moment, alone';
  const room = ROOMS[ctx.roomId]?.name || ctx.roomId || '';
  return `${people}, ${phrase}${partnerName ? `, with ${partnerName}` : ''}, in the ${String(room).toLowerCase()}, `
    + 'soft intimate lighting, '
    + (sceneOrientation() === 'landscape' ? 'wide composition, subject centered.' : 'tall vertical composition, subject centered.');
}

// ===== F3 — conversation scene visualizer (Discord feedback, 2026-08-23) =====
// A dedicated panel of the current conversation, drawn straight into
...
// this; see buildCharacterPrompt's own note).
//
// 2026-08-31 (chat-image persistence/reroll): was previously a fresh,
// uncached generation with a random seed per call — deliberately ungoverned
// by the camera-roll cache, since nothing was meant to revisit it. Chat
// images now PERSIST in the log (ui.js convRenderImages re-paints them on
// reopen), so a scene panel needs the same frozen prompt+seed contract as
// every other chat image: a deterministic per-panel key (npc + panel index)
// with seed = hash of that key, living in the shared LRU like the rest.
// The beat pool below is what makes two panels of the same conversation
// visibly different frames — now picked deterministically from the panel
// index, so a given panel always reproduces its own beat (D1).
const CONV_SCENE_BEATS = [
  'mid-sentence, gesturing with a hand',
  'leaning in, listening intently',
  'laughing at something that was just said',
  'quiet for a moment, weighing the next words',
  'sitting at ease, talking softly',
  'caught mid-laugh, relaxed',
  'resting an elbow on the table, talking',
  'pacing slightly, animated',
...
async function getAskPhotoImage(record) {
  ...
  if (cached) return { url: createObjectUrl(photoKey, cached), cached: true };
  try {
    // 2026-08-31 (chat-image reroll): a rerolled ask photo stores the EDITED
    // style-applied prompt with promptStyled set (same contract as
    // getPhotoImage), so it is never re-styled here.
    const promptForGen = record.promptStyled ? record.prompt : applyImageStyle(record.prompt);
    const result = await generateImageTracked(promptForGen, {
      resolution: IMAGE_CACHE.resolutions.char,
      seed: record.seed,
      negativePrompt: 'blurry, distorted, extra limbs, low quality, text, watermark',
...
// through the keyhole. `frameKey` is the cache key of the frame actually on
// screen — with the 2026-08-30 frame-sequencing the shown frame may be a
// per-second `_fN` variant rather than the canonical act key, and the reroll
// must overwrite the key the lens is showing, or the new art would never
// appear.
async function rerollPeekFrame(gs, roomId, npc, npcId, imgEl, fields, frameKey) {
  const actKey = npc.activity || npc.clothing || 'hanging_out';
  const key = frameKey || composePeekKey(gs, roomId, npc, actKey);
  try {
    const result = await generateImageTracked(fields.prompt, {
...
// --- Chat image reproduction + reroll (2026-08-31) ------------------------
// Chat image bubbles (ask photos, shared photos, conversation-scene panels)
// are now PERSISTED records (ui.js pushes them to npc.flags._convImages and
// re-paints them on reopen via convRenderImages), so each one carries the
// same frozen prompt+seed contract as a camera-roll photo: reproduce the
// pixels on demand from the shared LRU, and never hold a blob. Record shape:
//   { kind: 'askphoto'|'shared'|'scene', from, tag, caption, day, tick,
//     id?, photoId?, cacheKey?, prompt, seed, negativePrompt, promptStyled? }
//   - askphoto: id is the askphoto record id → getAskPhotoImage (photo_<id>)
//   - shared:   photoId into the camera roll → getPhotoImage
//   - scene:    cacheKey = `${npcId}_${panelN}` → convscene_<cacheKey>
async function getChatImageUrl(record) {
  if (!record) return { url: null, error: 'no record' };
  if (record.kind === 'askphoto') return getAskPhotoImage(record);
  if (record.kind === 'shared') {
    const photo = (currentGameState?.world?.phone?.camera?.roll || []).find(p => p.id === record.photoId);
    return photo ? getPhotoImage(photo) : { url: null, error: 'photo no longer in camera roll' };
  }
  const stylePart = imageStyleToken();
  const cacheKey = `convscene_${record.cacheKey}${stylePart ? '_' + stylePart : ''}`;
  const cached = await getCachedImage(cacheKey);
  if (cached) return { url: createObjectUrl(cacheKey, cached), cached: true };
  try {
    const promptForGen = record.promptStyled ? record.prompt : applyImageStyle(record.prompt);
    const result = await generateImageTracked(promptForGen, {
      resolution: IMAGE_CACHE.resolutions.scene[sceneOrientation()],
      seed: record.seed,
      negativePrompt: record.negativePrompt || IMAGE_NEGATIVE.scene,
    });
    const blob = await canvasToBlob(result.canvas);
    if (!blob) return { url: null, error: 'empty frame' };
    await setCachedImage(cacheKey, blob);
    return { url: createObjectUrl(cacheKey, blob), cached: false };
  } catch (e) {
    console.warn('Chat scene image generation failed:', e.message);
    return { url: null, error: e.message };
  }
}

// Chat-image reroll for the shared info/reroll modal. Mutates the PERSISTED
// record in place (the modal's closure holds the same object convPushImage
// stored), so the edited prompt/seed/negative survive reopen; pixels are
// overwritten in the LRU under the same key the record reproduces from.
// A shared photo reroutes to the camera-roll photo and rerollPhotoImage so
// the camera roll and the chat stay the same picture.
async function rerollChatImage(record, imgEl, fields) {
  if (!record) return { error: 'Image not found.' };
  if (record.kind === 'shared') {
    const photo = (currentGameState?.world?.phone?.camera?.roll || []).find(p => p.id === record.photoId);
    if (!photo) return { error: 'Photo not found.' };
    return rerollPhotoImage(photo, imgEl, fields);
  }
  const neg = fields.negativePrompt || (record.kind === 'askphoto' ? IMAGE_NEGATIVE.photo : IMAGE_NEGATIVE.scene);
  record.prompt = fields.prompt;
  record.promptStyled = true; // edited prompt is already style-applied
  record.seed = fields.seed ?? Math.floor(Math.random() * 2147483647);
  record.negativePrompt = neg;
  const cacheKey = `${record.kind === 'askphoto' ? 'photo_' + record.id : 'convscene_' + record.cacheKey}`;
  const stylePart = imageStyleToken();
  const fullKey = `${cacheKey}${stylePart ? '_' + stylePart : ''}`;
  try {
    const result = await generateImageTracked(record.prompt, {
      resolution: record.kind === 'askphoto' ? IMAGE_CACHE.resolutions.char : IMAGE_CACHE.resolutions.scene[sceneOrientation()],
      seed: record.seed,
      negativePrompt: record.negativePrompt,
    });
    const blob = await canvasToBlob(result.canvas);
    if (!blob) return { error: 'The model returned an empty frame.' };
    await setCachedImage(fullKey, blob);
    if (imgEl) imgEl.src = createObjectUrl(fullKey, blob);
    if (typeof saveAtBoundary === 'function') await saveAtBoundary('chat-image-reroll', currentGameState);
    return { ok: true };
  } catch (e) {
    console.warn('Chat image reroll failed:', e.message);
    return { error: e.message };
  }
}

// Action-outcome-window reroll: overwrites the frame under the window's own
// deterministic key (same contract as rerollPeekFrame — the chosen pixels
// persist, so reopening the same outcome shows the rerolled art).
async function rerollActionWindow(plan, imgEl, fields) {
  const gs = currentGameState || {};
  const key = plan.kind === 'instance' ? composeActionInstanceKey(gs, plan) : composeActionArchetypeKey(gs, plan);
  try {
    const result = await generateImageTracked(fields.prompt, {
      resolution: IMAGE_CACHE.resolutions.scene[sceneOrientation()],
      seed: fields.seed ?? hashStr(key),
      negativePrompt: fields.negativePrompt || IMAGE_NEGATIVE.actionWindow,
    });
    const blob = await canvasToBlob(result.canvas);
    if (!blob) return { error: 'The model returned an empty frame.' };
    await setCachedImage(key, blob);
    if (imgEl) imgEl.src = createObjectUrl(key, blob);
    return { ok: true };
  } catch (e) {
    console.warn('Action window reroll failed:', e.message);
    return { error: e.message };
  }
}
