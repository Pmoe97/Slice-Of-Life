...
// The need-decay multiplier the F1 settings impose (New Game/Sandbox
// "Need decay speed" slider + "Disable need decay entirely" toggle, stored
// on world.gameplayOptions). One choke point for the TIME-decay path
// (decayPlayerNeeds) AND the flat per-action drains (effects.js
// applyAdjustNeed, computer.js gig blocks) so the sliders actually govern
// how fast actions tire the player out, not just how fast the clock does.
// Missing world.gameplayOptions (a save from before this existed, or a bare
// test harness object) reads as scale 1 / disabled false, i.e. today's
// behavior exactly.
function needDecayScaleFor(gameState) {
  const gpo = gameState?.world?.gameplayOptions;
  if (!gpo) return 1;
  return gpo.needDecayDisabled ? 0 : (typeof gpo.needDecayScale === 'number' ? gpo.needDecayScale : 1);
}
...
function decayPlayerNeeds(player, minutes, gameState, options = {}) {
  ...
  // choke point, no per-bar changes.
  const decayScale = needDecayScaleFor(gameState);
