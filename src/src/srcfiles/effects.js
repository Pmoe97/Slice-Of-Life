...
function applyAdjustNeed(p, ctx) {
  ...
  if (p.who === 'player') {
    ...
    } else if (p.need === 'energy' && delta < 0) {
      // 2026-08-30: the F1 need-decay settings used to govern only TIME
      // decay (decayPlayerNeeds' closed form) — every flat per-action
      // drain (workout, gigs, intimacy, games...) still cost full price
      // regardless of "Need decay speed" or "Disable need decay entirely".
      // Energy drains now scale by the same needDecayScaleFor multiplier
      // the clock path uses; restores (shower, nap, relax, eat) stay
      // unscaled, so "limitless energy" stops the bleeding without
      // cancelling the recovery verbs.
      const scaled = delta * needDecayScaleFor(ctx.gameState);
      ctx.gameState.player[p.need] = clamp((ctx.gameState.player[p.need] || 0) + scaled, 0, 100);
    } else {
