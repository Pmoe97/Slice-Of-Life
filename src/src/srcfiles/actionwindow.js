  ...
  if (spec.tier !== 'B' && ow.image) {
    ...
    if (phrase) {
      ...
      spec.image = {
        ...
        // 2026-08-31 (action-photo fix): paired acts now thread their partner
        // into the image plan — result.shared.withIds[0] (the Make-a-Move
        // pick), exactly the source intimacyPartner (defs.actions.js) reads.
        // composeActionWindowPrompt needs the partner's physical identity in
        // the prompt (before this, the phrase named the partner in words but
        // the diffusion model was given nobody's body, so paired-act frames
        // rendered only the player). partnerClothing mirrors the player's
        // plan.clothing override — the partner's DURING-act state, which the
        // live npc.clothing may not hold yet at prompt time.
        partnerId: (result && result.shared && result.shared.withIds && result.shared.withIds[0]) || null,
        partnerClothing: (def.paired && def.paired.npcClothing) || null,
      };
    }
  }
  return spec;
  ...
  if (img) {
    if (s.imageUrl) {
      if (img.src !== s.imageUrl) {
        ...
        // carries — the window is not a special case. 2026-08-31: reroll
        // added (was info-only before), overwriting the frame under the
        // window's own deterministic key.
        if (typeof setImageMeta === 'function' && s.imagePrompt) {
          setImageMeta(img, {
            ...
            reroll: (fields) => rerollActionWindow(s.spec.image, img, fields),
          });
        }
      }
