...
async function startPeekSession(roomId, mode) {
  ...
  const s = {
    ...
    lastActKey: null,
    // 2026-08-30: per-frame sequence for the time-based refresh — each
    // fresh frame gets a unique `_fN` cache key so a held peek shows NEW
    // art on the refresh cadence, never the same cached blob.
    frameSeq: 0,
    _lastFrameAt: null,
    // One generation at a time: a frame takes far longer to draw than the
    // refresh cadence, and without this guard a tick that finds timeDue
    // while a previous frame is still generating would stack overlapping
    // generations (duplicate quota spend, racing src writes).
    _genInFlight: false,
    _viewLine: null,
    _peekImageKey: null,
    _peekImageUrl: null,
...
// refreshed when the scene's act key changes OR the frame-refresh cadence
// elapses (2026-08-30 — a held peek evolves), and only when the budget
// allows; otherwise the lens keeps its last frame (or the shadows line).
async function _refreshView(s, focus) {
  ...
  const now = Date.now();
  const timeDue = !!s._peekImageUrl && (now - (s._lastFrameAt || 0)) >= PEEK.frameRefreshSec * 1000;
  if (!changed && !timeDue) { updatePeekHold(gs, s); return; }
  if (s._genInFlight) { updatePeekHold(gs, s); return; }

  const img = document.getElementById('peek-img');
  const shimmer = document.getElementById('peek-shimmer');
  // A changed act restarts the frame sequence on the canonical key (cached
  // across sessions — revisiting the same moment reuses its art); a
  // time-based refresh continues the sequence under a unique `_fN` key so
  // repeated frames are genuinely new generations, not the same cached blob.
  if (changed) s.frameSeq = 0;
  const baseKey = composePeekKey(gs, s.roomId, focus.npc, desc.actKey);
  const key = changed ? baseKey : `${baseKey}_f${s.frameSeq}`;
  ...
  if (!peekImageBudgetAllows(gs, s)) {
    ...
    // _lastFrameAt paces the budget re-check so this branch isn't re-entered
    // every tick for the whole hold.
    s._lastFrameAt = now;
    if (!s._peekImageUrl) {
  ...
  if (shimmer) shimmer.removeAttribute('hidden');
  s._lastFrameAt = now; // pace the next refresh from THIS frame's start
  s._genInFlight = true;
  let result = null;
  try {
    result = await getPeekImage(gs, s.roomId, focus.npc, focus.npcId, key);
  } finally {
    s._genInFlight = false;
  }
  ...
  if (result && result.url) {
    ...
    // fresh frame for the act being watched, cached under the SAME key the
    // lens is showing (which may be a `_fN` variant, not the canonical act
    // key — the reroll must overwrite what's on screen).
    if (img) {
      setImageMeta(img, {
        label: 'Peek Frame',
        ...
        reroll: (fields) => rerollPeekFrame(gs, s.roomId, focus.npc, focus.npcId, img, fields, key),
      });
    }
