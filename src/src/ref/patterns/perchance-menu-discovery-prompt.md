...
Its output feeds **Phase 10** of `src/src/ref/complete/inventory-needs-menu-saves-plan.md`.

Paste its returned markdown into `src/src/ref/structural/perchance-menu-conventions.md`, then
re-run Phase 10.
...
- The target game already has `root.generateImage` wired through the
  ...
  [image.js:106](../src/src/srcfiles/image.js), and an LRU image cache in
...
- The existing menu to replace is `showMenuModal`
  ([ui.js:2899](../src/src/srcfiles/ui.js)); boot flow is `boot()`
  ([ui.js:2990](../src/src/srcfiles/ui.js)).
