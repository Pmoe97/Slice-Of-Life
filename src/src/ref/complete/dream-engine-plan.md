...
Every phase is covered by the Node suite (`node src/src/dev/verify/run-all.js`); 6 and
...
- `src/src/ref/complete/knowledge-gossip-memory-plan.md` — owns the fact / episode /
...
- `src/src/ref/complete/plan-x5-conversation-consequences.md` — `callDreamweaver`
...
`src/src/ref/complete/dream-engine-handoff-prompt.md` for the full session protocol.
...
session prompt live in `src/src/ref/complete/` now; the only reason to open either
...
- `node --check` on every file in `src/src/srcfiles`.
...
**Files:**
- `src/src/srcfiles/dreams.js` (new): stub only — the section header and a single
  `defaultDreamState()` returning the D-model shape above.
- `src/src/srcfiles/defs.dreams.js` (new): stub only — section header and an empty
  `DREAM_TUNING`.
- `src/src/srcfiles/state.js`: add `dreams` to `SAVE_KEYS.world`; add a
...
- `src/src/srcfiles/sim.js`: initialize `dreams` in `buildGameState`'s `world`
  literal.
- `src/src/srcfiles/defs.settings.js`: three flat fields in `SETTINGS_DEFAULTS`
  (D17) plus their option tables, following `SCENE_VIS_MODES`' shape; three
  cycle rows in the settings screen def.
- `src/src/srcfiles/settings.js`: three normalizer lines in the settings loader
...
**Verification:** `node src/src/dev/verify/run-all.js` still passes with the two new
...
**Files:**
- `src/src/srcfiles/defs.dreams.js`: all eight tables from the Data model section,
...
**Files:**
- `src/src/srcfiles/dreams.js`: `harvestResidue(gs, opts)` → `[fragment]`, plus one
...
**Files:**
- `src/src/srcfiles/dreams.js`: `compileDream(gs, { forSleep, index })`, built on a
...
**Files:**
- `src/src/srcfiles/llm.js`: `buildDreamPrompt(gs, dream)` and
...
**Files:**
- `src/src/srcfiles/image.js`: `composeDreamPanelKey(gs, panel, dream)` folding in
...
- `src/src/srcfiles/dreams.js`: `topUpDreamQueue(gs)` — single-flight via a
...
- `src/src/srcfiles/ui.js`: call `topUpDreamQueue` un-awaited on wake and on day
...
**Files:**
- `src/src/srcfiles/actionwindow.js`: a `body: 'dream'` branch in
  ...
  only on the last.
- `src/src/srcfiles/ui.js`: in `doSleep`, after `saveAtBoundary('sleep', …)` and
  ...
  un-awaited.
- `src/src/srcfiles/defs.actions.js`: `self.nap`'s `outcomeWindow` gains its
...
**Files:**
- `src/src/srcfiles/defs.computer.js`: a `dreams` `APP_DEFS` entry with
  `devices: ['computer','phone']` (D18), `entryScreen: 'diary'`, and two
  screens — a gallery and a detail.
- `src/src/srcfiles/render.computer.js`: two renderers registered in
...
**Files:**
- `src/src/srcfiles/dreams.js`: the `kind:'true'` branch of `compileDream` —
...
- `src/src/srcfiles/dreams.js`: the `kind:'recurring'` branch — pull a diary entry,
  preserve `slots.form`, `cast` and `motif`, re-roll `lens` and `tempo`, shift
  one beat, and stamp `recurrenceOf` (D11).
- `src/src/srcfiles/defs.dreams.js`: `trueDreamChance` and `recurrenceChance` in
