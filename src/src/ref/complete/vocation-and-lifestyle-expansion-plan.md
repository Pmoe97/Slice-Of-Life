...
Companions:
- `src/src/ref/complete/npc-overhaul-plan.md` (owns `rollCastSlot`, the bible schema
  and `OCCUPATION_POOL` — this plan rewrites the occupation record and the roll
  order inside it).
- `src/src/ref/complete/npc-cognition-plan.md` (owns the drive scorer and
  `DRIVE_CANDIDACY`; Phases 5–6 add drives through its existing shapes, never
  a parallel one).
- `src/src/ref/complete/continuous-behavior-engine-plan.md` (owns the `go_work`
  commitment and `WORK_BLOCKS`; D15 changes when that commitment opens).
- `src/src/ref/complete/intimacy-and-voyeurism-overhaul-plan.md` (owns
...
- `src/src/ref/complete/economy-and-rent-plan.md` (owns the tuned rent curve;
  Phase 8 is the only place this plan touches it, and it is deferred).
- `src/src/ref/complete/npc-initiative-plan.md` (owns `OVERTURE_DEFS`; D18's collab
...
`src/src/ref/wip/vocation-and-lifestyle-expansion-handoff-prompt.md` for the full
session protocol.
...
handoff. The document now lives in `src/src/ref/complete/`. The four Phase-7
...
**Files:**
- `src/src/srcfiles/sim.js`: add `npcIsOffsite` beside `npcDisinhibition` (~line
...
- `src/src/srcfiles/cognition.js`: route the work-commitment branch (~1002) and the
  `sched.block === 'work'` fallthrough (~1018) through it. Leave the
  `willReturnAt` scan (~817) alone — it reads the template, not the location.
- `src/src/srcfiles/interruption.js`: route the eligibility gate (~74) through it.
  Behavior identical this phase (offsite ⇒ ineligible).
- `src/src/srcfiles/movement.js`: the two work-walk landings (~167, ~200) keep
...
**Files:**
- `src/src/srcfiles/config.js`: expand `OCCUPATION_POOL` per D2/D10. Roughly:
  ...
  defaults.
- `src/src/srcfiles/sim.js`: add `occupationAffinity`. In `rollCastSlot` (~3500),
...
- `src/src/srcfiles/menu.js` (~742), `src/src/srcfiles/render.computer.js` (~2578),
  `src/src/srcfiles/ui.js` (~5789): the three places that build a category list from
  ...
  authoring UI.
- `src/src/srcfiles/computer.js` (~1201, ~2504, ~2602): stub and external-NPC
...
**Files:**
- `src/src/srcfiles/sim.js`: implement `isOfficeDay` (reads `officeDays`, D4) and
...
- `src/src/srcfiles/config.js`: add at-home work activity strings to
...
- `src/src/srcfiles/cognition.js`: gate the `go_work` opener on `npcIsOffsite` (D15),
  at the top, before `planWalk`.
- `src/src/srcfiles/npc.js`: `outfitTypeForContext` resolves at-home work blocks to
  `daily` (D14). Confirm `change_clothes` candidacy no longer fires daily for
  remote workers.
- `src/src/srcfiles/interruption.js`: implement D13 —
  `INTERRUPTION.workingFromHomeMultiplier` (first pass `0.4`), applied alongside
  the door and phase multipliers.
- `src/src/srcfiles/config.js`: `PEEK_VIEW_ACT` rows for the new work activities, so
...
**Files:**
- `src/src/srcfiles/config.js`: the `none`-mode pool entries (D3), all pointing at
...
- `src/src/srcfiles/llm.js` (~401, ~1150): the persona block currently prints
  ...
  title.
- `src/src/srcfiles/computer.js` (~1267): the browse filter's `incomeBand` list.
...
**Files:**
- `src/src/srcfiles/config.js`: `content_session` in `DRIVES` (D16), modeled on
...
- `src/src/srcfiles/cognition.js`: `DRIVE_CANDIDACY` entries. Both read
...
**Files:**
- `src/src/srcfiles/config.js`: an `OVERTURE_DEFS` row on the `propose` channel
...
- `src/src/srcfiles/config.js`: `content_collab` in `DRIVES` (D19), the pair variant
  of `intimate`.
- `src/src/srcfiles/cognition.js`: `DRIVE_CANDIDACY.content_collab` — reuses
...
- `src/src/srcfiles/overture.js`: no new code path expected. If one is needed, the
...
**Files:** `src/src/srcfiles/sim.js` (incomeRentShare, negotiatedOrDerived, the
two pre-population sites), `src/src/srcfiles/config.js` (`ECONOMY.rent.incomeShare`),
`src/src/srcfiles/render.computer.js` (debug readonly row),
