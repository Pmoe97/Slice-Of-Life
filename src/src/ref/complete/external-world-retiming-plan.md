...
- `src/src/ref/complete/external-world-npcs-overhaul-plan.md` (built the visit
  spine this plan retimes — read for the *why* of `world.visits`, which
  this plan does not re-argue).
- `src/src/ref/complete/restaurant-network-expansion-plan.md` (built the
...
- Phase 4 (D5) — gig work blocks' one time-cost literal: `doGigWorkBlock`
  ...
  workBlockMinutes: 30 }` in `src/src/srcfiles/defs.computer.js`, next to
...
pair.
**Files:** `src/src/srcfiles/sim.js` (`getActiveVisits`, `scheduleVisit`, the
friend-visit roll).
...
**Files:** `src/src/srcfiles/defs.computer.js` (`RESTAURANT_DEFS.hours` for
all twelve restaurants), `src/src/srcfiles/computer.js` (`isRestaurantOpen`,
...
**Files:** `src/src/srcfiles/config.js` (`FOOD_TUNING`), `src/src/srcfiles/computer.js`
(the order-placement and arrival-computation functions cited in Evidence),
`src/src/srcfiles/render.computer.js` (display formatting).
...
**Files:** `src/src/srcfiles/config.js` (new `GIG_TUNING.workBlockMinutes`,
...
existing gig-tuning neighborhood); `src/src/srcfiles/ui.computer.js`
