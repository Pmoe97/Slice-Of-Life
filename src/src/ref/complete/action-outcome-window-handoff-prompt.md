...
- When told to reuse a pattern, go read that code and match its current
  ...
  to mirror:
  - `src/src/srcfiles/peek.js` + `render.js`'s `renderPeekOverlay` — the
  ...
  - `src/src/srcfiles/image.js`'s `composePeekKey` and its siblings — the
    LRU-cache-by-composed-key pattern D5's archetype/instance split reuses
    rather than reinventing.
  - `src/src/srcfiles/effects.js`'s `applyEffects` and its typed effect list —
    the delta strip's only data source; never compute a delta by any other
    means.
  - `src/src/srcfiles/npc.js` / `src/src/srcfiles/overture.js` / `src/src/srcfiles/drives.js`
...
- **Hard technical rules**, each a Design invariant from the plan — repeat
  ...
  - `index.html`'s script tags and `dev/verify/loadgame.js`'s `ORDER` array
    both need a line for any new `src/src/srcfiles/*.js` file, in the same
