...
Companions: `src/src/ref/structural/perchance-menu-conventions.md` (**landed 2026-08-09** — the
...
`src/src/ref/complete/external-world-npcs-overhaul-plan.md` (built — the maid,
DoorDrop, and `world.visits[]`; this plan makes its ~200 delivered dishes
edible and gives its maid a new mess to clean),
`src/src/ref/complete/afterhours-redesign-plan.md` (built — the source of the `+0.25` mood
spike this plan re-bases onto a decaying impulse),
`src/src/ref/complete/economy-and-rent-plan.md` (the cost stack roommates eating your
groceries adds to), `src/src/ref/complete/apartment-expansion-plan.md` (the facility tiers
...
`src/src/ref/complete/inventory-needs-menu-saves-handoff-prompt.md` for the full session
protocol.
...
**Files:**
- `src/src/srcfiles/items.js`: extend the header comment's stack contract with the
...
- `src/src/srcfiles/defs.world.js`: add the categories the later phases need to
...
- `src/src/srcfiles/inventory.js` **(new)**: pure query/sort helpers, no DOM, no
...
- `src/src/srcfiles/render.js`: replace `renderInventory` (line 346) with the
...
- `src/src/srcfiles/ui.js`: action handlers dispatching through the existing
...
**Files:**
- `src/src/srcfiles/defs.world.js`: `container: { capacity, preservation, label }`
  on every item-holding `OBJECT_DEFS` entry, per the preservation table above.
- `src/src/srcfiles/inventory.js`: `containerStacks(obj)`, `transferPlan(from, to,
  defId, qty)` returning the `MOVE_ITEM` lines to emit. One shared
  implementation for every container — no fridge-specific path.
- `src/src/srcfiles/render.js`: the shared container view — contents left, bag
  right, Take / Take All / Put / Put All.
- `src/src/srcfiles/items.js`: `availableRecipes(pool)` returning **all**
...
- `src/src/srcfiles/defs.actions.js`: `self.cook`'s `prepare` (line 357) presents
  the satisfiable recipe list for the player to choose instead of taking
  `pickAvailableRecipe`'s first match.
- `src/src/srcfiles/ui.js`: open-container action wiring; the doormat becomes a
...
**Files:**
- `src/src/srcfiles/defs.actions.js`: replace `self.eat`'s flat
...
- `src/src/srcfiles/inventory.js`: `edibleStacks(gs, ctx)` merging bag + nearby
  container contents; serving math.
- `src/src/srcfiles/defs.world.js`: optional `servings: n` on meal defs. Eating
...
- `src/src/srcfiles/effects.js`: leave the hook for Phase 4's spoiled-food penalty;
...
**Files:**
- `src/src/srcfiles/items.js`: **fix B2 first** — make `addStack`'s merge key
  ...
  transfer.
- `src/src/srcfiles/sim.js`: a daily spoilage pass in the day-rollover path — mark
  stacks Rotten, and after `ROT.graceDays` convert them to a mess.
- `src/src/srcfiles/defs.world.js`: a `rotten_food` state on containers, carrying
...
**Files:**
- `src/src/srcfiles/config.js`: `NEEDS` gains `idleDecayMultiplier` (start `0.25`)
...
- `src/src/srcfiles/sim.js`: `decayPlayerNeeds` (line 1259) takes an options bag
...
- `src/src/srcfiles/effects.js`: `ADJUST_NEED player mood` becomes an impulse
  written into `eventTerm` rather than a direct write to `player.mood`. All
  existing call sites keep their current syntax.
- `src/src/srcfiles/time.js`: `runSimCheckpoint` (line 294) passes `idle: true`;
  `advanceAndResolveMinutes` (line 279) does not.
- `src/src/srcfiles/state.js`: persist the new player fields; add a `player`
...
**Files:**
- `src/src/srcfiles/defs.world.js`: comfort consumables as `ITEM_DEFS` entries
...
- `src/src/srcfiles/defs.actions.js`: one action per hobby object, sourced from it;
...
- `src/src/srcfiles/sim.js` / `src/src/srcfiles/drives.js`: social time pays — sharing
...
- `src/src/srcfiles/config.js`: all new numbers, none inline at the call site.
...
**Files:**
- `src/src/srcfiles/commitments.js` **(new)**: `world.commitments[]` per the data
...
- `src/src/srcfiles/sim.js`: `resolveScheduleActivity` (line 701) checks
  ...
  rather than hope.
- `src/src/srcfiles/computer.js` / `src/src/srcfiles/ui.js`: invite from the IM app or
  in person; pick day and time.
- `src/src/srcfiles/defs.actions.js`: the `set_meal` action at the kitchen or
  ...
  bonus.
- `src/src/srcfiles/state.js`: persist `world.commitments` in `saveAtBoundary`
...
**Files:**
- `src/src/srcfiles/effects.js`: **fix B1 first** — `CONSUME_ITEM` gains a `who`
...
- `src/src/srcfiles/npc.js`: `npc.inventory` seeded at creation from a lifestyle
  template derived from the bible — job, income tier, interests, quirks.
- `src/src/srcfiles/state.js`: `npcs` folder migration (2→3) backfilling
  inventories on existing saves.
- `src/src/srcfiles/prompt.js` / `src/src/srcfiles/llm.js`: possessions into prompt
  context so narration references things NPCs actually own. Cheapest step,
  biggest flavour return.
- `src/src/srcfiles/config.js` (`DRIVE_DEFS`) / `src/src/srcfiles/drives.js`: the
...
- `src/src/srcfiles/stealth.js` / `src/src/srcfiles/ui.js`: searching an NPC's room
...
> `src/src/ref/structural/perchance-menu-conventions.md` documents `hedonism-island`'s
...
**Files:**
- `src/src/srcfiles/state.js`: extract the persisted key list into a single
...
- `src/src/srcfiles/render.js` / new save-menu UI: the slot grid. Cards show day,
...
- `src/src/srcfiles/ui.js`: save/load/overwrite/delete handlers; export a record
...
**Reference:** `src/src/ref/structural/perchance-menu-conventions.md` is the source-level record
...
**Files:**
- `src/src/srcfiles/menu.js` **(new)**: the menu component, used in both contexts —
...
- `src/src/srcfiles/ui.js`: `boot()` (line 2990) currently auto-loads an existing
  ...
  a key chord.
- `src/src/srcfiles/image.js`: the slideshow, adopting §3.4–3.8's component onto
  ...
  deviation 3.
- `src/src/srcfiles/defs.menu.js` **(new)** or a `config.js` block: the compiled
...
| 10 | **Done** | Main menu: boot screen, slideshow, Discord, pause menu, Options (slideshow implements against `src/src/ref/structural/perchance-menu-conventions.md`, deviations 1–5) |
