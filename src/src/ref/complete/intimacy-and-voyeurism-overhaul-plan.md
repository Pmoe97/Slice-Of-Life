...
- `src/src/ref/complete/external-world-npcs-overhaul-plan.md` (the visit spine — outside partners generalize it)
- `src/src/ref/complete/knowledge-gossip-memory-plan.md` (fact provenance + gossip transmission — the scandal engine the consequences ride)
- `src/src/ref/complete/renovation-occupancy-overhaul-plan.md` (per-bedroom facilities, `residentCapacity: 2` — couples sharing a room)
- `src/src/ref/complete/afterhours-redesign-plan.md` (the player's existing masturbation path; the apartment act generalizes it)
- `src/src/ref/complete/needs-and-heartbeat-plan.md` (the heartbeat + closed-form fast-forward the real-time peek loop must respect)
- `src/src/ref/complete/continuous-behavior-engine-plan.md` (commitments/anchors — intimacy acts anchor on beds like everything else)
...
`src/src/ref/complete/intimacy-and-voyeurism-handoff-prompt.md` for the full session
protocol.
...
**Resume at:** None — **ALL 19 PHASES COMPLETE**. The overhaul is shipped and verified. If further work comes, it should be a NEW plan document; this one is archived to `src/src/ref/complete/` (update the `src/src/ref/README.md` + `src/src/ref/structural/ARCHITECTURE.md` indexes — already done in the archive step).
...
**Files:**
- `src/src/srcfiles/render.js`: `renderActionChips` teaches the submenu shape;
...
- `src/src/srcfiles/ui.js`: `handleAction` accepts the submenu verbs unchanged
  (they are ACTION_DEFS entries); add the popover open/close + blur handling.
- `src/src/srcfiles/defs.actions.js`: seed 2–3 demo `submenu` groups (bedroom
...
**Files:**
- `src/src/srcfiles/render.js`: `renderFloorPlan`/`renderFloorPlanLive` —
...
- `src/src/srcfiles/world.js` or `signals.js`: `derivePlausibleActivity` — the
...
and as plan glyphs.
**Files:**
- `src/src/srcfiles/scene.js`: `composeScene` gains a door-cue block when the
  player is adjacent to a door with a cue; text pools per cue kind, keyed,
  never one string.
- `src/src/srcfiles/signals.js`: `deriveDoorCues` (pure) — reads
  `perceiveSignals` + room occupancy + door object state.
- `src/src/srcfiles/render.js` + `index.html`: floor-plan glow glyph for
...
bought.
**Files:**
- `src/src/srcfiles/defs.world.js`: `CLOTHING_DEFS` (a solid starter set across
...
- `src/src/srcfiles/defs.computer.js`: Nile rows for clothing (priced, so
  `SHOP_CATALOG_LIST` picks them up automatically).
- `src/src/srcfiles/items.js`-adjacent helpers: outfit/wardrobe stack helpers
...
gate.
**Files:**
- `src/src/srcfiles/render.js` + `index.html`: the wardrobe panel (slot grid,
...
- `src/src/srcfiles/defs.actions.js`: `self.change_outfit` (submenu on the
  wardrobe object, Phase 1 pattern).
- `src/src/srcfiles/npc.js` + `llm.js`: `clothingLabel`/`getPhysicalDescriptionForPrompt`
...
- `src/src/srcfiles/sim.js`: player outfit persisted on the player; transient
...
**Files:**
- `src/src/srcfiles/config.js`: the `change_clothes` DRIVE_DEFS entry (timeOfDay
...
- `src/src/srcfiles/npc.js`: outfit selection (pure): schedule block + personality
  → outfit type → item pick; clothing state transition rules.
- `src/src/srcfiles/sim.js`: resolveTick applies the clothing-state transitions
...
**Files:**
- `src/src/srcfiles/config.js` (or a new `tuning` block): how each stat enters an
  existing formula (attraction term, desire source, willingness term) — pure
  data wiring, one reader per stat.
- `src/src/srcfiles/scene.js` + `llm.js`: outfit-aware prose (wearing the nice
...
**Files:**
- `src/src/srcfiles/config.js`: `DESIRE` tuning; `npc.desire` on the schema.
- `src/src/srcfiles/sim.js` (`decayPlayerNeeds` + the heartbeat path): player
  desire decay; desire sources applied when signals/flirtation/peeks occur.
- `src/src/srcfiles/cognition.js`/`drives.js`: `scoreCandidates` reads
  `utility.desire` as a bias term (never a hard gate — D12).
- `index.html` + `src/src/srcfiles/render.js`: the desire footer bar.
...
**Files:**
- New `src/src/srcfiles/willingness.js` (or in `npc.js`): the function + tuning
  table from the Data model; `assertWillingnessIntegrity`-style checks for
  the verify harness.
- `src/src/srcfiles/effects.js`/`actions.js` consumers: the first call sites
...
outcomes.
**Files:**
- New `src/src/srcfiles/peek.js`: the `PEEK_SESSION` loop (advances the clock in
  `PEEK.tickMinutes`, re-derives door cues each tick), `riskPerTick`, caught
  resolution into `PEEK_OUTCOMES`.
- `src/src/srcfiles/image.js`: `composePeekPrompt` + key composition — the
  occupant's current activity + clothing state (Phases 5/6) + lighting; the
  keyhole overlay is pure CSS/SVG (D6).
- `index.html` + `src/src/srcfiles/render.js`: the keyhole lens (mask + vignette
  + shimmer) and the peek UI (peek/listen/stop).
- `src/src/srcfiles/ui.js`: hooking the door submenu verbs (`door.keyhole`,
...
**Files:**
- `src/src/srcfiles/defs.actions.js`: `INTIMACY_ACT_DEFS` entries as ACTION_DEFS
  rows (the Data model shapes), including paired acts resolved via the
  shared-activity machinery (`resolveSharedActivity`).
- `src/src/srcfiles/actions.js`: paired-act execution (both parties' effects,
  deltas, willingness pre-check via Phase 9).
- `src/src/srcfiles/ui.js`: Make a Move (initiative-gate mirror of the NPC
...
an upgraded bedroom.
**Files:**
- `src/src/srcfiles/sim.js` (or new `src/src/srcfiles/relationships.js`): the
...
- `src/src/srcfiles/config.js`: `RELATIONSHIP` tuning (thresholds, progression
  cooldowns).
- `src/src/srcfiles/computer.js`/`roomlist`-adjacent: an NPC in a committed
...
player.
**Files:**
- `src/src/srcfiles/config.js`: the `masturbate` and `intimate` DRIVE_DEFS
  entries (desire-bias utility, private-room + partner-willingness candidacy,
  long cooldowns, `leaves`/`emitsSignal`/`setsClothing`).
- `src/src/srcfiles/overture.js`/`cognition.js`: desire-motivated overture
  variants; both drives route through `scoreCandidates` and the Phase 9
  willingness check.
- `src/src/srcfiles/drives.js`: the pair-act resolution (both NPCs' states,
...
consequences.
**Files:**
- `src/src/srcfiles/computer.js`: generalize the escort visit plumbing into
  outside-partner visits (arrival → partner rendezvous → departure),
  seeded from `world.relationships`.
- `src/src/srcfiles/ui.phone.js`/`computer.js`: the long-distance thread
...
that spend it.
**Files:**
- `src/src/srcfiles/ui.phone.js`/`render.js`: the codex (per-NPC pages, day-
  stamped ledger entries).
- `src/src/srcfiles/ui.js`: `doConfrontNpc` (uses ledger + willingness +
...
**Files:**
- `src/src/srcfiles/config.js`: `COLD_SHOULDER` tuning; shaming reaction pools
  keyed by dynamic tier; extra move-out triggers (boundary acts at low
  dynamic, public infidelity fallout).
- `src/src/srcfiles/npc.js`/`drives.js`: the cold-shoulder flag's effects on
...
consequential.
**Files:**
- New `src/src/srcfiles/boundary.js`: `BOUNDARY_ACT_DEFS` + the narrow context
  gate (relationship-gated attempted value, wake-up/catch risk curve) — never
  a relaxed willingness.
- `src/src/srcfiles/defs.actions.js`: player-facing verbs (sleeping-room verbs
  appear on a bed submenu only when the occupant is asleep); NPC equivalents.
- `src/src/srcfiles/config.js`: three-way dynamic gating (all parties' willingness).
...
couple-level choice.
**Files:**
- `src/src/srcfiles/sim.js`: `world.pregnancies` lifecycle (conceive → term →
  birth) on the day-rollover pass; conception checks on qualifying acts when
  `trying` or unprotected.
- `src/src/srcfiles/npc.js`/`llm.js`: visible-from-day belly in scene prose; the
  birth day event; the baby presence affecting schedules/mood.
- `src/src/srcfiles/ui.js`: the player-pregnancy path mirrors the NPC one.
...
**Files:**
- `src/src/srcfiles/defs.world.js` + `defs.computer.js`: `SOUND_DEVICE_DEFS`
  objects + Nile rows.
- `src/src/srcfiles/signals.js`: the `music` signal by volume; headphones as a
  received-signal filter (player and NPCs).
- `src/src/srcfiles/defs.actions.js`: set_volume/play/eject verbs (Phase 1
...
## Status — **ALL 19 PHASES COMPLETE** (archived to `src/src/ref/complete/`)
