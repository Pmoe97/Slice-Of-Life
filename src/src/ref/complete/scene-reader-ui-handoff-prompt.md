...
in `src/src/ref/complete/scene-reader-ui-plan.md`.
...
- `renderSceneReader` / `renderSceneMoodles` / `buildLogEntryNode` — `src/src/srcfiles/render.js`
- `perceiveSignals` / `mergePerceived` / `signalPhrase` / `signalsByRoom` — `src/src/srcfiles/signals.js`
- `addRecentExchange` / `getRecentExchanges` — `src/src/srcfiles/npc.js` (the
  channel-tagged conversation buffer Phase 5 reads)
- The conversation overlay — `openConversationOverlay`, `convAddBubble`,
  `convAddBeat`, `doConvSend` in `src/src/srcfiles/ui.js`, markup at
...
- **Pure logic → the Node harness.** `node src/src/dev/verify/run-all.js` runs all of
  it; `node src/src/dev/verify/verify-r1.js` runs one. Read `dev/verify/README.md`
...
5. **If this was the last phase:** mark the plan's Status header complete, move
   ...
   `src/src/ref/structural/ARCHITECTURE.md`, and the Plan 2 row in
