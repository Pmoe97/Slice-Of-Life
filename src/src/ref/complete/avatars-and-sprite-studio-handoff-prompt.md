...
in `src/src/ref/complete/avatars-and-sprite-studio-plan.md`.

The first phase not marked **Done** is your phase. **As of 2026-08-27, all 8
are Done** — this plan and this prompt now live in `src/src/ref/complete/`
...
1. **A new `src/src/srcfiles/*.js` needs a line in TWO places, in the same
...
- **Two checks that are easy to skip and have both caught real bugs here:**
  ...
  expect 0).
- **The suite:** `node src/src/dev/verify/run-all.js`. The baseline is **72 failed / 8
...
6. **Mark the plan complete** if this was the last phase, and move it plus this
   prompt to `src/src/ref/complete/` as a pair.
