...
Companions: `src/src/ref/complete/apartment-upgrades-plan.md` (the facility model this extends —
tier structure, cost bands, quality formula, and design invariants 1-3 carry
over unchanged), `src/src/ref/complete/economy-and-rent-plan.md` (rent math), `src/src/ref/complete/contractor-tutorial-overhaul-plan.md`
(who performs and prices these jobs, and the tutorial that teaches them),
`src/src/ref/complete/external-world-npcs-overhaul-plan.md` (future off-site substitutes for
...
`src/src/ref/patterns/perchance-agent-handoff-prompt.md` for the full session protocol.
...
`src/src/ref/structural/ARCHITECTURE.md:2209` left untouched — it's a past-tense historical bug-fix
...
(`src/src/ref/complete/game-opening-plan.md`).
...
> `src/src/ref/complete/external-world-npcs-overhaul-plan.md` Phase 4, which shipped after
