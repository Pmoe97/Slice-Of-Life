...
- `src/src/ref/complete/npc-initiative-plan.md` (Plan 5 — built the overture
...
`src/src/ref/complete/npc-initiative-retiming-plan.md` now.
...
effective duration.
**Files:** `src/src/srcfiles/config.js` — grep for every `cooldownTicks:`
...
**Files:** `src/src/srcfiles/drives.js` (`isOnCooldown`, `setCooldown`, and
every call site — `cognition.js`'s `isDriveCandidate` among them);
`src/src/srcfiles/cognition.js` (`recencyMultiplier`, cognition.js:100–112 —
