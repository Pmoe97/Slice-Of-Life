...
> overhaul was run, and as the template `src/src/ref/patterns/` points at. Dream
...
in `src/src/ref/complete/dream-engine-plan.md`.
...
- **Actually run the phase's Verification steps.** Node harness work runs via
  `node src/src/dev/verify/run-all.js`; anything touching `root.kv`, the image plugin
