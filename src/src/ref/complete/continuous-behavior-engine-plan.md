...
- `src/src/ref/complete/decor-economy-plan.md` (the anchor table this plan
  reads for object-sourced actions is populated by that plan's catalog).
- `src/src/ref/complete/npc-cognition-plan.md` (Plan 3 — the utility scorer
  this plan keeps almost unchanged; only its hold/cooldown units move from
  ticks to minutes).
- `src/src/ref/wip/floorplan-and-movement-plan.md` (status: built, but still
  filed under `wip/` rather than moved — matching its own status header)
  and `src/src/ref/wip/home-design-studio-plan.md` (the position/walk/anchor
...
the shared roadmap checklist. The file now lives in `src/src/ref/complete/`;
...
- **D13 — The decision-queue module's home is decided by a size rule, not
  ...
  ~150 lines**, create `src/src/srcfiles/behavior.js`, loaded immediately
...
**Files:**
- `src/src/srcfiles/npc.js` (or wherever `npc.pursuit` is currently declared —
...
- `src/src/srcfiles/state.js`: since this plan explicitly carries no save-
...
**Files:**
- `src/src/srcfiles/actions.js`: generalize `executeAction`/`resolveTimeCost`
...
- `src/src/srcfiles/cognition.js`: `blockFilter` checks replaced by D4's
