...
### `INTRO_BEATS` — `src/src/srcfiles/defs.intro.js` (Phase 5)
...
| 3 | The studio. New `src/src/srcfiles/studio.js` + `#player-studio` markup/CSS. Seven tabs generated from `PLAYER_STUDIO_TABS`; add/remove rows from `STUDIO_ROW_GROUPS` (one builder, three groups); every field validated through `validateNpcField`. `menu.new-game` rerouted; legacy form → debug panel. |
