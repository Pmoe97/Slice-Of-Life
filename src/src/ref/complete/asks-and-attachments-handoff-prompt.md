...
table in `src/src/ref/complete/asks-and-attachments-plan.md`.
...
- **The reusable LLM prompt is `src/src/ref/complete/asks-llm-prompt.md`** — Phase 1
...
- **Hard technical rules every phase must respect:**
  ...
  - **`textContent`, not `innerHTML`,** for anything from player or LLM
    input.
  - **Load order.** New `src/src/srcfiles/asks.js` registers in index.html's
...
3. Promote any resolved open question into Locked decisions as a new
   D-number.
4. If you changed the ask-directive wording, update `src/src/ref/complete/asks-llm-prompt.md`
