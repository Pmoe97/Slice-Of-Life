...
table in `src/src/ref/complete/knowledge-gossip-memory-plan.md`. The first phase not
...
* `addMemoryFact`, `evictLowestScored`, `retrieveRelevantMemories`,
  `buildMemorySliceV2`, `migrateNpcToV2` — `src/src/srcfiles/npc.js`. Facts and
...
* The `npc_chat` drive — `src/src/srcfiles/drives.js` (~line 455). Templated
  event with `data: { other }`, no payload; D5's deterministic leg adds the
  payload here.
* `compactMemory` — `src/src/srcfiles/llm.js`. The piggyback-on-player-contact
  pattern D8 copies.
* `renderRoomListStudio` — `src/src/srcfiles/render.computer.js` (~line 1860),
...
* `BELIEF`, `FACT_DISPLAY`, `EMOTIONAL_WEIGHTS`, `TRANSMISSION`,
  `RUMINATION` — `src/src/srcfiles/config.js`. Author these new tables, do not
...
* Pure logic → the harness. `node src/src/dev/verify/run-all.js` runs the whole suite;
  `node src/src/dev/verify/verify-k1.js` runs one. Read `dev/verify/README.md` first.
