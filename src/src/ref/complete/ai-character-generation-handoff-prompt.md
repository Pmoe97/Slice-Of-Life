...
> `src/src/ref/complete/`. Its Status header says COMPLETE and its Status table has
...
in `src/src/ref/complete/ai-character-generation-plan.md`.
...
- When told to reuse a pattern, go read that code and match its current shape.
  ...
  - `PLAYER_STUDIO_TABS` (`src/src/srcfiles/studio.js`) — the one-table discipline:
    populate and read both walk the same table, so a field can never be
    offered and then silently dropped.
  - `rollCastSlot`'s partial contract (`src/src/srcfiles/sim.js`) — "whatever is
    supplied is held fixed; everything else is rolled". A concept fill is a
    fat partial, never a second construction path.
  - `doClassifiedsStudioSaveEdits` (`src/src/srcfiles/ui.computer.js`) — the
    validate → skip-no-op → apply → log-to-`bibleChanges` → one-revision-per-pass
    loop. Phase 6 reuses this rather than writing its own.
  - `mergeProseIntoBible` + `roommateAuthoredFields` (`src/src/srcfiles/llm.js`,
    `src/src/srcfiles/menu.js`) — the authored-field lock and its prefix-match
    coverage rule.
  - `comboControl` / `offPoolValues` (`src/src/srcfiles/fields.js`) — how a
...
- **Actually run the phase's Verification steps.** Verification happens in two
  ...
  - **Node harnesses** (`node src/src/dev/verify/verify-*.js`) for anything pure. At
...
5. If this was the last phase, mark the Status header complete and move both
   the plan and this prompt to `src/src/ref/complete/` **together** — they are a
   pair — and update `src/src/ref/README.md` and
