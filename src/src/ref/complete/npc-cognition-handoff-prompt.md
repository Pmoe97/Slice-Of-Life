...
in `src/src/ref/complete/npc-cognition-plan.md`.
...
- `evaluateDrives`, `checkDriveGates`, `driveGateValue`, `isOnCooldown`,
  `setCooldown` — `src/src/srcfiles/drives.js`. Note the five custom-resolver
...
- `DRIVE_DEFS` — `src/src/srcfiles/config.js`. Sixteen entries.
...
- `perceiveSignals` / `mergePerceived` — `src/src/srcfiles/signals.js`. Already
...
- `composeScene` / `openScene` / `markCalloutsShouted` — `src/src/srcfiles/scene.js`.
  The pure-reader / named-writer split this plan copies.
- `resolveTick` and `resolveBatch` — `src/src/srcfiles/sim.js`, including the NPC
...
- **Two words are already taken.** `commitments.js` owns *commitment* (meal
  ...
  classifier. Use `pursuit`, and put new code in `src/src/srcfiles/cognition.js`.
...
- **Pure logic → the Node harness.** `node src/src/dev/verify/run-all.js` runs all of
  it; `node src/src/dev/verify/verify-c1.js` runs one. Read `dev/verify/README.md`
...
6. **If this was the last phase:** mark the plan's Status header complete, move
   both the plan and this prompt to `src/src/ref/complete/`, and update all three
   indexes in the same commit — `src/src/ref/README.md`,
   `src/src/ref/structural/ARCHITECTURE.md`, and the Plan 3 row in
