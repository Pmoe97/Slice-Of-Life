...
Companions:
- `src/src/ref/complete/player-creation-and-intro-plan.md` (built the Player
  ...
  *means*, only how a draft can be filled and what kind of value a control
  will accept).
- `src/src/ref/complete/sandbox-editor-overhaul-plan.md` (built the sandbox
...
- `src/src/ref/complete/seasonal-calendar-and-sandbox-plan.md` (authored
  ...
  merge point).
- `src/src/ref/complete/npc-overhaul-plan.md` (authored the bible, the
...
`src/src/ref/complete/ai-character-generation-handoff-prompt.md`. A future session
...
`src/src/srcfiles/concept.js` shipped, registered in **both** `index.html` (after
...
Phase 1 shipped. New file `src/src/srcfiles/fields.js` (`comboControl`,
...
1. **A half-built version of this feature already ships.** The in-game
   ...
   `buildAIGenerationPrompt` (`src/src/srcfiles/computer.js` ~1465/1545), wired
   through `doClassifiedsStudioAIGenerate` (`src/src/srcfiles/ui.computer.js`
   ~1068) and rendered by `renderStudioCreateMode`
   (`src/src/srcfiles/render.computer.js` ~2543). **It generates zero
...
3. **The pools are not validation.** `validateNpcScalar`
   (`src/src/srcfiles/state.js` ~1787) enforces `enum` only where the schema
   ...
   (`src/src/srcfiles/npc.js` ~2485) concatenates those strings straight into
...
4. **`rollCastSlot`'s partial contract is the fill mechanism** and needs no
   ...
   (`src/src/srcfiles/sim.js` ~4005). A concept fill is a fat `partial`. The one
   ...
   (`src/src/srcfiles/computer.js` ~1418). Phase 2 must go through that same
...
### `CONCEPT_SCOPES` (Phase 2) — `src/src/srcfiles/concept.js`
...
### D4's resolvers — **BUILT, in `src/src/srcfiles/sim.js`** (Phase 1)
...
**Files:**
- `src/src/srcfiles/fields.js` **(new)**: the shared control builders. One
...
- `index.html`: one `<script src="src/src/srcfiles/fields.js?v=1">` tag,
...
- `src/src/srcfiles/menu.js`: `sbxSelectControl` gains a `free` flag; when set it
...
- `src/src/srcfiles/studio.js`: the `kind: 'select'` branch of the field renderer
...
- `src/src/srcfiles/render.computer.js`: `studioSelectField` gains the same
  ...
  removable.
- `src/src/srcfiles/ui.computer.js`: `collectStudioDraft` already preserves
...
- `src/src/srcfiles/sim.js`: **D4's four pure resolvers**, and the three
...
**Files:**
- `src/src/srcfiles/concept.js` **(new)**:
...
**Files:**
- `src/src/srcfiles/render.computer.js`: `renderStudioCreateMode`'s AI section
  ...
  appearance form.
- `src/src/srcfiles/ui.computer.js`: `doClassifiedsStudioAIGenerate` rewritten to
  call `fillFromConcept(text, 'npcFull', ctx)` and merge per D10. Delete
  nothing else in this file.
- `src/src/srcfiles/computer.js`: **delete** `generateCharacterWithAI` and
...
**Files:**
- `src/src/srcfiles/studio.js`: `renderPlayerStudio` renders the section above
...
**Files:**
- `src/src/srcfiles/menu.js`: `renderSandboxRoommateDetailHead` renders the
...
**Files:**
- `src/src/srcfiles/render.computer.js`: `renderStudioProfileMode` gains the
  ...
  is preserved.
- `src/src/srcfiles/ui.computer.js`: `doClassifiedsStudioConceptRewrite` builds
...
- `src/src/srcfiles/image.js`: no change — confirm the `genSeed` bump is
...
**Files:**
- `src/src/srcfiles/studio.js`: a `player` fill writes `draft.portrait.prompt`
  ...
  before writing.
- `src/src/srcfiles/ui.computer.js`: replace Phase 1's `TODO(concept-D4)` in
  `studioPoolNamesToValues` with the real `resolveInterests` /
  `resolveValues` calls.
- `src/src/ref/structural/ARCHITECTURE.md` + `src/src/ref/README.md`: index rows for
