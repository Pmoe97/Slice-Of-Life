...
table in `src/src/ref/wip/food-overhaul-plan.md`.
...
- When the plan says to reuse a pattern, go read that code and match its
  ...
  - **Equipment tiers:** `src/src/ref/complete/renovation-occupancy-overhaul-plan.md`'s
...
- **Hard technical rules** (each with its consequence):
  ...
  - **A `verify-*.js` that hasn't actually been run is not verified.**
    ...
    harness for real (`node src/src/dev/verify/verify-<name>.js`) and confirm it
    prints the pass/fail summary line, then run `node src/src/dev/verify/run-all.js`
...
5. If this was the last phase, mark the plan's Status header complete and
   move the plan + this prompt to `src/src/ref/complete/` (update the
   `src/src/ref/README.md` and `src/src/ref/structural/ARCHITECTURE.md` indexes in
