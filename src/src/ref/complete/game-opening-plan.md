...
The rent model (`src/src/ref/complete/economy-and-rent-plan.md`) is built around solo living
...
- **The apartment is a wreck.** It starts bare bones and in disrepair;
  ...
  `src/src/ref/complete/apartment-upgrades-plan.md`.
...
- **Starting energy is lower** than the eventual ceiling (see
  `src/src/ref/sleep-and-alarm-plan.md`), so the early game is genuinely tight on
...
- How much of the apartment is *usable* at the start? Disrepair answers
  ...
  rooms are reachable. Tracked in `src/src/ref/complete/apartment-upgrades-plan.md`.
