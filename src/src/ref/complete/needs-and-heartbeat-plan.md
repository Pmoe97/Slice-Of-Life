...
**Files:** `src/src/srcfiles/config.js` (`NEEDS` block).
...
**Files:**
- `src/src/srcfiles/time.js`: `clockFrame` gains the new accumulator (D2).
- `src/src/srcfiles/sim.js`: the needs-decay half of `resolveTick` moves out
...
**Files:** `src/src/srcfiles/time.js` (battery), the memory-decay call site
