...
- The plan's "src/src/srcfiles/prompt.js" citation for the deviant context line
...
**Files:**
- `src/src/srcfiles/afterhours.js` (**new**; Phase 1 ships only the adapter layer,
...
- `src/src/srcfiles/ui.computer.js`: `fetchAfterHoursClips(catId)` (line 490)
...
- `src/src/srcfiles/computer.js`: `defaultComputerState` (line 47) gains
  `afterHoursSourcePages: { ph: 1, ep: 1 }`.
- `src/src/srcfiles/render.computer.js`: grid cards gain the source badge +
...
- `src/src/srcfiles/config.js`: `AH_TUNING` (blend weights 50/50, per-page count,
...
**Files:**
- `src/src/srcfiles/afterhours.js`: `AH_VIEWS` view renderer registry, `ahNav(view,
...
- `src/src/srcfiles/computer.js`: `defaultComputerState` gains
  `afterHoursView: { view:'home', params:{}, stack:[] }` and `afterHoursSeed`.
- `src/src/srcfiles/config.js`: `AH_TUNING` gains route names and skeleton timing.
...
**Files:**
- `src/src/srcfiles/afterhours.js`: the `player` view; `fetchRelated(clip)` — one
...
- `src/src/srcfiles/ui.computer.js`: `doAfterHoursWatch` (line 443) rewires to
...
**Files:**
- `src/src/srcfiles/afterhours.js`: `home` view + `fetchRow(sectionId, query)`
...
- `src/src/srcfiles/config.js`: `AH_TUNING.homeSections` (which rows, their
  queries and order).
- `src/src/srcfiles/computer.js`: `defaultComputerState` gains
...
**Files:**
- `src/src/srcfiles/defs.computer.js`: `AH_ADS` — the parody ad copy pool
  (HornyGoat™, Hot MILFs in YOUR area, visitor #1,000,000, casino, fake
  browser-update), each entry `{ slot: 'banner'|'skyscraper', copy, cta }`.
- `src/src/srcfiles/afterhours.js`: `renderAdSlot(slot)` (rotates on a
...
**Files:**
- `src/src/srcfiles/state.js`: `defaultAfterHoursState()`; lazy-init
...
- `src/src/srcfiles/afterhours.js`: record-on-watch (history + continueWatching),
  like/unlike, search-history recording, the home "Continue Watching" tile.
- `src/src/srcfiles/interruption.js`: the masturbation memory text (line 215)
...
**Files:**
- `src/src/srcfiles/sim.js`: `ensureHotSinglesRoster(gameState)` — mirrors
...
- `src/src/srcfiles/computer.js`: `createExternalNpc` gains an optional
...
- `src/src/srcfiles/afterhours.js`: `hotsingles` browse view + `hot-single`
...
- `src/src/srcfiles/state.js`: persist `world.hotSinglesRoster` (the
  `escortRoster` precedent).
- `src/src/srcfiles/config.js`: `AH_HOT_SINGLES_TUNING` (roster size ~6, deviant
...
**Files:**
- `src/src/srcfiles/afterhours.js`: "Invite Over" on the profile — writes a
...
- `src/src/srcfiles/ui.js`: visit-arrival narration flavour for `ah-*` visits.
- `src/src/srcfiles/sim.js`: any visit-activity text the spine needs for Hot
  Single guests (reuses `VISIT_TUNING.activities`, config.js:714).
- `src/src/srcfiles/interruption.js`: `deviantLevel` scales the
...
- `src/src/srcfiles/prompt.js`: `bible.deviantLevel` surfaced in the NPC context
