...
**2026-09-21 correction — two of these rows were never true.** A
character/NPC-creation find-and-improve session grepped
`knowledge-gossip-memory-plan.md` (the roadmap's Plan 4, complete since
2026-09-05) for `sampleLines`/`history`/`few-shot`: zero mentions. Plan 4
shipped without ever touching either field, so both sat as pure flavor text —
generated by every AI character fill, editable in the Character Studio,
displayed on the profile screen — with **zero effect on any conversation
`buildNpcBlockV2` ever produced**, despite `sampleLines` existing specifically
so a character's dialogue would sound like them. This was the same class of
staleness `verify-p5.js`'s own RESERVED block would have caught if it asserted
non-reader rather than "still declared" — it only checks the schema entry
exists, not that nothing reads it, so the claim rotted silently for over a
month. Both are now **wired**: `buildNpcBlockV2` (`llm.js`) emits a
`[History]` line when `bible.history` is set, and an "Example lines in their
voice" line under `[Speech]` when `bible.sampleLines` is non-empty (worded
like `catchphrases`/`verbalTics` above it — cadence to draw from, never a
script to repeat). `buildImPrompt` inherits both for free since it calls
`buildNpcBlockV2` directly. Verified via `verify-p5.js` plus
`verify-voc-p34.js`/`verify-x2.js`/`verify-acc-p3.js` (the other three callers
of `buildNpcBlockV2` in the suite) and a full `run-all.js` sweep, all green.
Player-facing — patch-noted under 0.14.1.

**The other ten Reserved rows were re-audited the same way** (grep the real
code for a non-comment, non-default-backfill reader; grep the claiming plan's
doc for the field name). Eight check out — genuinely wired, just not
necessarily by the plan credited:

| Field | Actual reader |
|---|---|
| `episodes[].emotionalTag` | `rumination.js` theme grouping, `dreams.js`, `chatter.js`'s mood-bucketed templates, `x5.js` — Plan 4 delivered this one |
| `episodes[].participants` | `rumination.js`, `render.computer.js` profile view, `x5.js`, `sim.js`'s `eventParticipants` — also delivered |
| `relPlayer.lastInteractionDay` | `dreams.js`'s absence-gap check (`ctx.day - rel.lastInteractionDay`, `DREAM.absenceDays`) — wired, but by the Dream Engine plan, not Plan 4/5 as credited |
| `castWeb.compatibility` / `.friction` / `.beatPositive` | `sim.js`'s cast dynamic types (`frictionPair`, `unresolvedConflict`, `alliance`) — Plan 3 delivered these |
| `bibleRevision` / `bibleChanges` | `ui.computer.js` increments/records on every Character Studio edit; `state.js` persists them; `render.computer.js` displays them — real bookkeeping, delivered |

**Three more were the same dead claim as `sampleLines`/`history`, found the
same session — and fixed in a same-day follow-up once the user locked real
design decisions for each** (each genuinely needed one; none was a one-line
prompt addition):

| Field | Claimed by | What was actually true, and the locked fix |
|---|---|---|
| `physical.typicalAttire` | Plan 2 — "what are they wearing right now" | Grepped `scene-reader-ui-plan.md`: zero mentions. Locked decision: real mechanical wiring, not text. `npc.js`'s `typicalAttireStyleLean` extracts whichever known `CLOTHING_DEFS` trait/styleTag words the relevant slot's authored text contains and feeds them through `composeOutfit`'s existing `styleLean` bias (`npcOutfitForContext`) — the same mechanism `occupation.styleLean` already uses, never a new gate. Free text written before this vocabulary existed (Del's `work: 'coveralls'`) legitimately extracts nothing — an honest no-op, not a bug; proved live with a matched pair (`blouse` vs `dress_shirt`) where the pick provably flips only with the lean applied. `verify-p5.js`. |
| `bible.interests[].skill` | Plan 5 — shared activities | Grepped `npc-initiative-plan.md`'s actual Phase 5: shared-activity credit runs entirely off `ACTION_DEFS`' `shared` field (D17) and `SHARED_ACTIVITY.rates` — never touched `interests[].skill`. Locked decision: both (a) and (c) together. (a) Seven `shared` entries now name an `interestTag` (`defs.actions.js`); `actions.js`'s `sharedActivityDelta` scales the credit continuously by the matching interest's skill (`SHARED_ACTIVITY.skillMultiplierDivisor`, 1x at skill 0 up to 2.25x at 100) — real generated data (0–39 from `rollCastSlot`) finally doing something. (c) `notice.js` gained `hobby_skill`, its first-ever subject kind about a RESIDENT rather than the player — `opinionValence` took an optional `perceiverId` so the relationship bias can read the bystander↔skilled-resident `castWeb` pair instead of `relPlayer` (every existing call site omits it, so all seven prior kinds are byte-identical, re-verified). Fires only above `SHARED_ACTIVITY.hobbySkillNoticeFloor` — this is about a genuinely skilled hobbyist being noticed, not mediocrity becoming gossip. `verify-i5.js`, `verify-acc-p3.js`. |
| `relPlayer.firstMetDay` | Plan 4 — `daysKnown` in the player model | Grepped all of Plan 4: no `daysKnown` was ever built. Wired the same additive-prompt-line way as `[History]`: `buildNpcBlockV2` prints "You've known them for N days" once `day - firstMetDay` is positive (day-1 default → no line on day 1 itself, so no behavior change for a fresh game; starts saying something true only once play continues, or once something backdates `firstMetDay` — see `sandbox-editor-overhaul-plan.md`'s 2026-09-21 addendum, the Sandbox prior-relationship feature, which does exactly that). `verify-p5.js`. |

All three: `run-all.js` before/after identical failure count, no regressions.

Treat every "claimed by" attribution in this table as unverified until it's
been re-grepped this way — the table's original claims were written when
Phases 1-4 landed (2026-08-10), months before the plans that were supposed to
cash them in actually shipped, and evidently nobody checked back.
...
**Files:**
- `src/src/srcfiles/npc.js`: in `applyProposal`, move the `playerAction` recent-exchange block above the `proposal.dialogue` loop (D4). Raise `addRecentExchange`'s cap 10 → 40 and add the `channel` parameter, defaulting to `'scene'` for callers that do not pass one (D5, D6). `getRecentExchanges(npc, count, channel)` gains the filter and its default count rises 5 → 16. Fix `resolveGrievance`'s condition: drop the unreachable third clause and require a word-boundary match rather than a bare `includes`, which is what the existing comment already claims it does.
- `src/src/srcfiles/llm.js`: `buildNpcBlockV2` gains a `channel` argument threaded from its two callers; `buildMemorySliceV2` passes it through to `getRecentExchanges`. `buildImPrompt` takes the last `IM_PROMPT.threadDepth` messages from the real thread and renders them as a transcript (D7).
- `src/src/srcfiles/computer.js`: `assembleImContext` passes the thread through on the context object so `buildImPrompt` does not reach into `gameState` itself.
- `src/src/srcfiles/config.js`: `IM_PROMPT = { threadDepth: 12 }`.
...
**Files:**
- `src/src/srcfiles/npc.js`: replace `deriveConversationPhase`'s formula per D1/D2. Keep the return shape `{ intimacyLevel, conversationPhase }` — every caller is unchanged.
- `src/src/srcfiles/state.js`: register an `npcs` folder version bump whose migration re-derives `relPlayer.intimacyLevel` and `relPlayer.conversationPhase` from the stored axes (D3). Pure recomputation, no data invented.
- `src/src/srcfiles/config.js`: lift the three thresholds out of the function into `PHASE_THRESHOLDS = { intimate: 70, close: 40, familiar: 20 }` so they are tunable without touching logic.
...
**Files:**
- `src/src/srcfiles/config.js`: `MEMORY_IMPORTANCE` per the data model.
- `src/src/srcfiles/npc.js`: `addMemoryEpisode` evicts the lowest `importance × decay` when over budget rather than calling `shift()`, exempting `day === 0` shared-history episodes as today. `addMemoryFact` gets the same treatment against `importance`.
- `src/src/srcfiles/ui.js`: `advanceAndResolve`'s event loop stops passing a hardcoded `0.5`. Importance comes from a new `eventImportance(evt)` helper keyed on `evt.type` against `MEMORY_IMPORTANCE` (D8).
- `src/src/srcfiles/drives.js`: drive-produced events carry an explicit `importance` on the event object so `eventImportance` reads it rather than guessing from the type string.
...
**Files:**
- `src/src/ref/wip/npc-correctness-fixes-plan.md`: the disposition table, appended to this document as a new section. This is the deliverable — the code changes follow from it.
- `src/src/srcfiles/config.js`, `sim.js`, `npc.js`: remove the fields marked `prune`, including their generation, their schema entries and their migration backfills. Expected: `bible.occupation.stressProfile`, `bible.interests[].skill`, `bible.speech.vocabularyLevel`, `physical.typicalAttire`, `physical.genitals`, `styleCounters.lastJobMention`, `styleCounters.lastHobbyMention`, `relPlayer.firstMetDay`.
...
**Verification:** grep each pruned identifier across `src/src/srcfiles/` and
