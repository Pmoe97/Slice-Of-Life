...
**Three more are exactly the same dead claim as `sampleLines`/`history` and
were left unfixed on purpose** — each needs an actual design decision, not a
one-line prompt addition, so fixing them was out of scope for this session:

| Field | Claimed by | What's actually true |
|---|---|---|
| `physical.typicalAttire` | Plan 2 — "what are they wearing right now" | Grepped `scene-reader-ui-plan.md`: zero mentions. Still only an edit-mode field in the Character Studio (`render.computer.js`); no outfit-selection code (`composeOutfit` et al.) has ever read it. Fixing this means deciding how a per-context wardrobe preference should bias outfit rolls — a real mechanic, not text. |
| `bible.interests[].skill` | Plan 5 — shared activities | Grepped `npc-initiative-plan.md`'s actual Phase 5: shared-activity credit runs entirely off `ACTION_DEFS`' new `shared` field (D17) and `SHARED_ACTIVITY.rates` — never touches `interests[].skill`. The field is real, non-trivial generated data (`rollCastSlot` rolls 0–39 per interest; `concept.js`'s AI fill always writes a flat 0, itself a quiet inconsistency nothing has ever needed to care about) with **no reader of any kind, not even a display one** — worse off than `sampleLines`/`history`, which at least render in the profile. Fixing this means deciding what a "skilled at their hobby" NPC should actually do differently. |
| `relPlayer.firstMetDay` | Plan 4 — `daysKnown` in the player model | Grepped all of Plan 4: no `daysKnown` was ever built. Only a migration default-backfill (`npc.js`) and a raw-field display in the Character Studio (`render.computer.js`) read it — no `currentDay - firstMetDay` computation exists anywhere. The lowest-risk of the three to actually fix (a `daysKnown` line would slot into `buildNpcBlockV2`'s `[Relationship with player]` block exactly like this session's `[History]` line did) — left for a future session to keep this one scoped to what was already re-verified. |
