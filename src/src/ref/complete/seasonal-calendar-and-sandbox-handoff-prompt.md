...
in `src/src/ref/complete/seasonal-calendar-and-sandbox-plan.md`.
...
- When told to reuse a pattern, go read that code and match its current shape
  ...
  - **Part A** — the calendar helpers block in `src/src/srcfiles/sim.js` (all pure
    ...
    and `processBillsForDay` in `src/src/srcfiles/computer.js` for how a cadence
    actually fires.
  - **B1** — `expandCharacterProse` and the `fallback*` family in
    `src/src/srcfiles/llm.js`, and the `candidateBible` construction inside
    `approveCastAndStartGame` in `src/src/srcfiles/ui.js` that you are replacing.
  - **B2** — `generatePlayerAppearance` in `src/src/srcfiles/sim.js`. Its merge
    ...
    description.
  - **B3** — `applyStructuralUpgrades` (`src/src/srcfiles/config.js`),
    `applyFacilityCompletionStates` (`src/src/srcfiles/ui.js`),
    `moveNpcIntoRoom` (`src/src/srcfiles/sim.js`), and the recruitment room-picker
    in `src/src/srcfiles/computer.js`. All four already exist; the phase is
    sequencing them, not writing them.
  - **B4** — `startSoloGame` in `src/src/srcfiles/ui.js`, beat for beat. The
  ...
  - **B5** — `PLAYER_STUDIO_TABS` and the populate/read walk in
    `src/src/srcfiles/studio.js`. Its header comment states the rule you are
  ...
  - **B4/B5/B6 UI wiring** — the `data-action` dispatch chain in
    `src/src/srcfiles/ui.js` (`handleAction`) and the menu sub-screen pattern in
    `src/src/srcfiles/menu.js` (`showMenuScreen`). This codebase does not attach
...
- **Actually run the phase's Verification steps.** Verification splits by
  ...
  - **A1-A4, B1-B3, and the harness half of B7** verify in
    `dev/verify/` — `node src/src/dev/verify/verify-<name>.js` for the new harness,
    then `node src/src/dev/verify/run-all.js` for the whole suite. `loadgame.js`
  ...
  - **At minimum for every phase, whichever layer:** `node src/src/dev/verify/run-all.js`
...
5. If this was the last phase, mark the plan's Status header complete and move
   both the plan and this prompt from `src/src/ref/wip/` to `src/src/ref/complete/` —
   they move as a pair — updating `src/src/ref/README.md`'s contents table in the
   same commit.
