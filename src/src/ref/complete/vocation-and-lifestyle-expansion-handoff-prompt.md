...
in `src/src/ref/wip/vocation-and-lifestyle-expansion-plan.md`.
...
| Food and taste, if diet is your dimension | `taste.js`, and `src/src/ref/complete/food-overhaul-plan.md` |
...
8. **A new `src/src/srcfiles/*.js` file needs a line in TWO places:** `index.html`'s
...
```
node src/src/dev/verify/verify-voc-p1.js        (44 checks)
node src/src/dev/verify/verify-voc-p1-equiv.js  (4)
node src/src/dev/verify/verify-voc-p2.js        (45)
node src/src/dev/verify/verify-voc-p34.js       (28)
node src/src/dev/verify/verify-voc-p56.js       (43)
```
...
6. **If this was the last phase**, mark the plan's Status header complete and
   move the document from `src/src/ref/wip/` to `src/src/ref/complete/` — then update
   its row in **both** `src/src/ref/README.md` and
   `src/src/ref/structural/ARCHITECTURE.md` in the same commit. Five docs were once
