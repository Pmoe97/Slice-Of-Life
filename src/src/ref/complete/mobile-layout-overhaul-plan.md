...
Companions:
- `src/src/ref/complete/scene-reader-ui-plan.md` (the `#main-content` bands this plan lays out on phones)
- `src/src/ref/complete/inventory-needs-menu-saves-plan.md` (the `.invp-box`/`.svp-box` overlay chrome, already vw-capped)
- `src/src/ref/structural/ARCHITECTURE.md` (the as-built map; BrineOS/phone & desktop-shell sections cover the other two shells this plan sweeps)
...
behaviour hooks in `src/src/srcfiles/ui.js`/`render.js` (chip first-tap hint,
...
- `src/src/srcfiles/ui.js` (or `render.js`, whichever owns the post-render hook):
