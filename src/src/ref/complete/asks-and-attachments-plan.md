...
Companions:
- `src/src/ref/complete/plan-x5-conversation-consequences.md` (the Assessor/Chronicler split this plan writes asks *through* — the writer already scores nothing, and asks extend that to "the writer also never *decides* outcomes on ask turns")
- `src/src/ref/complete/npc-initiative-plan.md` (the conversation overlay, `doConvSend`, and the free-text "Say or do something" box that becomes the surface for asks)
- `src/src/ref/complete/intimacy-and-voyeurism-overhaul-plan.md` (willingness gates — the deterministic engine the intimacy ask reuses whole)
- `src/src/ref/complete/continuous-behavior-engine-plan.md` (`resolveScheduleActivity` + `COMMITMENT_TUNING.busyBlocks` — the schedule probe scheduled asks bind)
- `src/src/ref/structural/ARCHITECTURE.md` (commitments, effects, memory, the clock)
...
`src/src/ref/patterns/HANDOFF-PROMPT-ARCHITECTURE.md` for the full session protocol.
...
Lives at `src/src/ref/complete/asks-llm-prompt.md` — the ask-directive block
...
**Files:**
- `src/src/srcfiles/asks.js` (new): `ASK_CATEGORIES`/`ASK_TYPES` registry,
...
- `src/src/srcfiles/config.js`: `ASK_TUNING` block.
- `src/src/srcfiles/llm.js`: `buildScenePrompt` appends the ask-directive section
  when `context.askDirective` is present (compile from
  `asks-llm-prompt.md`).
- `src/src/srcfiles/ui.js` `doConvSend`: `parseAskInput` check at the top →
...
- `src/src/srcfiles/ui.js` `convAddBubble`: accept an optional `tag` arg; render a
...
- `src/src/srcfiles/ui.js`: open/close/nav handlers, tree builder from
...
words match.
**Files:**
- `src/src/srcfiles/asks.js`: `_askCounts` read/write in `resolveAsk`; penalty in
  the decision; 3+ → `REL_DELTA`; ladder stance strings.
- `src/src/srcfiles/ui.js` `processDayRollover`: reset stale `_askCounts`
  entries.
...
modal (D8/D9).
**Files:**
- `src/src/srcfiles/render.js` `openAskScheduleModal`: the schedule modal (day
...
- `src/src/srcfiles/asks.js`: `schedule: true` leaves; `freeSlotsFor(npc, dayAbs)`
  probing `resolveScheduleActivity` against `busyBlocks`; final-yes →
  `createCommitment` with the chosen slot.
- `src/src/srcfiles/ui.js` `doConvSend`: on accept of a scheduled ask → open
  modal → slot confirm (recheck hard blocks) → create commitment → LLM pass 2
  (or template fallback).
- `src/src/srcfiles/llm.js`: the scheduling-confirm directive variant.
...
**Files:**
- `src/src/srcfiles/asks.js`: `ask_meal` leaf (schedule: true); slot → label via
  `COMMITMENT_TUNING.mealSlots`; record + confirm pass carry the inferred
  label.
- `src/src/srcfiles/render.js` `openAskScheduleModal`: modal day rows show the
...
writes.
**Files:**
- `src/src/srcfiles/asks.js`: `ask_loan` — amount parsed from flavor
...
logic (D14).
**Files:**
- `src/src/srcfiles/asks.js`: `ask_intimacy` leaf — `resolveWillingnessGate` is
...
**Files:**
- `src/src/srcfiles/asks.js`: `ask_photo` leaf — available when the NPC is
...
- `src/src/srcfiles/ui.js` / `index.html`: conversation image bubble render
...
**Files:**
- `src/src/srcfiles/asks.js` / `inventory.js`: gift-from-inventory flow (item
...
**Files:**
- `src/src/srcfiles/asks.js`: audit every leaf's `effects` and memory writes;
  wire `_loanOwed` lifecycle; confirm ask turns produce `memory.recent`
  entries the Assessor/Chronicler can judge.
- `src/src/ref/structural/ARCHITECTURE.md` + `src/src/ref/README.md`: doc index.
...
**Files:**
- `src/src/ref/wip/asks-and-attachments-plan.md` — the whole document,
  including the Status table and this phase's own numbering.
- `src/src/ref/wip/asks-llm-prompt.md` — the reusable prompt vs
  `buildAskDirective` (asks.js) and the fallback list (ui.js).
- The code each phase cited, current shapes, at minimum:
  `src/src/srcfiles/asks.js` (registry, `parseAskInput`, `resolveAsk`,
