...
`src/src/srcfiles/actionwindow.js` and both halves of D5's image split are
...
Companions:
- `src/src/ref/complete/intimacy-and-voyeurism-overhaul-plan.md` (owns `peek.js` —
...
- `src/src/ref/complete/scene-reader-ui-plan.md` (owns the narration log and the
  moodle-strip pattern — `renderSceneMoodles` — this plan's "what changed"
  strip is proposed to reuse).
- `src/src/ref/complete/food-overhaul-plan.md` (owns the interactive cooking
...
- `src/src/ref/complete/afterhours-redesign-plan.md` (owns the AfterHours site
  D8 patches — the Hot Singles "Invite Over" scheduling fix touches its
  `doAfterHoursInviteOver` flow).
- `src/src/ref/complete/asks-and-attachments-plan.md` (owns `invite-dinner`'s
...
session — see `src/src/ref/complete/action-outcome-window-handoff-prompt.md` for
...
- **`defs.actions.js`** (now `?v=44`) — added `outcomeWindow` to every
  ...
  `sitWindowPhrase`.
- **No new `src/src/srcfiles/*.js` files** → `index.html`/`loadgame.js` ORDER
...
- **`index.html`**: added `#aw-picker` inside `#action-window-overlay` +
  ...
  `afterhours.js?v=30` script tags. **No new `src/src/srcfiles/*.js` files →
...
**Files:**
- `src/src/srcfiles/actionwindow.js` (new): the render/dismiss logic, mirroring
...
- `src/src/srcfiles/image.js`: the D5 archetype-vs-instance cache-key split —
  two composing functions alongside the existing `composePeekKey` and
  siblings, not a new caching mechanism.
- `src/src/srcfiles/defs.actions.js` / `src/src/srcfiles/actions.js`: an
...
**Files:**
- `src/src/srcfiles/actionwindow.js`: the `trigger: 'world'` path and the
  `'handoff'` dismissal — tap-to-close, then transition into conversation,
  never an instant dissolve (D6).
- `src/src/srcfiles/ui.js`: wherever `overture.accept`/`overture.decline` are
  ...
  (citations drift).
- `src/src/srcfiles/commitments.js` / wherever `npc.overture` completion is
...
**Files:**
- `src/src/srcfiles/defs.actions.js`: new `sit` action entry; `set_meal`'s
  entry trimmed to lay-the-table-only (no eating resolution).
- `src/src/srcfiles/signals.js`: read (not extend, unless the existing smell/
  sound signals don't already cover "a meal is happening") for the
  ambient-interest roll.
- `src/src/srcfiles/drives.js` / `src/src/srcfiles/overture.js`: the per-NPC
  "attempt to join" decision — reuse `npc.overture`'s commit/decide
  substrate, scoped to a meal-joining flavor, per D12.
- `src/src/srcfiles/actionwindow.js`: the Tier D in-window ask sub-step (one
  beat per candidate NPC) ahead of the full scene.
- `src/src/srcfiles/world.js`: the leftover-mess object/state write for the
...
**Files:**
- `src/src/srcfiles/peek.js`: `renderPeekOverlay` calls become `ActionWindow`
...
- `src/src/srcfiles/ui.computer.js`: the interruption and caught-peeping bubble
...
**Files:**
- `src/src/srcfiles/afterhours.js`: `doAfterHoursInviteOver` routes through the
  new picker instead of its current hardcoded response.
- `src/src/srcfiles/ui.js`: `doInviteDinner` (already a calendar picker per the
  ...
  new function.
- `src/src/srcfiles/actionwindow.js`: the Tier D wrapping (D2) for the picker.
...
component behavior.
**Files:** `src/src/srcfiles/defs.actions.js` and the relevant hand-written
...
|---|---|---|
| 1 | **Done** (2026-08-24, verified live) | The `ActionWindow` component itself (`src/src/srcfiles/actionwindow.js`), proven on `self.eat` (fresh/instance image) and `self.shower` (reused/archetype image). Delta strip reads `applyEffects`' returned `applied` list only; tap-only dismissal; clock pauses while open |
...
- **Dream Engine specifics** — explicitly deferred by the user to a
  ...
  plan now exists: `src/src/ref/complete/dream-engine-plan.md`, paired with
  ...
  **CLOSED 2026-08-25: all nine phases of the Dream Engine landed and the plan
  moved to `src/src/ref/complete/dream-engine-plan.md`.** This item needs nothing
...
Every distinct player-invokable verb found across `src/src/srcfiles/*.js`,
