...
Companions:
- `src/src/ref/wip/character-cutout-scene-rendering-plan.md` (code-complete, one
  ...
  them.
- `src/src/ref/wip/home-design-studio-plan.md` (foundation built) — the precedent
...
- `src/src/ref/complete/ai-character-generation-plan.md` (built) — the Character
...
- `src/src/ref/complete/scene-reader-ui-plan.md` (built) — the frosted reader the
...
`src/src/ref/complete/` now, paired with
`src/src/ref/complete/avatars-and-sprite-studio-handoff-prompt.md`. That prompt's
...
document and its paired handoff prompt now live in `src/src/ref/complete/`. A
...
- **Built and verified.** `dev/verify/verify-sprite-p8.js` — **22
  ...
  Plus two new functions in `src/src/srcfiles/state.js`:
...
- **`ARCHITECTURE.md` updated in this pass**: `sprites.js`/`avatar.js` after
  ...
  prompt, matching every other finished plan's format. `src/src/ref/README.md`
  updated the same way (moved from its `wip/` table to its `complete/`
  table).
- **This plan and its handoff prompt moved to `src/src/ref/complete/`** as a
...
- **Built and verified.** `dev/verify/verify-sprite-p7.js` — **24
  assertions**. No new `src/src/srcfiles` file — the recrop surface, the wildcard
...
- **Built and verified.** `dev/verify/verify-sprite-p4.js` — **29 assertions**.
  New files: `src/src/srcfiles/spritestudio.js` (the verbs) and
  `src/src/srcfiles/render.spritestudio.js` (the screens), the pairing
...
- **Built and verified.** `dev/verify/verify-sprite-p2.js` — **26 assertions**;
  `verify-sprite-p1.js` grew to **34**. New file `src/src/srcfiles/avatar.js`,
...
- **Built and verified.** `dev/verify/verify-sprite-p1.js` — **31 assertions,
  all passing.** New file `src/src/srcfiles/sprites.js`, registered in **both**
...
- **D11 — One component, everywhere, and the old ones are deleted.**
  `avatarChip(identity, opts)` in a new `src/src/srcfiles/avatar.js` (following
...
**Files:**
- `src/src/srcfiles/sprites.js` **(new)** — the whole spine. `spriteSlotId`,
...
- `src/src/srcfiles/config.js` — `SPRITE_STORE` (above). Nothing else; the cutout
  tables are consumed unchanged.
- `src/src/srcfiles/state.js` — `kv.sprites` joins the folder set. Extend the save
...
- `index.html` — one `<script src="src/src/srcfiles/sprites.js?v=1">` tag, after
  `image.js`.
- `dev/verify/verify-sprite-p1.js` **(new)**.

**Verification:** `node src/src/dev/verify/verify-sprite-p1.js`, and it must print its
...
**Files:**
- `src/src/srcfiles/sprites.js` — `detectHeadCrop` and `deriveAvatarFromCutout`
...
- `src/src/srcfiles/avatar.js` **(new)** — `avatarChip(identity, opts)` returning a
...
- `src/src/srcfiles/render.js` — the three blocker fixes; `avatarMarkerHtml` and
...
- `src/src/srcfiles/ui.js` — `conv-avatar` (`ui.js:6156`) becomes a chip; each
...
- `src/src/srcfiles/render.computer.js` — the nine `rl-card-avatar` /
...
- `src/src/srcfiles/render.phone.js` — the contact rows and the camera share row
  (`render.phone.js:525`).
...
**Files:**
- `src/src/srcfiles/sprites.js` — `spriteQueue`: tiering by D12, the single-flight
...
- `src/src/srcfiles/config.js` — `SPRITE_QUEUE`.
- `src/src/srcfiles/image.js` — `layoutSceneCutouts` takes
...
- `src/src/srcfiles/render.js` — `renderSceneCutouts` stamps `is-ambient` on
...
- `src/src/srcfiles/defs.settings.js` — an Appearance row: **Character art** with
...
**Files:**
- `src/src/srcfiles/defs.computer.js` — the `sprites` app: `id:'sprites'`,
  ...
  computer-only).
- `src/src/srcfiles/render.spritestudio.js` **(new)** — the renderers, registering
...
- `src/src/srcfiles/spritestudio.js` **(new)** — the app's actions, separate from
...
- `src/src/srcfiles/icons.js` — one app icon.
...
**Files:**
- `src/src/srcfiles/spritestudio.js` — the editor engine, all of it pure functions
...
- `src/src/srcfiles/render.spritestudio.js` — the editor screen: tool rail left,
...
**Files:**
- `src/src/srcfiles/spritestudio.js` — brush (size / hardness / opacity / colour,
...
- `src/src/srcfiles/render.spritestudio.js` — the tool rail's second group, the
...
**Files:**
- `src/src/srcfiles/render.spritestudio.js` — the recrop surface: the cutout master
  ...
  side by side.
- `src/src/srcfiles/spritestudio.js` — writing a linked record's `crop` (no blob,
...
- `src/src/srcfiles/sprites.js` — the wildcard sweep (D5): a **Where does this
...
- `src/src/srcfiles/state.js` — a save/load round trip carrying overrides, and an
...
- `src/src/ref/structural/ARCHITECTURE.md` — the four new files in the script-order
...
**Verification:** The full suite (`node src/src/dev/verify/run-all.js`) against the
