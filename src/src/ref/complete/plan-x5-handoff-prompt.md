...
`src/src/ref/complete/plan-x5-conversation-consequences.md` holds *what to build*.
...
in `src/src/ref/complete/plan-x5-conversation-consequences.md`. The first phase not
...
* `callLLM`, `callImLLM`, `buildNpcBlockV2` — `src/src/srcfiles/llm.js`. What you
...
* `validateProposal`, `applyProposal`, `applyRelDelta`, `deriveConversationPhase`
  — `src/src/srcfiles/npc.js`. **Both passes emit proposal-shaped output and go
  through these** (D4). Do not build a second ingestion path.
* `compactMemoryIfNeeded` — `src/src/srcfiles/ui.js`. The precedent for a deferred,
...
* `openScene`, `currentScene` — `src/src/srcfiles/scene.js`, and `sceneId` on
  `meta.sessionLog` entries. The Assessor's window already exists and is
  persisted; do not invent a second one.
* `addRecentExchange`, `MEMORY_BUDGET.maxRecent` — `src/src/srcfiles/npc.js`. The
  Chronicler's window is "unprocessed entries in `memory.recent`".
* `addMemoryFact`, `addMemoryEpisode`, `backfillFactRecordV2` —
  `src/src/srcfiles/npc.js`. Plan 4's belief contract that every extracted fact
  must satisfy.
* `ruminate` — `src/src/srcfiles/rumination.js`. The consumer whose inference rules
...
* **Pure logic → the Node harness.** `node src/src/dev/verify/run-all.js`;
  `node src/src/dev/verify/verify-x1.js` for one. **Read `dev/verify/README.md` first** —
...
6. If this was the last phase: mark the Status header complete, move both files
   to `src/src/ref/complete/`, update `src/src/ref/README.md` and
   `src/src/ref/structural/ARCHITECTURE.md`, and **note in
   `src/src/ref/wip/npc-initiative-plan.md`'s Handoff that its Phase 2 prerequisite
   is now met.**
