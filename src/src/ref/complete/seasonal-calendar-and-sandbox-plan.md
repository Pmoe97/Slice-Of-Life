...
Companions:
- `src/src/ref/complete/economy-and-rent-plan.md` (owns the bill cadence, tax
  period and rent clock this plan re-times; every number in Part A is a
  re-tuning of a constant that plan introduced).
- `src/src/ref/structural/game-clock-time-system.md` (the as-built map of the
...
- `src/src/ref/complete/game-opening-plan.md` (owns `ECONOMY.opening` — the solo
  start, rent grace and first-bill delay that Part B's sandbox path bypasses).
- `src/src/ref/complete/player-creation-and-intro-plan.md` (owns the Player Design
  studio and `PLAYER_STUDIO_TABS`, the table Part B reuses for NPC appearance).
- `src/src/ref/complete/contractor-tutorial-overhaul-plan.md` (owns the milestone
  flags Phase B7 must pre-set so a restored-house sandbox does not fire the
  tutorial on day 1).
- `src/src/ref/wip/character-cutout-scene-rendering-plan.md` (owns portrait/cutout
...
`src/src/ref/complete/seasonal-calendar-and-sandbox-handoff-prompt.md` for the full
session protocol.
...
**Status: COMPLETE.** All phases A1-A5 and B1-B7 are built and verified (2026-08-22). The plan and its session prompt moved to src/src/ref/complete/ (as a pair) in this commit; this is design record and precedent, no work remains.
...
- **node src/src/dev/verify/verify-sbx-p7.js + node src/src/dev/verify/run-all.js**: not run - this session has no shell (same as B3-B6). verify-sbx-p7.js is written to the p3 pattern; it derives the day-shaped fields the same way snapshotSandboxDayFields does so the list can't drift from the guard; covers D20 (flag set on / absent off; table-key set == 7), the heaviest-vs-solo cross diff, the guard firing, and a write->flush->load round-trip asserting the tutorial flags survive + all day fields stay day-1-shaped. Fixes carried in p7's copy (p3's were too thin - it was never run): p3's makeMemKv lacked keys/delete/update (writeGeneratedGameState needs them) and its D19-style day test set meta.clock.day instead of clock.day; both corrected here so the first harness that actually runs behaves. Run node src/src/dev/verify/run-all.js once a shell exists and confirm green.
...
**Files:**
- `src/src/srcfiles/config.js`: replace `CALENDAR` with the Data-model shape.
...
- `src/src/srcfiles/sim.js`: apply the full rename map. `getWeekday` gets the
...
- `grep -rn "daysPerQuarter\|getQuarter\|isQuarterEnd\|monthNames\|daysPerMonth" src/src/srcfiles/`
  must return **zero** hits outside comments.
- `node src/src/dev/verify/run-all.js` — the whole suite, to catch anything that read
  a weekday index.
...
**Files:**
- `src/src/srcfiles/config.js`: `BILL_DEFS` — set `cadenceDays: 35` on electric,
...
- `src/src/srcfiles/sim.js`: `initBillState`'s `firstDue` offsets are
...
**Files:**
- `src/src/srcfiles/config.js`: `TAX_CONFIG.interestRate` 0.02 → 0.015 with the
...
- `src/src/srcfiles/computer.js`: `computeTaxOwed`'s `postingsPerQuarter`
...
- `src/src/srcfiles/ui.js`: `processTaxesForDayUi`'s `isQuarterEnd` guard
...
**Files:**
- `src/src/srcfiles/config.js`: add `INVESTING.daysPerFinancialYear: 360` with
...
- `src/src/srcfiles/render.computer.js`: the fund row at `:4272` renders
...
**Files:**
- `src/src/srcfiles/sim.js`: replace the Phase A1 `formatDate` stub with the real
  implementation, plus `formatDateShort` and `ordinalSuffix` (Data model).
- `src/src/srcfiles/render.js:45`, `src/src/srcfiles/time.js:390`,
  `src/src/srcfiles/render.phone.js:404`: switch to `formatDateShort`. Take a
...
- `src/src/srcfiles/render.computer.js`: `renderTaxPanel` (`:3699`). The
...
- `src/src/srcfiles/tracker.js`: `trackerTaxes` (`:131`) — the `nextDue`
  computation already adapts after A1; update the two `detail` strings
  ("this quarter", "at quarter end") to say period.
- `src/src/ref/structural/game-clock-time-system.md`: update every statement
...
- Assert the tax panel's countdown on day 69 reads 1 day and on day 71 reads
  69 days.
- `grep -rn "[Qq]uarter" src/src/srcfiles/` — every surviving hit must be a
...
**Files:**
- `src/src/srcfiles/config.js`: add `authoredFields` to `CHARACTER_SCHEMA` as an
...
- `src/src/srcfiles/llm.js`: add `mergeProseIntoBible(bible, prose, authoredFields)`
  ...
  on.
- `src/src/srcfiles/ui.js`: `approveCastAndStartGame` (`:5989`) — replace the
...
**Files:**
- `src/src/srcfiles/sim.js`: extract `applyAuthoredPhysical(rolledPhysical, authored)`
  ...
  is untouched (D15).
- `src/src/srcfiles/sim.js`: update `rollCastSlot`'s header comment (`:3428`),
...
**Files:**
- `src/src/srcfiles/sim.js` (or a new `sandbox.js` — decide by whether it needs
...
- `src/src/srcfiles/config.js`: add `SANDBOX_HOUSE_PRESETS` — the three D17
...
- `index.html` (this is **`index.html` inside Perchance** — see the handoff
  ...
  changes.
- `src/src/srcfiles/menu.js`: the `menu.sandbox` action. Follow the existing
  menu-action dispatch (`:219`) — this codebase routes through `ui.js`'s
  global `data-action` chain, not per-element listeners.
- `src/src/srcfiles/ui.js`: `startSandboxGame(cfg)`, mirroring `startSoloGame`
...
**Files:**
- `src/src/srcfiles/studio.js`: generalise the studio to take a `subject`
...
- `src/src/srcfiles/ui.js` (or the sandbox module): the roommate list UI — add /
...
**Files:**
- `src/src/srcfiles/ui.js` (or the sandbox module): the house panel. Three preset
...
- The sandbox module: implement step 7 of `applySandboxPreset` — D20's
  ...
  id — find them with `grep -rn "fireContractorMilestone(" src/src/srcfiles/`
  ...
  `grep -rn "flags?\.tutorial_" src/src/srcfiles/`.
...
- `src/src/ref/complete/seasonal-calendar-and-sandbox-plan.md`: mark complete.
...
- Start `restored` with `suppressTutorial: false` and assert the milestone
  *does* fire, proving the flag is doing the work and the beat is not simply
  broken.
- `node src/src/dev/verify/run-all.js` green.
