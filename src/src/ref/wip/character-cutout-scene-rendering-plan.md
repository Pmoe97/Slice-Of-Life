...
does not exist yet — write it from `src/src/ref/patterns/HANDOFF-PROMPT-ARCHITECTURE.md`
...
- **The idea's provenance.** `perchance.org/persona-realm` (fetched to
  ...
  section below.
- **Our current cost.** `src/src/srcfiles/image.js:getSceneImage` bakes every
...
**Files:**
- `src/src/srcfiles/image.js`: `cutoutKey`/`cutoutOutfitToken`; `composeCutoutSeed`
...
- `src/src/srcfiles/config.js`: `CUTOUT_TUNING`; `IMAGE_CACHE.resolutions.cutout =
...
**Files:**
- `src/src/srcfiles/image.js`: `buildBackgroundPrompt(roomId, phase, roomObjects)`
...
- `src/src/srcfiles/config.js`: nothing new (plate resolution reuses
...
**Files:**
- `src/src/srcfiles/render.js`: `sceneArtContext` returns plate key/prompt/seed
...
- `src/src/srcfiles/image.js`: `layoutSceneCutouts(gs, sceneState, plateKey)`
...
**Files:**
- `src/src/srcfiles/image.js` + `src/src/srcfiles/ui.js`: `rerollSceneImage` now
  regenerates under the **plate key**; modal prompt/seed fields are
  plate-scoped (D11).
- `src/src/srcfiles/render.js`: degrade paths — plate failure → existing
...
**Files:**
- `src/src/srcfiles/image.js`: final `CUTOUT_OUTFIT` keying (`cutoutOutfitToken`
  ...
  mood, else neutral).
- `src/src/srcfiles/config.js`: final `CUTOUT_POSES`/`CUTOUT_EXPRESSIONS`;
  speck params tuned against real cutouts (vision pass on several
  characters — halos, clipped fingers, floating hair).
- `src/src/srcfiles/settings.js` (or `kv.menu 'settings'` read-if-present):
