...
- When told to reuse a pattern, go read that code and match its current
  ...
  - `planWalk` / `advanceFrameWalks` / `deriveLocationFromPosition`
    (`src/src/srcfiles/movement.js`) — the path re-planner and the live
  ...
  - `renderFloorPlanLive` / `floorPlanAvatarPlacement` (`src/src/srcfiles/render.js`)
    — the per-frame loop the presentation layer takes over, and the SIM
    position source it must not mutate.
  - `resolveActionAnchor` (`src/src/srcfiles/actions.js`) — where the stand-point
    computation lives; Phase 2 swaps its centroid fallback for
    `resolveObjectStandPoint`.
  - `renderAutoFurniture` (`src/src/srcfiles/render.js`) — the packing Phase 2
    extracts into `resolveAutoPlacements`; extract verbatim so the picture
    doesn't change.
  - `buildScenePrompt` / `conversationContinuityLine` (`src/src/srcfiles/llm.js`)
  ...
  - `activeConversationSession` / `doConvSend` (`src/src/srcfiles/ui.js`) — the
