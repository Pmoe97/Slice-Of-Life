...
Companions:
- `src/src/ref/structural/game-clock-time-system.md` — the dilation loop (`TIME.clockFrame`, the context stack) that Phase 10's speed multiplier wraps, and the `AUTOSAVE_MS` timer Phase 4 retunes. Read §3 and §9 before Phase 10.
- `src/src/ref/complete/inventory-needs-menu-saves-plan.md` — built the VN-style save panel (`#save-panel`), the main-menu component, and the `kv.menu` 'options'/'prefs' keys that Phases 1–3 rework.
- `src/src/ref/complete/external-world-npcs-overhaul-plan.md` — the visit spine and every external presence (maid, escorts, contractors, friends-of-friends, food drivers) whose generation Phase 6 re-populates.
- `src/src/ref/complete/afterhours-redesign-plan.md` — Hot Singles / AfterHours profile generation (a Phase 6 population target).
- `src/src/ref/structural/perchance-menu-conventions.md` — the reference-game menu structure the pause screen restyles. The deviation notes in it are deliberate; keep them.
- `src/src/ref/complete/prompt-generator-v2.md` — built `PROMPT_V2` in `defs.menu.js`, which Phases 5–7 consume (art-actor distribution, race descriptors, style suffix).
- `src/src/ref/wip/home-design-studio-plan.md` — explicitly **not** touched: themes must not alter in-world visuals (`ROOM_DECOR`/shapes), only UI chrome.
...
- `src/src/srcfiles/time.js`: `getTimeScale()` now returns
...
- `src/src/srcfiles/render.js`: new `renderSpeedButtons()` (marks `data-active`
  ...
  full render pass.
- `src/src/srcfiles/ui.js`: `'speed.set'` added to `MENU_ACTIONS` and
...
- `src/src/srcfiles/defs.settings.js`: the Data tab config replaces the empty
...
- `src/src/srcfiles/settings.js`: `storageSummary()` (reads kv LIVE, never a
...
- `src/src/srcfiles/menu.js`: new `kind:'storage'` branch in
...
- `src/src/srcfiles/ui.js`: MENU_ACTIONS + ENERGY_GATE_EXEMPT gained
...
**Files:**
- `src/src/srcfiles/defs.settings.js`: `SETTINGS_DEFAULTS`, `SETTINGS_TABS`
  ...
  one place.
- `src/src/srcfiles/settings.js`: `loadSettings()` (migrate + merge),
...
- `src/src/srcfiles/ui.js`: call `loadSettings()` once at boot before first
...
- `src/src/srcfiles/menu.js`: `openSettingsScreen(tab)` (records origin:
...
- `src/src/srcfiles/ui.js`: new actions in `MENU_ACTIONS` + dispatch:
  `'settings.tab'`, `'settings.back'`, `'settings.open'` (with `extra.tab`).
- `src/src/srcfiles/defs.settings.js`: General tab rows that are already real —
...
- `src/src/srcfiles/menu.js`: `showMainMenu('pause')` shows the pause screen,
...
- `src/src/srcfiles/ui.js`: the `'menu'` action and dispatch already route
...
content pipeline.
**Files:**
- `src/src/srcfiles/state.js`: `AUTOSAVE_MS` becomes a runtime read
  (`autosaveIntervalMs()`); `startAutosave` uses it.
- `src/src/srcfiles/settings.js`: `setSettings` re-arms a live autosave timer
  when `autosave`/`autosaveInterval` changes and a game is active.
- `src/src/srcfiles/settings.js` + `src/src/srcfiles/ui.js`: `applySfwMode` wiring —
...
- `src/src/srcfiles/defs.settings.js`: General rows — SFW mode (toggle),
...
**Files:**
- `src/src/srcfiles/defs.settings.js`: `RACES` (10-entry starter pool, D13
...
- `src/src/srcfiles/settings.js`: `genderDistSampler()` / `raceDistSampler()`
  (proportional picks from the settings distributions), `identityToArtTag()`
  (D14 mapping), `raceArtPhrase(species)`.
- `src/src/srcfiles/defs.menu.js`: `chooseActorGenders`/`rollActors` draw from
...
- `src/src/srcfiles/ui.js`: `'set.population-dist'` action (axis set → normalize
  → `setSettings`); drop `prefs.*` actions from `MENU_ACTIONS`.
- `src/src/srcfiles/menu.js`: remove the six toggle rows from the boot options
...
**Files:**
- `src/src/srcfiles/sim.js`: `rollGender` reads `settings.genderDist` (fallback
...
- `src/src/srcfiles/config.js`: `CHARACTER_SCHEMA.bible.species` —
  `{ type:'string', required:false, default:'human', enum: RACES ids }`.
- `src/src/srcfiles/computer.js`: thread `species` (same append-at-end rule) into
...
- `src/src/srcfiles/npc.js`: `getPhysicalDescriptionForPrompt` prepends the race
  article+noun and appends `traitPhrase` for non-human species.
- `src/src/srcfiles/state.js`: save migration backfills `bible.species`='human'
...
**Files:**
- `src/src/srcfiles/defs.settings.js`: `IMAGE_STYLES` (18 + `custom` sentinel) +
  Images tab config (style grid, Custom text field, Clear cached images).
- `src/src/srcfiles/image.js`: `applyImageStyle(prompt)` helper (appends the
...
- `src/src/srcfiles/settings.js`: `clearImageCache()` (wipe the image LRU's kv
...
**Files:**
- `src/src/srcfiles/defs.settings.js`: `COLOR_THEMES` (14, D10 groups),
...
- `src/src/srcfiles/settings.js`: `applyTheme()` sets `data-theme` on
...
- `src/src/srcfiles/settings.js`: `currentSpeed()/setSpeed(id)` (session-local,
  D12).
- `src/src/srcfiles/time.js`: `clockFrame` multiplies the resolved context scale
...
- `src/src/srcfiles/render.js`: `renderHeader` highlights the active speed
  (data-attr).
- `src/src/srcfiles/ui.js`: `'speed.set'` action + `MENU_ACTIONS` entry.
