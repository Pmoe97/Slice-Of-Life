...
```
src/src/ref/complete/continuous-behavior-engine-plan.md
src/src/ref/complete/needs-and-heartbeat-plan.md
src/src/ref/complete/external-world-retiming-plan.md
src/src/ref/complete/npc-initiative-retiming-plan.md
src/src/ref/complete/decor-economy-plan.md
```
...
- `resolveWalk` / `planWalk` conventions
  (`src/src/ref/wip/floorplan-and-movement-plan.md`, status: built) — the
...
2. **`state.js` is the sole kv access point.** A new module never calls
   ...
   worked example).
3. **A new `src/src/srcfiles/*.js` file needs a line in TWO places: `index.html`'s
...
(`node src/src/dev/verify/run-all.js`) — you cannot execute it directly.
...
4. **If this session completed the document's last phase:**
   ...
   - Move the file from `src/src/ref/wip/` to `src/src/ref/complete/` — this
   ...
   - Update `CONTINUOUS-SIMULATION-ROADMAP.md`'s own section for that
     ...
     Step 0.
   - Update `src/src/ref/README.md`'s doc tables and
     `src/src/ref/structural/ARCHITECTURE.md`'s doc index to move that row
