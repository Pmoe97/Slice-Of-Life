...
**Repo layout: the doubled `src/src/` is deliberate.** Perchance's own
container is `index.html` + `main.pjs` (hard-coded, sitting outside the
editor's own top-level folder) + one folder — also called `src` there —
that the editor lets you Download as a single zip; that outer container
itself can't be renamed, moved, or downloaded as a whole, and `dev/`,
`ref/`, `srcfiles/` all live one level inside it. This repo mirrors that
shape exactly: the outer `src/` here stands in for Perchance's
un-downloadable top container, and everything Perchance actually lets you
export/import as one unit lives in the nested `src/src/` beneath it. Only
`index.html`/`main.pjs` sit outside both layers, matching Perchance's two
hard-coded files. Restructured 2026-08-31 (was a flat `src/{ref,srcfiles}`
plus a top-level `dev/` before) so a Perchance download/upload round-trips
cleanly with no path translation.

Designs live in their own plan docs rather than here, so they survive across
sessions. **`src/src/ref/README.md` is the index of record** — it lists every
...
| Plan doc | Covers |
|---|---|
| `src/src/ref/complete/economy-and-rent-plan.md` | Rent, cost stack, metered utilities, quarterly taxes, investing |
| `src/src/ref/complete/apartment-upgrades-plan.md` | Disrepair start, facility repair, quality → rent leverage |
| `src/src/ref/complete/game-opening-plan.md` | The Stardew-like intro and how the player acquires the apartment |
| `src/src/ref/complete/apartment-expansion-plan.md` | The Mirrored H layout (built — kept for the adjacency/room rationale) |
| `src/src/ref/complete/npc-overhaul-plan.md` | NPC bible/personality/memory system (built — kept for design rationale; no summary row below) |
| `src/src/ref/complete/renovation-occupancy-overhaul-plan.md` | Timed staged renovation jobs, per-bedroom facilities (**built** — all 4 phases) |
| `src/src/ref/complete/contractor-tutorial-overhaul-plan.md` | Del Connors, job pricing, the tutorial he anchors (**built** — all 4 phases) |
| `src/src/ref/complete/external-world-npcs-overhaul-plan.md` | External world: visit spine, contacts, maid, food delivery, friends-of-roommates, escorts, move-in advocacy (**built** — all 8 phases) |
| `src/src/ref/complete/restaurant-network-expansion-plan.md` | 12-restaurant roster, full menus, wrap-aware hours, cross-midnight deliveries (**built** — all 4 phases) |
| `src/src/ref/complete/afterhours-redesign-plan.md` | AfterHours site expansion: PH+EP blended feed, routed mini-site, player page + related rail, parody ads, watch persistence, Hot Singles NPCs (**built** — all 8 phases + audit) |
| `src/src/ref/complete/inventory-needs-menu-saves-plan.md` | Inventory, containers, eating, spoilage, needs rebalance, Set Meal, NPC inventories, save system v2, main menu (**built** — all 10 phases) |
| `src/src/ref/complete/decor-economy-plan.md` | The "Home" computer app sells furniture through Nile's exact checkout/delivery mechanism (`checkoutCart`/`processDeliveriesForDay`), unmodified. Placing a delivered piece writes a real object instance into `gameState.objects` — not a `ROOM_DECOR` visual entry — so `continuous-behavior-engine-plan.md`'s anchor resolution finds player-furnished rooms the same way it already finds a stove. Structural changes stay `STRUCTURAL_UPGRADES`/RenoFix, a deliberately separate catalog. **Built** — all 3 phases; Phase 3 verified the buy→deliver→place→anchor worked example end to end against `resolveActionAnchor`. |
| `src/src/ref/complete/needs-and-heartbeat-plan.md` | Needs decay/restoration, phone battery, and memory decay move off the 30-minute tick onto one fine periodic heartbeat — one accumulator inside the continuous loop, one heartbeat for all three consumers. Fast-forward always resolves in closed form, never by looping the heartbeat. `hunger` (already derived via `HUNGER_RHYTHM`) explicitly excluded. **Built** — all 4 phases, D1–D8 locked. Phase 4's live pass confirmed `HEARTBEAT_MINUTES` at **5** (D8): 1/5/30-min chunking of the closed form is byte-identical across every consumer and checkpoint block transition, so the finer cadence bought nothing at 5× the calls. The `?? 50` vs `|| 50` floor-clamp note and the "bare `resolveTick` now freezes needs" finding live in its Handoff — read them before driving needs anywhere. A 2026-08-15 audit found and fixed a real gap in the discrete path's phone-battery/memory-decay join (see the plan's Addendum). |
| `src/src/ref/complete/external-world-retiming-plan.md` | The visit spine, restaurant hours, food-delivery ETAs, and gig work blocks convert from `[startTick,endTick)` to `clockToAbsolute`-space windows. Restaurant hours convert as a *recurring* daily rule (minutes-from-midnight, still wrap-aware) rather than a one-shot window like the other three — the plan's one genuine subtlety. Day rollover, rent, and bills confirmed out of scope. **Built** — all 4 phases; gig work's one time-cost literal now reads `GIG_TUNING.workBlockMinutes` (D5). |
| `src/src/ref/wip/character-cutout-scene-rendering-plan.md` | **The scene-rendering overhaul** — plates + cutouts. Scenes become a people-free background plate plus per-character transparent cutout layers (BRIA RMBG-1.4 via the plugin's `removeBackground:true`, persona-realm's specks cleanup + alpha-bbox), cached separately and moved with CSS transforms. D1–D16 locked 2026-08-21; **all 6 phases code-complete** — `sceneArtContext`/`renderScene` now draw a people-free plate plus diffed `.scene-cutout` layers, `composeSceneKey`/`composeSceneSeed`/`getSceneImage` are deleted (`buildImagePrompt` survives only as `buildPhotoPrompt`, scoped to the camera, which legitimately keeps its people), `IMAGE_PROMPT_VERSION` is `pv4`, LRU cap 500. Verified by `dev/verify/verify-cutout-p{1,2,3,45,6}.js` (96 checks) plus live browser checks of the layer diff/geometry/resize. **Still in `wip/`**: no real cutout pixel has been generated (Perchance-only `generateImage`) — see the plan's live-run checklist. Peek/portraits/menu gallery untouched. |
| `src/src/ref/complete/avatars-and-sprite-studio-plan.md` | **Every person stops being a letter in a coloured circle** — the instrument that closes the cutout plan's live-run gate above: not a better algorithm, but handing the player an eraser. A permanent `kv.sprites` override store, deliberately OUTSIDE `SAVE_KEYS` (browser-local, like `kv.menu`/the image LRU) so loading an older save can never destroy art painted later; `resolveSprite` is the one function allowed to read a sprite blob, in one fixed order (exact override → wildcard → session memo → generated cache → generate → placeholder, D3), style-agnostic slot ids no version bump can orphan (D4/D5), every override keeping its untouched `master` beside the working image (D6). One `avatarChip` component (`src/src/srcfiles/avatar.js`) replaced 26 hand-rolled letter-circle templates project-wide (D11); avatars derive from the cutout by default and are independently overridable (D8); a background queue keeps the household's art ready without competing with foreground generation (D12–D14). The **Sprite Studio** (`src/src/srcfiles/spritestudio.js` + `render.spritestudio.js`, computer-and-phone, D16) gives the player a cleanup suite wired to the cutout plan's own `CUTOUT_TUNING` live (D18) plus a full paint/erase/colour/crop editor — parametric matte+colour+geometry over a destructive stroke+paint buffer, one commit per drag (D17/D20/D23–D25) — and a Phase 7 recrop surface for the headshot crop itself. D1–D25 locked, design session 2026-08-27; **COMPLETE — all 8 phases built and verified 2026-08-27** (`verify-sprite-p1..p8.js`, 227 assertions). Phase 8's own sweep pinned the boundary invariants as assertions (D1's origin allowlist, D21's studio-writes-pixels-never-people) and added `exportSpriteOverrides`/`importSpriteOverrides` (`state.js`) so a painted household can be carried to a new save, reusing `exportSaveRecord`'s gzip/base64 envelope; live-verified byte-identical in the browser. Two Phase 6 items remain open by design (reopening an editor session loses destructive edits back to the tuned master; the scene preview generates from a render path) — recorded in the plan's own Handoff, not silently fixed. Paired with `avatars-and-sprite-studio-handoff-prompt.md`. |
| `src/src/ref/complete/SENSORY-AND-SOCIAL-ROADMAP.md` | **Umbrella for six linked overhauls** turning the apartment sensory and the NPCs autonomous. Holds the cross-cutting decisions `R1`–`R8` and the theses for Plans 2–5. Read before any NPC or presentation work. |
| `src/src/ref/complete/settings-and-pause-overhaul-plan.md` | **The settings + pause overhaul** — a real pause menu over a dimmed game (no more boot-title reuse; Resume/Save/Load/Options/Discord/Quit to Menu) and a searchable tabbed settings surface (General/Population/Images/Appearance/Data) where every option is wired or cut. D1–D14 locked, design session 2026-08-19 (revised after the race poll), planned — not started. Two settings surfaces (D1): boot Options row keeps bg-art/debug, gameplay settings live in the tabs. Browser-local `kv.menu` 'settings' store (D4) with one-way migration from the old 'options'/'prefs' keys; SFW toggle is the one cross-over — it patches `meta.contentConfig.contentFlags.mature` and rides the pipeline that already exists (`menuRatingCap`, `CONTENT_DIRECTIVES`, `intimateAllowed`, `activeContentFlags`). **The Population tab governs gender AND race densities of ALL cast generation** — apartment cast at new game, Hot Singles, maid/escort/friends-of-friends/contractors/food drivers, applicants, background-art actors — via the `rollGender`/`rollCastSlot` choke points (D8); races default to human 100% with fantasy OFF (player poll), 10-race starter pool (D13); gender distribution is the five cast identities from `CHAR_GEN.genderWeights`, art actors derive their presentation tag from the roll (D14); Del is exempt. 18 premade styles + Custom appended through one `applyImageStyle` helper in image.js, folded into every cache key. 14 themes over the `:root` tokens. Font-only text sizing. Data tab = export/import/reset/storage insight. x0/x1/x20/x100 game-speed HUD in the header (D12), a multiplier over the clock context scale — explicitly not a settings option. Paired with `settings-and-pause-overhaul-handoff-prompt.md`. |
| `src/src/ref/complete/settings-and-pause-overhaul-handoff-prompt.md` | The session prompt for the row above (one-prompt-per-plan convention). Exceptions to in-order execution named: Phases 4/7/8/9 independent once Phase 2 lands; Phase 10 needs only Phase 1; **Phase 6 must follow Phase 5** (consumes `raceDist`/`RACES`). Hard rules: settings stay out of `SAVE_KEYS` (only the D5 SFW flag patches `meta.contentConfig`), determinism (new draws appended at the END of sequences — mid-stream insertion shifts every seed's cast; styles are prompt-text only, style folds into image cache keys), authored NPCs exempt (`partial.species` pins a species), `?v=` bumps + `loadgame.js` `ORDER` registration in the same commit, every setting row's action id must exist in `MENU_ACTIONS`, and verification on the live page (`browser_eval`/`vision`) with the settings round-trip as the minimum. Phase-specific obligations: 4 records the scaled text components, 5 the `RACES` ids + identity→art-tag mapping, 6 every generation site threaded + the default-settings same-seed cast check, 7 the touched cache keys, 8 the 14 theme ids, 10 the measured x100 checkpoint behaviour. |
| `src/src/ref/complete/vocation-and-lifestyle-expansion-plan.md` | **The NPC vocation + lifestyle overhaul** — an occupation stops being a character-independent absence generator. Today `OCCUPATION_POOL` is 20 flat entries whose only behavioral effect is that `work` blocks set `location: null`, and occupation is rolled BEFORE temperament in `rollCastSlot`, making personality coupling structurally impossible. The plan puts a **`workMode`** (`on_site`/`hybrid`/`remote`/`self_employed`/`none`) on the occupation and deliberately does **not** add schedule block names (D1) — so `BLOCK_TIME_OF_DAY`, `ACTIVITY_TABLES`, `WORK_BLOCKS`, the drives' `timeOfDay` filters and the harness block-name assertions are untouched, and only the nine sites that assume work-means-offscreen ask the new question through one predicate, `npcIsOffsite` (D12). Personality coupling reuses **`npcDisinhibition`** rather than adding a temperament axis (D8 — `modesty` is a *garment* stat, not a disposition; the earlier proposal had this wrong), with affinity supplied through `weightedPick`'s existing `weightFn` (D7, there is no `baseWeight` field to multiply) and a hard disinhibition floor as the pool's only zero-weight gate (D9/D11). Adds unemployment as a workMode with real money behind it (`incomeSource: means`, riding `SCHEDULES.standard`'s existing no-work-block path, D3/D20/D21), content-creation work as **drives** rather than blocks — bedroom sessions, a rare late-night pool session, a high-relationship collab invitation on the existing `propose` overture channel, and a couple variant of `intimate` that never bypasses the willingness gate (D16–D19). Two flagged consequences nobody had noticed: a remote worker becomes newly eligible for the `interruption.js` walked-in-on roll (D13 gives it a multiplier rather than letting the rate move by accident) and would wear the office outfit at home all day (D14). D1–D23 locked, design session 2026-08-22; **complete — Phases 1–8 all built and verified** (Phase 8, income→rent, shipped with its reader per D23; tuned rent curve untouched, A/B Δ +$104/wk player pressure, direction up). Four further Phase-7 lifestyle dimensions captured spec-only in the doc, not authored. Phase 1 is a pure byte-identical refactor; Phase 8 (income→rent) is deferred and is the only phase permitted to touch a rent number. |
| `src/src/ref/complete/vocation-and-lifestyle-expansion-handoff-prompt.md` | The session prompt for the row above (one-prompt-per-plan convention). Step 0 once named Phase 7 as the only remaining phase to start unprompted and Phase 8 (income→rent) as deferred BY DESIGN (an agent must stop and ask the user rather than begin it just because the table says Not started, because the rent curve is tuned and carries the game's central pressure). **All eight phases are now complete (2026-08-22): Phase 8 shipped first, then the four Phase-7 lifestyle dimensions (`styleLean`/`foodLean`/`sleepRhythm`/`spendingLean`) shipped field+reader together, verified in `verify-voc-p9.js`.** Carries the nine hard rules (D23 no-field-without-its-reader — the `stressProfile` scar, and the rule that governed Phase 7 absolutely; the closed block vocabulary; one `npcIsOffsite`; nudity decided in one place; the willingness gate never bypassed; cast generation never hard-failing; the D6 RNG-reorder fallout; two-place registration for new files; `?v=` bumps). Includes the **Node→`browser_eval` translation rule** for a Perchance-native agent with no shell — read the `dev/verify/verify-voc-*.js` harness, reproduce its thresholds against the live page, and `browser_refresh` first — plus the measured Phase 7 finding (unemployed NPCs idle because the best candidate scores ~0.356 against a 0.40 threshold, so the answer is low-stakes idle drives and NOT lowering `actionThreshold`) and the list of pre-existing verify failures not to go chasing. |
| `src/src/ref/complete/seasonal-calendar-and-sandbox-plan.md` | **The calendar resize + sandbox mode.** Part A turns `CALENDAR` into four **35-day seasons** (140-day year, five 7-day weeks per season, so every season AND year begins on a Sunday forever — the 360-day year's weekday drifted 3 days a year) and **splits the season period from the tax period**: `daysPerSeason: 35` drives HVAC and the date line, `daysPerTaxPeriod: 70` drives taxes, which now bill at the end of Summer and the end of Winter. `getQuarter`/`isQuarterEnd`/`getQuarterDay` become `getTaxPeriod`/`isTaxPeriodEnd`/`getTaxPeriodDay`; new `getSeasonIndex`/`isSeasonEnd`; `getWeekday` becomes `((day + 5) % 7)` so day 1 is a Sunday **without touching `WEEKDAY_NAMES`** (maid contracts persist raw 0–6 weekday indices); `monthsPerYear`/`daysPerMonth`/`monthNames` are deleted. All six utility bills go to `cadenceDays: 35` with the existing first-due stagger **kept deliberately**. Everything is re-tuned to hold **dollars-per-day constant** — per-cycle constants ×7/6, per-day and per-unit rates untouched — and `INVESTING.dailyReturn`'s `/360` becomes a named `daysPerFinancialYear` that is deliberately NOT `CALENDAR.daysPerYear`. Part B adds Sandbox mode: `bible.authoredFields` + `mergeProseIntoBible` (authored fields provably safe rather than accidentally safe), a `physical` partial on `rollCastSlot` sharing `generatePlayerAppearance`'s extracted merge, `PLAYER_STUDIO_TABS` generalised to a subject, and `applySandboxPreset` — an ordered patch between generation and the first write whose step order is load-bearing (`applyStructuralUpgrades` and `applyFacilityCompletionStates` do not fire themselves). **Sandbox always starts on day 1 (D19)** — it advances the house, never the calendar, and rewrites no absolute day field. D1–D21 locked, design session 2026-08-21; **COMPLETE — all phases built and verified 2026-08-22**. Paired with `seasonal-calendar-and-sandbox-handoff-prompt.md`. |
| `src/src/ref/complete/sandbox-editor-overhaul-plan.md` | **The Sandbox pre-game editor rewrite** — turns the one 400-line scrolling `renderSandboxScreen()` (menu.js) into a real tabbed/sub-tabbed editor: Player / Roommates / House / Economy & Difficulty, modeled on the settings screen's data-driven `SETTINGS_TABS` shape (D1) rather than the appearance studio's flat per-tab field list — the studio itself (`PLAYER_STUDIO_TABS`) is explicitly kept as a leaf dependency, launched unchanged from every Design button, never reimplemented inline (D2). Roommates gets the real new surface: a three-level nest (top tab -> roommate rail -> five per-roommate sub-tabs: Identity/Personality/Interests & Values/Backstory/Placement & Prose, D3) that neither existing tab pattern in the codebase has ever done. Economy stops being a near-total stub (today it displays exactly one of its four data-model fields, read-only) and becomes a real difficulty system: named presets (Comfortable/Standard/Tight) over four already-existing-but-never-wired fields, with "Custom" always DERIVED by comparing live values against the presets rather than stored as a flag that could drift (D8). Pure view-layer rewrite — `pendingSandboxConfig`, the roommate `partial` shape, and `applySandboxPreset` are explicitly unchanged (D10) — with one exception a same-day post-implementation audit forced: two of the four Economy fields (`rentGraceDays`/`billsStartDay`) were UI-writable but read by nothing, fixed by threading an optional `economyCfg` param through `SIM_generateHouse`/`buildGameState`/`initBillState` rather than writing them in `applySandboxPreset`, where D19's guard forbids touching an already-stamped day field. D1-D12 locked, design session 2026-08-23; **complete — all 6 phases built and verified 2026-08-23**. Phase order ran shell -> House -> Player -> Economy & Difficulty -> Roommates -> polish, then the audit pass. |
| `src/src/ref/complete/seasonal-calendar-and-sandbox-handoff-prompt.md` | The session prompt for the row above. Phases **A1–A5 / B1–B7**; the two halves are **independent** — all of Part B depends on nothing in Part A, so either is fallback work when the other stalls. Carries the hard rules that keep the re-tuning honest (per-day parity as the acceptance test; scale `interestRate` but not `underpaymentPenalty`; no save migration; the stagger stays; `lastQuarterBilled` keeps its name; sandbox never rebases a day field) and the verification split (Node harnesses for the pure half, live Perchance page for anything the render layer touches). |
| `src/src/ref/complete/bug-fix-audit-2026-08-17.md` | **CLOSED — the tracked 2026-08-17 playtest audit** — Good/Bad/Ugly findings with evidence + status. Pass 1 (two-device Messages fix, functional showers at start, weekly rent label, gig-economy rebalance, energy-gate deliver fix, LLM-judge parse retry), Pass 2 (the hunger pass — edible starter ingredients/Eat chip Day 1, real meal-size hunger restores, sleep-slowed overnight hunger), and Pass 3 (the picker-race eating fix) all shipped 2026-08-17; all 7 Bad + 4 Ugly findings fixed and verified live. Moved from `wip/` 2026-08-18 — a future playtest audit is a new dated document, not a reopening of this one. |
| `src/src/ref/wip/npc-speech-pattern-fix-plan.md` | **OPEN — the 2026-08-30 NPC speech-pattern fix; all six reported symptoms fixed and verified live** (prompt-text + guard-logic checks; no real LLM call yet). Megan's dialogue had looped into a script: (1) "honestly… X, right? fair enough." every line — `[Speech]` tics/catchphrases read as a required format → reworded to explicit RARE seasoning in `buildNpcBlockV2`'s `[Speech]` (llm.js) + a new tic rule in `buildStyleSection` (prompt.js); (2) verbatim self-repetition → new "never reuse a line from [Memories — recent]" CRITICAL RULE plus a mechanical guard (`normalizeDupText`/`findTranscriptDuplicates`/`retryWithDedupRule`, wired into `callLLM`): any produced line ≥15 chars matching the speaker's scene transcript triggers ONE banned-lines retry, re-validated before accept; (3) canned "back already" room-entry greeting → new `buildTurnTypeDirective` (llm.js), fires only on the literal `'You walk into the room.'` playerAction (ui.js:7553): no greeting/commentary, speak only with a genuine reason and only a transcript-fresh line, silence allowed; (4) static-pose re-description + filler micro-actions → narration-must-ADVANCE and no-filler-actions rules (also caught by the mechanical guard); (5) non-sequitur dialogue → must-answer-the-specific-player-action rule; (6) monotone internal thoughts + tone-label topics → internal-must-vary-and-reference-something + topic-must-be-concrete rules. No profile/save edits (prompt+guard covers every NPC); old pattern lines aren't rewritten but die out of the 40-cap buffer. Stays in `wip/` pending the user's next real conversation; the doc carries full wording and the `?v=`-bump question. |
| `src/src/ref/complete/food-overhaul-plan.md` | **The food feature overhaul** — kcal as the one metabolic currency and a living metabolic rate merged with the hunger rhythm (D1–D4); meals as derived plate instances with a Servings bar, sum-of-parts + complexity bonus, batch yields and leftovers (D5–D7, D25); fats/seasonings, the interactive verb+stage+method cooking engine with F–S+ grades (D8, D16, Phase 5), equipment gating failure risk/throughput and grade-gated auto-cook (D12–D15, Phase 6, split from the engine phase); real dish objects with sizes/capabilities and capacity-modeled washing (D9–D11); freezer + thawing + gentler, consolidated spoilage + auto-sorted grocery pickup (D17–D19, D29); frozen food must be reheated before eating or forfeits its `betterHot` mood bonus, with a mood penalty for eating ordinary food frozen (`frozenFood`-tagged items like ice cream exempt) (D26–D28); cook-from-storage (D20); recipe website with unlock-on-taste and add-to-cart + meal planner (D21/D22); NPC tastes (D23/D24). **Complete — all 9 phases built, D1–D41 locked, 2026-08-18.** Phases 1–6: freezer/thaw + spoilage + pickup; kcal + metabolism; plates + Servings bar; dishes/cookware/dishwasher; the cooking engine + grades; equipment tiers + grade-gated auto-cook. Phase 7: derived-but-stable NPC tastes (`taste.js`), the eat drive eating plate servings by calories→hunger, NPC auto-cook, taste-scaled set_meal deltas. Phase 8: ChefBook the recipe website (`APP_DEFS.recipes`, `devices:['computer','phone']`), `RECIPE_CARDS`-shaped objects built on demand from `RECIPES` through the real `cooking.js` engine, unlock-on-taste, Add All Ingredients to Cart + the meal planner's Fill Nile Cart (both diff the whole kitchen AND the cart already queued, so a repeat click never doubles an order). **Phase 9** (balance/migration/long-tail) turned out to be almost entirely an audit — `METABOLISM`/`ROT`/`PLATE_TUNING`/`DISH_TUNING` were already correctly tuned by the phases that own them, `fullnessStatusText`/kcal-in-pickers/the doormat auto-transfer button were already built, `mealsToday` had one consistent writer — plus D41: no new save-migration code was actually needed (kcal lives in code not save data; legacy pre-Phase-3 `meal_*` stacks are intentionally left un-migrated per design invariant 1; the freezer backfills onto old saves through the existing `ensureAllObjectBuckets` layout-version pipeline Phase 1 already wired), verified end-to-end by a new `verify-food-phase9.js` and a live week-long browser simulation (cook/eat/freeze/reheat/feed-an-NPC, zero console errors). Phase 9 also cleared a harness-tooling debt Session 10 had flagged: `dev/verify/run-all.js` now reports **0 harness(es) errored** (down from 18), including several genuine bugs in unrelated plans' harnesses (a missing closing brace, double-escaped quotes, bare references to vm-sandboxed symbols from outer Node scope). |
| `src/src/ref/complete/food-overhaul-handoff-prompt.md` | The session prompt for the row above (one-prompt-per-plan convention). Hard rules beyond the standard set: the async-picker live-state hazard (an action that awaits inside `prepare()` then mutates state must re-derive the live `currentGameState`, not write through the pre-await capture — found and fixed three times in this project already), `loadgame.js`'s `ORDER` array must list every file `index.html` loads in the same commit (a real prior incident silently dropped 175 assertions), and a new `dev/verify` harness isn't verified until it's actually run and prints its pass/fail summary line — not merely `node --check`-clean (five brand-new harnesses in this repo were syntactically broken and had never once executed before this was caught). Also states the pure-logic-vs-live-page verification split (`dev/verify` for anything below render/ui, `browser_eval` for the rest). Its Step 0 already handles the all-phases-complete case (stop and report), so it needs no edit now that the plan it serves is done. |
| `src/src/ref/complete/npc-correctness-fixes-plan.md` | Roadmap Plan 0 — the five defects the 2026-08-10 NPC audit found: reversed conversation buffer, unreachable `early` relationship phase, FIFO memory eviction, pinned need economy, dead-field triage (**built** — all 5 phases, 151 assertions). Holds the authoritative dead-field disposition table. |
| `src/src/ref/complete/perception-and-signals-plan.md` | Roadmap Plan 1 — the signal substrate: standing + transient signals, propagation over `ROOM_ADJACENCY` with per-channel door attenuation, notes, and ONE perception query shared by the player and every NPC (**built** — all 5 phases, 171 assertions). `src/src/srcfiles/signals.js` is the section it created. |
| `src/src/ref/complete/scene-reader-ui-plan.md` | Roadmap Plan 2 — `#main-content`'s log becomes a room-scoped scene: composed establishing prose, beats, collapsed history, a moodle strip, sensory icons on the floor plan, attention callouts, and a conversation pane that remembers (**built** — all 5 phases, 110 assertions). Created `src/src/srcfiles/scene.js`. |
| `src/src/ref/complete/npc-cognition-plan.md` | Roadmap Plan 3 — utility scoring replaces the independent per-drive coin flips in `evaluateDrives`; an NPC commits to one `pursuit` for several ticks. **Built** — all 5 phases: created `src/src/srcfiles/cognition.js` and tuned (Phase 5) to 0.2514 at-home actions/npc-tick on the D2 target. |
| `src/src/ref/complete/knowledge-gossip-memory-plan.md` | Roadmap Plan 4 — the belief record (`provenance`/`confidence`/`salience`/`pinned`/`emotionalTag` on facts, `BELIEF`/`FACT_DISPLAY`), gossip transmission (`npc_chat` payload + overhearing), deterministic rumination + open questions, the D11 player model, the D13 bridge, and the Character Studio (D12/D16/D17: profile tabs, `validateNpcField`, Edit Mode). **Built** — all 5 phases, D1–D23 locked. |
...
| `src/src/ref/wip/actions-and-activities-overhaul-plan.md` | `actions-and-activities-handoff-prompt.md` (**The actions & activities overhaul — fill the apartment with verbs.** Design session 2026-08-30 + partner design-review 2026-08-31 (citations checked against live code, D30–D36 added, Pets cut, sauna locked). **D1–D36 locked, 20 phases planned (1–19 + 1B), none started, design gate clear** — only Q4 parked, deferred to P17. Spine: a **central invitation system built on the Asks system** — `$Invite` ask leaf → `commitments.js` roster/eventType → booked events the scheduler runs, Clear-the-Calendar as the escape hatch (D1–D4). **Second spine, P1B**: room-entry stealth/peeping/phone-snooping already ship — the real gap was dead stealth XP; new scope is pickpocketing, Sneaking, an explicit phone-snoop-photo branch, cover-your-tracks (D32–D36). "Make a move" (`doMakeAMove`, ui.js:905) reroutes into the Ask tree with a physical-affection ladder, willingness gate the only sex door, sleeping/unaware targets routed through the existing boundary-act gate rather than a relaxed willingness floor, symmetric for NPC-on-player with real player agency (D5–D7, D30–D31). NPC **flags/conditions engine** on the freeuseofficeclicker `17-flags-detection.js` pattern (D15). Symmetric `player.moneyLedger` (D9) + gift/borrow/steal item ownership (D8). Plus reverse overtures (D10), follow (D11), belief-gated apology (D12), boundaries-as-flags (D13), cook-off-as-event (D14), temperature + clothing (D16), cleaning/dirt incl. Clean Hallway (D17), kitchen/dining + bathroom/grooming (D18–D19), 5-state laundry + Snoop (D20), mailbox/answer-the-door (D21), the **East Wing hotspot (declared priority)** — pool games, loungers, lockers, a sauna subroom, balcony, pool parties (D22), crossword minigame (D23), Chatter social layer (D24), skill research (D25), house parties + touring (D26–D27), original music + SFX (D29). Pets (D28) cut entirely. Dep: P1B precedes P2/P4/P5; P3 flags → P7/P8/P17; P9 cleaning → P17; P19 audio parallel anytime.) |
| `src/src/ref/complete/mobile-layout-overhaul-plan.md` | Fixes the mobile horizontal crop (root cause found & verified 2026-08-15: `#footer`'s `min-width: auto` lets the 1114px action-chip row blow the whole `#app` grid out on phones) and polishes the play shell, header, chips, and overlay sweep for portrait/touch. **Complete — all three phases built and verified live 2026-08-15.** |
| `src/src/ref/complete/floorplan-and-movement-plan.md` | Moved from `wip/` 2026-08-18. The apartment stops being a collection of boxes. Supersedes the floor-plan half of `apartment-expansion-plan.md` (the Mirrored H concept survives; its coordinates do not). **All 6 phases plus both follow-ups built, 94 assertions in `verify-plan.js`.** The measurement that motivated it: **zero of seventeen** declared adjacencies shared a wall, and the graph was a **tree** — one route between any two rooms. Now 19 rooms tile on a hand-drawn plan (a room is a LIST of rects, so the L-shaped Gym and Living Room survive), `ROOM_THRESHOLDS` types every edge `door`/`open`/`glass` and the signal layer reads it per-edge instead of per-room, `resolveWalk` (WORLD, pure/deterministic/model-free) auto-paths with blockers and interrupts for **seconds derived from the geometry**, the renderer draws walls with the doorways cut out of them over ~40 top-down furniture symbols that reflect live object state, and `STRUCTURAL_UPGRADES` edits the graph itself (`ROOM_ADJACENCY`/`ROOM_THRESHOLDS` are rebuilt in place from base + built upgrades, so all thirty-odd existing readers are untouched). Consequences worth knowing: the south bedroom wing sits behind a zero-threshold archway off the kitchen and gets 3.3× the cooking smell the north wing does — deliberate, and the rent model does not price it yet; and `doMove` now enforces locked doors, which it never had. Geometry is authored in `dev/mapper.html` — **open it before editing any adjacency by hand.** |
| `src/src/ref/complete/CONTINUOUS-SIMULATION-ROADMAP.md` | **Umbrella for five linked plans** — all five built: Plan 1 (continuous behavior engine), 2 (the heartbeat), 3 (external-world retiming), 4 (npc-initiative retiming) and 5 (the decor economy), each moved to `complete/`. Retires the 30-minute tick as the unit NPC decisions happen in: `SCHEDULES`, `resolveScheduleActivity`, and every tick-counted field across the codebase convert to `clockToAbsolute`-space commitments. Holds the cross-cutting decisions `C1`–`C8` all five plans inherit (the action/drive boundary, anchors, movement, determinism, closed-form fast-forward, geometric perception carried forward as an unscheduled thesis). Documentation only, design session 2026-08-14. |
| `src/src/ref/complete/continuous-behavior-engine-plan.md` | The core of the five — `npc.commitment` (real absolute-minute completion times, `kind:'action'\|'drive'\|'work'`) replaces `npc.pursuit`'s tick-counted hold; event-driven decision scheduling (each NPC's own next-decision time) replaces the flat per-tick scan of every active NPC; routine survives as a time-of-day scoring weight, not a hard `blockFilter` gate; work/commute is ONE long commitment with a real front-door walk (D5); absorbs `floorplan-and-movement-plan.md`'s position/walk/render-split work as its own physical-output layer. **Built — all 6 phases, D1–D16.** Phase 6 (tuning + live pass) landed `COGNITION.recencyWindow` 2→1.5, the commute-block grace on `ageCommitment`'s release, the pass-3 transit fall-through, and the heartbeat status-strip redraw; verified over three live simulated days (meals 1.33/npc-day with breakfast appearing, showers 1.0/day, every work start at its boundary or one 30-min tick later). A 2026-08-15 audit found Phase 3's `kind:'action'` anchor path was dead code and wired it (see the plan's Addendum). |
| `src/src/ref/complete/npc-initiative-retiming-plan.md` | Every `*CooldownTicks` field and the shared `isOnCooldown`/`setCooldown` mechanism (`drives.js`) convert to absolute-minute comparison, closing the representational hole behind **D34** (`npc-initiative-plan.md`'s finding: a cooldown at or above `ticksPerDay` could never elapse) rather than re-tuning around it. Includes `cognition.js`'s `recencyMultiplier`, a second wrapped-delta site sharing the same field. **Built** — all 3 phases, D1–D5 locked. Phase 3 re-measured the population rates live: conversion behavior-invisible at the mechanism level (zero cooldown violations across 524 stamps; monotone text-cooldown lever sweep; propose 0.425 vs published 0.437), totals drifted from Plan 5's published table with the continuous behavior-engine overhaul — D5 holds the current baseline for behavior-engine Phase 6. |
| `src/src/ref/complete/continuous-simulation-handoff-prompt.md` | The session prompt for the five rows above — hand this to an implementation session, not the roadmap. One prompt, several linked plans (mirrors `patterns/perchance-agent-handoff-prompt.md`'s shape, not the one-prompt-per-plan convention `complete/` otherwise uses): a fixed 20-row dependency-safe phase checklist across all five documents, the standing hard technical rules (two-place load-order registration, `state.js`-only kv access, determinism), and a translation rule mapping every `dev/verify/*.js` reference in the five plans onto `browser_eval` against the live generator page — written for a Perchance-native agent that has no Node/shell access, only `read`/`write`/`edit`/`patch`/`execute_js`/`browser_eval`/`browser_refresh`/`vision`. |
...
>
> A later cleanup (2026-08-04) deleted `src/src/ref/HANDOFF.md`,
> `src/src/ref/vocation-and-gigs-plan.md`, `src/src/ref/sleep-and-alarm-plan.md`,
> `src/src/ref/adult-content-overhaul-plan.md`, and `src/src/ref/BrineOS-The-Phone-plan.md` —
...
> `src/src/ref/HANDOFF.md`'s operational notes (iframe testing technique, load
...
`cutoutKey`/`getCharacterCutout`. `spritestudio.js` (Phase 4, the studio's
...
already started. It calls into SIM/IMAGE/STATE at runtime only and needs
nothing at load time, so "as late as possible, but before boot" is exactly
right.

`peek.js` (Intimacy & Voyeurism Phase 10, D6/D7) sits between `image.js` and
`render.js`: its pure half (peekRiskPerTick, peekCaughtChance,
peekOutcomeWeights, resolvePeekCaughtOutcome, composePeekViewLine,
peekImageBudgetAllows/Spend, pickPeekProse) is loaded — and exercised by
...
**A new `src/src/srcfiles/*.js` needs a line in TWO places**: `index.html`'s script
...
**The same failure mode, one level up (2026-08-28).** `loadEngine()` swallows
a missing file unless the caller passes `required: [...]` — so when the whole
`SRC` path is wrong, it loads ZERO files, every `api()` call throws
`ReferenceError` into a `check()` that scores it as a plain FAIL, and a
harness with no `required:` prints a summary line that looks like a real run.
The tree sat in exactly that state: `loadgame.js` and ~57 harnesses pointed
at `srcfiles/` instead of `src/src/srcfiles/`, and six used a path one level
above the repo root. **Always pass `required:`**, and if a suite result ever
looks surprisingly clean or surprisingly broken, check `loaded.length`
before debugging anything else.

**The entry document is `index.html`** (renamed from `main.html` 2026-08-28
and normalised across every doc, harness and comment). Perchance's own
uploader exposes it as a fixed field with no filename, so the local name is
ours to choose — but it must be chosen ONCE: the two names coexisting is
what let a stale copy silently revert 57 harnesses.
...
`root.generateText`) to fully boot, P0 was verified by loading `index.html`
...
snapshots `index.html` and does not re-fetch `<script src>` tags on
...
against this file requires injecting a fresh `<iframe src="index.html?...">`
...
(this browser preview snapshots `index.html`, see the P2 note above — every
...
child of `#main-content` (`index.html`), shown/hidden purely via
...
8 rooms → 17. See `src/src/ref/complete/apartment-expansion-plan.md` for the layout
...
designs they are heading toward live in `src/src/ref/sleep-and-alarm-plan.md` and
`src/src/ref/complete/economy-and-rent-plan.md`.
...
- **AfterHours is live.** Categories map to search queries against the host's
  ...
  host's own site is planned (`src/src/ref/adult-content-overhaul-plan.md`).
...
**Plan:** `src/src/ref/complete/game-opening-plan.md`.
...
**Plan:** `src/src/ref/sleep-and-alarm-plan.md`.
...
**Plan:** `src/src/ref/complete/apartment-upgrades-plan.md`.
...
**Plan:** `src/src/ref/adult-content-overhaul-plan.md` ("Still open" section).
...
**Plan:** `src/src/ref/complete/economy-and-rent-plan.md` (investing as accelerator).
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. These land first so the phone
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. Done on the computer only, while
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. No shell yet — this phase gives
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. The phone's first *shell*: an
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. One pure derived pass turns game
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. Two halves: make every shared app
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. Premise correction made at the
...
worked, reachable only by typing a command — `src/src/ref/sleep-and-alarm-plan.md`'s
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. Deliberately its own phase,
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. A photo record deliberately does
...
**Plan:** `src/src/ref/BrineOS-The-Phone-plan.md`. The final BrineOS phase — makes
...
`src/src/ref/HANDOFF.md` warns about: `applyMemoryEpisodeEffect` *replaces*
