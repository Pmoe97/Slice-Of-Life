...
async function doBrowserVisit(siteId, device) {
  ...
  try {
    ...
    // AfterHours: trigger the initial clip fetch after the screen renders.
    // Write the resolved category back before fetching — saves made before
    // afterHoursCategory had a default carry it as undefined, and
    // fetchAfterHoursClips' staleness guard would reject its own first call.
    if (siteId === 'afterhours') {
      ...
      if (!browser.afterHoursClips && !browser.afterHoursClipsLoading) {
...
// AfterHours category tab switch — updates the filter and fetches fresh
// clips from Pornhub's webmaster API for the new category. No time cost.
...
// Phase 10: search AfterHours. Sets the search query and refetches.
function doAfterHoursSearch(query) {
  const browser = currentGameState.world.computer.apps.browser;
  browser.afterHoursSearchQuery = (query || '').trim();
  browser.afterHoursWatching = null;
  browser.afterHoursClips = null;
  browser.afterHoursClipsLoading = false;
  browser.afterHoursClipsError = null;
...
// Phase 10: navigate to a specific page
...
// Refresh button — re-fetch clips for the current category (next page)
function doAfterHoursRefresh() {
  const browser = currentGameState.world.computer.apps.browser;
  const catId = browser.afterHoursCategory || 'featured';
  browser.afterHoursCategory = catId; // see doBrowserVisit — the guard needs it set
  browser.afterHoursClips = null;
  browser.afterHoursClipsLoading = false;
  browser.afterHoursClipsError = null;
...
// Close the embed player — stop watching, keep the grid
...
// continuous clock, advances MASTURBATION.timeCostMinutes game-minutes,
// applies cum effects, then returns to browsing context. After effects,
// rolls the interruption check (Phase 5) — an NPC may walk in on the
...
async function doAfterHoursCum() {
  ...
  try {
    ...
    // Phase 5: roll interruption check
    ...
    if (result) {
      ...
      // Discard any pending pre-generated text
      pendingInterruption = null;
    }
  } finally {
    hideLoading();
...
// AfterHours content watch — applies watch effects, then opens the
// embed player for the selected clip. The player iframe loads the
...
async function doAfterHoursWatch(clipId, device) {
  ...
  // BROWSING — free, no time cost, no effects. Just open the player.
  // Time and effects happen when the player masturbates and cums (Phase 3).
  browser.afterHoursWatching = clipId;
  // Switching clips while masturbating is fine — keeps the session going
  // on the new clip. No need to reset masturbating state.

  addLogEntry('narration', `You start watching "${clip.title}" on AfterHours.`);
  await saveAtBoundary('ah-watch', currentGameState);

  // Only render the computer screen — calling render() would trigger a
...
// Fetch clips from Pornhub's webmaster API via superFetch. Maps the
// AfterHours category search term to a Pornhub search query, parses the
// JSON, and stores normalized clips in browser.afterHoursClips. Guards
// against the category having changed while the request was in flight.
// Phase 10: supports free-text search queries and pagination.
async function fetchAfterHoursClips(catId) {
  ...
  try {
    ...
    // Phase 10: free-text search overrides the category search.
    ...
    if (userQuery) {
      url += `&search=${encodeURIComponent(userQuery)}`;
    } else if (search) {
    ...
    const clips = videos.map(v => {
      ...
      // Prefer JPG/PNG thumbs from pix-fl/ei CDNs; pix-cdn77 returns
      // MP4 preview clips that can't render as <img> elements. Search
      // all available thumb URLs for a usable one.
      const allThumbs = [v.default_thumb, v.thumb, ...(v.thumbs?.map(t => t.src) || [])].filter(Boolean);
      let thumb = allThumbs.find(u => !u.includes('pix-cdn77')) || allThumbs[0] || '';
      return {
        id: videoId,
        title: v.title,
        duration: v.duration,
        ...
        // Phase 10: store the full URL for "watch on site" fallback.
        watchUrl: v.url || `https://www.pornhub.com/view_video.php?viewkey=${videoId}`,
      };
    }).filter(c => c.id);

    if (browser.afterHoursCategory !== catId) return; // stale
    browser.afterHoursClips = clips;
    // Phase 10: track total pages for pagination display.
    browser.afterHoursTotalPages = Math.max(1, Math.min(50, j.totalPages || (clips.length > 0 ? page + 1 : page)));
  } catch (e) {
